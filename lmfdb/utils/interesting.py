...
def interesting_knowls(category, table, url_for_label,
                       label_col="label", regex=None, query={}, **kwds):
    # category might be something like ec.q
    main_cat = category.split(".")[0]
    if category[-1] != ".":
        category += "."
    knowls = knowldb.search(
        category=main_cat, types=[
            "top", "bottom"], filters=[
            "beta", "reviewed"], projection=[
                "id", "title", "type"])
    ...
    if query:
        ...
        # and others might appear multiple times (higher genus families, where
        # the "label" is for a family and isn't unique)
        sorted_labels = {lab: i for i, lab in enumerate(
            table.search({label_col: {"$in": labels}}, label_col))}
        if sorted_labels:
            M = max(sorted_labels.values()) + 1
        else:
            M = 1
        knowls = [
            unsorted[lab] for lab in sorted(
                unsorted.keys(),
                key=lambda x: sorted_labels.get(
                    x,
                    M))]
