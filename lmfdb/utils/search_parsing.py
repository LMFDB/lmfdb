...
def prep_raw(inp, names={}):
    ...
    inp = implicit_mul(
        inp, level=10
    )  # level = 10 includes (a+b)(c+d) -> (a+b)*(c+d) which isn't safe in Sage but should be okay for us
...
@search_parser(
    clean_info=True, prep_plus=True
...
def parse_rational_to_list(inp, query, qfield):
    if QQ_RE.match(inp):
        ...
        opts = [
            {qfield: [QQ(x).numerator(), QQ(x).denominator()]} for x in inp.split(",")
        ]
        collapse_ors(["$or", opts], query)
    else:
        raise SearchParsingError(
...
@search_parser(
    clean_info=True, prep_ranges=True
)  # see SearchParser.__call__ for actual arguments when calling
...
@search_parser(
    clean_info=True, clean_spaces=False, prep_ranges=False
)  # see SearchParser.__call__ for actual arguments when calling
def parse_ints_raw(inp, query, qfield, names={}):
    ...
    if re.search(r"[A-Za-z]", inp):
        collapse_ors(
            parse_range2(
                inp, qfield, lambda inp: prep_raw(inp, names), split_minus=False
            ),
            query,
        )
    else:
    ...
    if "," in arg:
        ...
        if mod1:
            ...
            if s > e:
                ...
                return (
                    ["$or", [{key: {"$gte": s - eps()}}, {key: {"$lte": e + eps()}}]],
                    "%s-0.5,-0.5-%s" % (start, end),
                )
        return ([key, {"$gte": s - eps(), "$lte": e + eps()}], "%s-%s" % (start, end))
    else:
        x = my_float(arg)
        if mod1:
            x = reduce_mod1(x)
        ...
        return [key, {"$gte": x - eps(prec), "$lte": x + eps(prec)}], "%s-%s" % (
            start,
            end,
        )
...
def parse_element_of(
    inp, query, qfield, split_interval=False, parse_singleton=int, contained_in=None
):
    if split_interval:
...
@search_parser
def parse_subset(
    inp, query, qfield, parse_singleton=None, mode=None, radical=None, product=prod
):
...
def parse_bracketed_posints(
    ...
    if (
        ...
        raise SearchParsingError(
            "It needs to be a %s in square brackets, such as %s." % (lstr, example)
        )
    else:
        if (
            inp == "[]"
...
@search_parser(
    clean_info=True, error_is_safe=True
)  # see SearchParser.__call__ for actual arguments when calling
...
def nf_string_to_label(FF):  # parse Q, Qsqrt2, Qsqrt-4, Qzeta5, etc
    ...
    if (
        F1 and len(str(F1.parent().gen())) == 1
    ):  # we only support single-letter variable names
    ...
    if F[0] == "q":
        ...
        if F[1:5] in ["sqrt", "root"]:
            ...
            if d == 0:
                raise SearchParsingError(
                    "After {0}, the remainder must be a nonzero integer.  Use {0}5 or {0}-11 for example.".format(
                        FF[:5]
                    )
                )
            if d == 1:
        ...
        if F[0:5] == "qzeta":
            ...
            if not match_obj:
                raise SearchParsingError(
                    "After {0}, the remainder must be a positive integer or a positive integer followed by '+'.  Use {0}5 or {0}19+, for example.".format(
                        F[:5]
                    )
                )
...
def input_to_subfield(inp):
    ...
    if F[0] == "q":
        ...
        if F[1:5] in ["sqrt", "root"]:
            ...
            if d == 0 or d == 1:
                raise SearchParsingError(
                    "After {0}, the remainder must be a nonzero integer which is not a perfect square.  Use {0}5 or {0}-11 for example.".format(
                        inp[:5]
                    )
                )
...
def parse_nf_elt(inp, query, name, qfield, field_label):
    ...
    if "," in inp:
        collapse_ors(
            ["$or", [{qfield: pol_string_to_list(f, deg=deg)} for f in inp.split(",")]],
            query,
        )
    else:
...
@search_parser(
    pass_name=True, clean_info=True
)  # see SearchParser.__call__ for actual arguments when calling
        ...
        if "," in inp:
            collapse_ors(
                [
                    "$or",
                    [
                        {qfield: ",".join(pol_string_to_list(f, deg=deg))}
                        for f in inp.split(",")
                    ],
                ],
                query,
...
@search_parser
def parse_regex_restricted(
    inp, query, qfield, regex, err=None, errknowl=None, errtitle=None, comma_split=True
):
    if comma_split:
        inps = inp.split(",")
    else:
...
):  # Note that postgres -> index is one-based
    for piece in inp.split(","):
        piece = piece.strip().split("=")
        if len(piece) != 2:
            raise SearchParsingError(
                "It must be a comma separated list of expressions of the form %sN=T"
                % (prefix)
            )
        n, t = piece
        n = n.strip()
    ...
    def make_sub_query(part):
        ...
        if part[0] == "[":
            ...
            if parse_singleton is str:
                ...
                all_operand = [
                    val for val in parsed_values if type(val) == parse_singleton
                ]
    ...
    def make_sub_query(part):
        ...
        if part[0] == "[":
            ...
            if "-" in part[1:]:
                raise SearchParsingError("Ranges not supported.")
            sub_query[qfield] = {
                "$startswith": "%s %s " % (" ".join(initial_segment), part)
            }
        return sub_query
