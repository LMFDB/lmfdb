...
def modlmf_render_webpage():
    ...
    if len(args) == 0:
        ...
        level_list = ["%s-%s" %
                      (start, end -
                       1) for start, end in zip(level_list_endpoints[:-
                                                                     1], level_list_endpoints[1:])]
        ...
        return render_template("modlmf-index.html", info=info, credit=credit,
...
def modlmf_by_label(lab):
    ...
    if modlmf_label_regex.match(lab):
        flash_error(
            "The mod &#x2113; modular form %s is not recorded in the database or the label is invalid",
            lab)
    else:
        flash_error(
            "No mod &#x2113; modular form in the database has label %s",
            lab)
...
def download_search(info):
    ...
    s = '\n'
    s += c + \
        ' Mod l modular forms downloaded from the LMFDB on %s. Found %s mod l modular forms.\n\n' % (
            mydate, len(res))
...
def render_modlmf_webpage(**args):
    ...
    if data is None:
        ...
        bread = [
            ('mod &#x2113; modular forms',
             url_for(".modlmf_render_webpage"))]
        flash_error(
            "%s is not a valid label for a mod &#x2113; modular form in the database.",
            lab)
        return render_template("modlmf-error.html", title=t,
    ...
    bread = [('Modular forms', "/ModularForm"), ('mod &#x2113;',
                                                 url_for(".modlmf_render_webpage")), ('%s' % data['label'], ' ')]
    credit = modlmf_credit

    for m in ['characteristic', 'deg', 'level', 'weight_grading',
              'n_coeffs', 'min_theta_weight', 'ordinary']:
        info[m] = int(data[m])
    ...
    if data['coeffs'] != "":
        ...
        info['q_exp_display'] = url_for(
            ".q_exp_display", label=data['label'], number="")
        ...
        t = "Mod " + str(info['characteristic']) + \
            " modular form " + info['label']
    info['properties'] = [
        ('Label', '%s' % info['label']),
    ...
    return render_template("modlmf-single.html", info=info, credit=credit, title=t, bread=bread,
                           properties=info['properties'], learnmore=learnmore_list(), KNOWL_ID='modlmf.%s' % info['label'])
...
def completeness_page():
    ...
    bread = [('Modular forms', "/ModularForm"), ('mod &#x2113;',
                                                 url_for(".modlmf_render_webpage")), ('Completeness', '')]
    credit = modlmf_credit
...
def how_computed_page():
    ...
    bread = [('Modular forms', "/ModularForm"), ('mod &#x2113;',
                                                 url_for(".modlmf_render_webpage")), ('Source', '')]
    credit = modlmf_credit
...
def labels_page():
    ...
    bread = [('Modular forms', "/ModularForm"), ('mod &#x2113;',
                                                 url_for(".modlmf_render_webpage")), ('Labels', '')]
    credit = modlmf_credit
...
def download_modlmf_full_lists(**args):
    ...
    res = db.modlmf_forms.lookup(
        label, projection=[
            'characteristic', 'deg', 'coeffs'])
    mydate = time.strftime("%d %B %Y")
    if res is None:
    ...
    c = download_comment_prefix[lang]
    outstr = c + \
        ' List of q-expansion coefficients downloaded from the LMFDB on %s. \n\n' % (
            mydate)
    if lang == 'magma':
