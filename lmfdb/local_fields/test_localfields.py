...
class LocalFieldTest(LmfdbTest):
    ...
    def test_field_page(self):
        ...
        # assert 'x^{3} - 11 t' in L.get_data(as_text=True) # bad (not robust)
        # test, but it's the best i was able to find...
    ...
    def test_search_download(self):
        page = self.tc.get(
            '/padicField/?Submit=gp&download=1&query=%7B%27p%27%3A+2%2C+%27n%27%3A+2%7D&n=2&p=2').get_data(as_text=True)
