...
def local_algebra_data(labels):
    labs = labels.split(',')
    f1 = labs[0].split('.')
    labs = sorted(labs, key=lambda u: (int(j)
                                       for j in u.split('.')), reverse=True)
...
def local_field_data(label):
    ...
    ans += r'Extension of $\Q_{%s}$ defined by %s<br>' % (
    ...
    ans += '<div align="right">'
    ans += '<a href="%s">%s home page</a>' % (
        str(url_for("local_fields.by_label", label=label)), label)
    ans += '</div>'
    return ans
...
def lf_display_knowl(label, name=None):
    if name is None:
        name = label
    return '<a title = "%s [lf.field.data]" knowl="lf.field.data" kwargs="label=%s">%s</a>' % (
        label, label, name)


def local_algebra_display_knowl(labels):
    return '<a title = "{0} [lf.algebra.data]" knowl="lf.algebra.data" kwargs="labels={0}">{0}</a>' % (
        labels)
...
def plot_polygon(verts, polys, inds, p):
    ...
    L = Graphics()
    if ymax > 0:
        # 2 comes from the fact that the actual image has width 500 and height
        # 250.
        asp_ratio = (xmax + 2 * txshift) / (2 * (ymax + 2 * tyshift))
    else:
    ...
    for i in range(len(verts) - 1):
        P = verts[i]
        Q = verts[i + 1]
        # actually the negative of the slope
        slope = ZZ(P[1] - Q[1]) / ZZ(Q[0] - P[0])
        d = slope.denominator()
        poly = polys[i]
        if slope != 0:
            while nextq <= Q[0]:
            ...
            # For tame inertia, the coefficients can occur at locations other
            # than powers of p
    ...
    L += points([(p**i, ind)
                 for i, ind in enumerate(inds)], size=30, color="black")
    L.axes(False)
    L.set_aspect_ratio(asp_ratio)
    return encode_plot(L, pad=0, pad_inches=0,
                       bbox_inches="tight", figsize=(8, 4))
...
def index():
    ...
    return render_template("lf-index.html", title="$p$-adic fields",
                           titletag="p-adic fields", bread=bread, info=info, learnmore=learnmore_list())
...
lf_columns = SearchColumns([
    ...
    MathCol("e", "lf.ramification_index", "$e$",
            short_title="ramification index"),
    MathCol("f", "lf.residue_field_degree", "$f$",
            short_title="residue field degree"),
    MathCol("c", "lf.discriminant_exponent", "$c$",
            short_title="discriminant exponent"),
    ...
    PolynomialCol(
        ...
        "Unram. Ext.",
        default=lambda info:info.get("visible")),
    ProcessedCol(
        "eisen",
    ...
    ProcessedCol(
        ...
        "Ind. of Insep.",
        formatbracketcol,
        default=lambda info: info.get("ind_of_insep")),
...
def lf_postprocess(res, info, query):
    cache = knowl_cache(
        list({f"{rec['n']}T{rec['gal']}" for rec in res if 'gal' in rec}))
    for rec in res:
        rec["cache"] = cache
    return res
...
def local_field_search(info, query):
    ...
    parse_rats(
        ...
        name='Top slope',
        process=ratproc)
    parse_newton_polygon(
        info,
        query,
        "slopes",
        qfield="slopes_tmp",
        mode=info.get('slopes_quantifier'))
    parse_newton_polygon(
        info,
        query,
        "visible",
        qfield="visible_tmp",
        mode=info.get('visible_quantifier'))
    parse_newton_polygon(
        info,
        query,
        "ind_of_insep",
        qfield="ind_of_insep_tmp",
        mode=info.get('insep_quantifier'),
        reversed=True)
    ...
    if 'label' in args:
        ...
        if data is None:
            if LF_RE.fullmatch(label):
                ...
                flash_error(
                    "%s is not a valid label for a $p$-adic field.", label)
        ...
        if data.get('galois_label'):
            ...
            galphrase = 'This field is' + isgal + \
                abelian + r' over $\Q_{%d}.$' % p
            if the_gal.order() == gn:
                auttype = 'gal'
        ...
        # we don't know the Galois group, but maybe the Aut group is obvious
            ...
            info['aut_gp_knowl'] = abstract_group_display_knowl(
                f"{data['aut']}.1")
        prop2 = [
            ('Label', label),
            ('Base', r'\(%s\)' % Qp),
            ...
            ('Galois group', group_pretty_and_nTj(gn, gt)
             if data.get('galois_label') else 'not computed'),
        ]
        ...
        if data['f'] == 1:
            ...
            unramp = prettyname(unramdata) + ' $\\cong ' + \
                Qp + '(t)$ where $t$ is a root of ' + unramp
            eisenp = Ptx(str(data['eisen']).replace('y', 'x'))
            eisenp = raw_typeset(
                str(eisenp),
                web_latex(eisenp),
        ...
        rflabel = db.lf_fields.lucky(
            {'p': p, 'n': {'$in': [1, 2]}, 'rf': data['rf']}, projection=0)
        if rflabel is None:
        ...
        if data['f'] == 1 or data['e'] == 1:
            ...
            eform = '$' + \
                eisensteinformlatex(data['coeffs'], data['unram']) + '$'
        ...
        if data.get('ram_poly_vert'):
            info.update({'ram_polygon_plot': plot_polygon(
                data['ram_poly_vert'], data['residual_polynomials'], data['ind_of_insep'], p)})
        if data.get('residual_polynomials'):
            info.update({'residual_polynomials': ",".join(
                f"${teXify_pol(poly)}$" for poly in data['residual_polynomials'])})
        if data.get('associated_inertia'):
            info.update({'associated_inertia': ",".join(
                f"${ai}$" for ai in data['associated_inertia'])})
        if data.get('galois_label'):
...
def statistics():
    ...
    return render_template("display_stats.html", info=LFStats(
...
# We want to look up gap ids and names only once, rather than once for
# each Galois group
