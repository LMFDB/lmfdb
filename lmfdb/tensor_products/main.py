...
# The method "show" shows the page for the Lfunction of a tensor product
# object.  This is registered on to the tensor_products_page blueprint
# rather than going via the l_function blueprint, hence the
# idiosyncrasies.  Sorry about that.  The reason is due to a difference in
# implementation; the tensor products are not (currently) in the database
# and the current L functions framewo
...
def index():
    bread = get_bread()
    return render_template("tensor_products_index.html",
                           title="Tensor Products", bread=bread)
...
def navigate():
    ...
    return render_template("tensor_products_navigate.html",
                           title="Tensor Products Navigation", bread=bread, type1=type1, type2=type2)
    ...
    if len(galoisRepObjs) == 2:
        ...
        try:
            ...
            if (tp.numcoeff > len(tp.dirichlet_coefficients) + 10):
                info['zeroswarning'] = 'These zeros may be inaccurate because we use only %s terms rather than the theoretically required %s terms' % (
                    len(tp.dirichlet_coefficients), tp.numcoeff)
