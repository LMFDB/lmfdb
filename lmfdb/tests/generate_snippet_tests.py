...
def _start_snippet_procs(langs, chimp_spec=None):
    ...
    for lang in langs:
        if lang == 'oscar':
            ...
            #  The random seed is reset per snippet file rather than here, see _eval_code_file.
...
def _eval_code_file(data, lang, proc, logfile):
    ...
    with logfile.open('w') as f:
        # Magma seeds its random number generator from the clock, and several of its algorithms
        # (e.g. the decomposition underlying NewformDecomposition) are randomised.  The answers are
        # always correct, but the choice of generator of a number field can differ from run to run,
        # which shows up as a spurious diff in the log file (e.g. a q-expansion printed in terms of
        # a rather than -a).  Reset the seed for every snippet file, so that a log does not depend
        # on which snippets happened to run before it in the same process.  As with the CHIMP spec
        # above, this is done while no logfile is attached, so it does not appear in the log.
        # (proc.child.logfile still refers to the previous file's handle, which is now closed.)
        if lang == 'magma':
            proc.child.logfile = None
            proc.run_command('SetSeed(1);')
