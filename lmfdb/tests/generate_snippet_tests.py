...
def _eval_code_file(data, lang, proc, logfile):
    ...
    def send_and_wait(line, timeout=60*3):
        """ Send 'line' and return True if the REPL is left at its
        continuation prompt, i.e. the statement is unfinished
        """
        proc.child.sendline(line)
        # index 1 means we matched the continuation prompt
        return proc.child.expect_exact(patterns, timeout=timeout) == 1
