...
class TemplateNamespaceCollisionTest(LmfdbTest):
    ...
    def test_template_collisions(self):
        ...
        templates = list(itertools.chain.from_iterable(
            [(x, cur) for x in files] for cur, dir, files in os.walk('.') if cur.split('/')[-1] == 'templates'))
        ...
        if collisions:
            print("")
        for x in dups:
            print("Template file %s appears in: %s" %
                  (x, [r[1] for r in collisions if r[0] == x]))
