...
class DynamicKnowlTest(LmfdbTest):
    ...
    def test_prod_knowl_sync(self):
        ...
        if db.config.postgresql_options["host"] == "proddb.lmfdb.xyz":
            ...
            for D in [dev_config.default_args["postgresql"],
                      dev_config.postgresql_options, dev_config.options["postgresql"]]:
            ...
            # Updates happen every 20 minutes, so we only compare knowls older
            # than that (plus a buffer).
            cutoff = datetime.utcnow() - timedelta(minutes=30)

            t_query = SQL(
                "SELECT timestamp FROM kwl_knowls WHERE timestamp < %s LIMIT 1")
            dev_t = dev_db._execute(t_query, [cutoff]).fetchone()[0]
            prod_t = db._execute(t_query, [cutoff]).fetchone()[0]

            cnt_query = SQL(
                "SELECT COUNT(*) FROM kwl_knowls WHERE timestamp < %s")
