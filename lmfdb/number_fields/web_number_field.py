    ...
    if d == '4':
        ...
        if len(subs) == 3:  # only for V_4 fields
            ...
            if not any(z._data is None for z in subs):
                ...
                labels = sorted(
                    [[integer_squarefree_part(ZZ(z[2])), - int(z[1])] for z in labels])
                # put in +/- sign
...
def formatfield(coef, show_poly=False, missing_text=None,
    ...
    if thefield is None or thefield._data is None:
        ...
        if missing_text is None:
            mypol = '<a title = "Field missing" knowl="nf.field.missing" kwargs="poly=%s&raw=%s">deg %d</a>' % (
                mypol, mypolraw, deg)
        else:
            if link:
                jump_link = str(
                    ...
                    '?jump=%s' %
                    mypol)
                mypol = '<a title = "Field with link to db" knowl="nf.field.link" kwargs="poly=%s&link=%s">%s</a>' % (
                    mypol, jump_link, missing_text)
            else:
                mypol = '<a title = "Field missing" knowl="nf.field.missing" kwargs="poly=%s">%s</a>' % (
                    mypol, missing_text)
        return mypol
    if data is None:
...
def nf_display_knowl(label, name=None):
    ...
    return '<a title = "%s [nf.field.data]" knowl="nf.field.data" kwargs="label=%s">%s</a>' % (
        name, label, name)
...
def nf_knowl_guts(label):
    ...
    out += '<br>Galois group: ' + \
        group_pretty_and_nTj(wnf.degree(), wnf.galois_t(), True)
    ...
    if wnf.is_galois():
        ...
        if 'gal' in res:
            ...
            galord = db.gps_transitive.lookup(
                '%dT%d' %
                (wnf.degree(), wnf.galois_t()), 'order')
            galstring = r'degree %d extension of $\Q$ is not in the database' % galord
    out += '<br>Galois closure: ' + galstring
    out += '</div>'
    out += '<div align="right">'
    out += '<a href="%s">%s home page</a>' % (
        str(url_for("number_fields.by_label", label=label)), label)
    out += '</div>'
    return out
...
class WebNumberField:
    ...
    def from_polredabs(cls, pol):
        return cls.from_coeffs([int(c)
                                for c in pol.coefficients(sparse=False)])
    ...
    def from_polynomial(cls, pol):
        ...
        pol *= pol.denominator()
        # For some reason the error raised by Pari on a constant polynomial is
        # not being caught
        if pol.degree() < 1:
    ...
    def rd(self):
        return RealField(300)(
            ZZ(self._data['disc_abs'])).nth_root(self.degree())
    ...
    def gg(self):
        if not self.haskey('gg'):
            self._data['gg'] = WebGaloisGroup.from_nt(
                self.degree(), self.galois_t())
    ...
    def discrootfield(self):
        ...
        return formatfield(
            rfcoeffs, missing_text=r'$\Q(\sqrt{%s}$)' % compress_int(newd, sides=5)[0])
    ...
    def myhelper(self, coefmult):
        ...
        return [nf_display_knowl(subfield.get_label(),
                                 subfield.field_pretty()), coefmult[1]]
    ...
    def siblings(self):
        ...
        if 'sib' in resall:
            ...
            helpout = [[len(string2list(a)) - 1, formatfield(a)]
                       for a in resall['sib']]
        else:
            helpout = []
        degsiblist = [[d, cnts[d], [dd[1]
                                    for dd in helpout if dd[0] == d]] for d in sorted(cnts)]
    ...
    def unit_galois_action_type_knowl(self):
        ...
        return '<a title = "%s [%s]" knowl="%s">%s</a>' % (
            ty, knowlid, knowlid, ty)
    ...
    def gpK(self):
        if not self.haskey('gpK'):
            ...
            basis = [Qx(el.replace('a', 'x'))
                     for el in self.zk()] if self.degree() > 1 else []
    ...
    def cnf(self):
        if self.degree() == 1:
            return r'=\mathstrut &amp \frac{2^1 (2\pi)^0 \cdot 1\cdot 1}{2\cdot\sqrt 1}' + \
                r'\cr' + r'= \mathstrut &amp; 1'
        ...
        if not self.haskey('class_group'):
            ltx = r'%s\frac{%s%s %s \cdot %s}{%s\cdot\sqrt{%s}}' % (
                approx1, r1term, r2term, 'R', 'h', w, disc)
            return ltx + \
                r'\cr\mathstrut &amp; \text{<td> some values not computed }'
        ...
        approx1 += r"\mathstrut &amp;"
        ltx = r'%s\frac{%s%s %s \cdot %s}{%s\cdot\sqrt{%s}}' % (
            approx1, r1term, r2term, str(reg), h, w, disc)
        ltx += "\\cr"
        ltx += r'\approx \mathstrut &amp; %s' % (
            2**r1 * (2 * RR(pi))**r2 * reg * h / (w * sqrt(RR(disc))))
        return ltx
    ...
    def disc_factored_latex(self):
        ...
        if D < 0:
            s = r'-\,'
        return s + factor_base_factorization_latex(
            factor_base_factor(D, self.ramified_primes()), cutoff=30)
    ...
    def class_group(self):
        if self.haskey('class_group'):
            ...
            return '$%s$, which has order %s' % (
                cg_string, self.class_number_latex())
        return na_text()
    ...
    def make_code_snippets(self):
        ...
        for lang in self.code['field']:
            f = self.poly()
            if lang == "pari":
                # In pari, x is the highest priority variable, so it's
                # impossible to create an extension on top of this field if we
                # use x.
