...
def bread_prefix(): return [
    ('Number fields', url_for(".number_field_render_webpage"))]
...
class DiscriminantCol(MultiProcessedCol):
    def display(self, rec):
        ...
        s = r'-\,' if D < 0 else ''
        D = factor_base_factorization_latex(
            factor_base_factor(D, rec['ramps']), cutoff=30)
        return '$' + s + D + '$'
...
def global_numberfield_summary():
    init_nf_count()
    return r'This database contains %s <a title="number fields" knowl="nf">number fields</a> of <a title="degree" knowl="nf.degree">degree</a> $n\leq %d$.  Here are some <a href="%s">further statistics</a>.  In addition, extensive data on <a href="%s">class groups of quadratic imaginary fields</a> is available for download.' % (
        comma(nfields), max_deg, url_for('number_fields.statistics'), url_for('number_fields.render_class_group_data'))
...
def render_groups_page():
    ...
    return render_template("galois_groups.html", al=group_alias_table(
...
def render_labels_page():
    ...
    return render_template("single.html", info=info, kid='nf.label',
                           title=t, bread=bread, learnmore=learnmore)
...
def render_discriminants_page():
    ...
    bread = [('Number fields', url_for(
        ".number_field_render_webpage")), ('Completeness', ' ')]
...
def render_class_group_data():
    ...
    if 'Fetch' in info:
        ...
        if info['filenamebase'] in ['cl3mod8',
                                    'cl7mod8', 'cl4mod16', 'cl8mod16']:
            filepath = "%s/%s/%s.%d.gz" % (class_group_data_directory,
                                           info['filenamebase'], info['filenamebase'], k)
    ...
    return render_template("class_group_data.html", info=info,
                           title=t, bread=bread, learnmore=learnmore)
...
def statistics():
    ...
    sn = galstatdict([u[max(len(u) - 1, 0)]
                      for u in nt_all], n, [len(u) for u in nt_all])
    an = galstatdict([u[max(len(u) - 2, 0)]
                      for u in nt_all], n, [len(u) - 1 for u in nt_all])
    # t-numbers for D_n
    dn_tlist = [1, 1, 2, 3, 2, 3, 2, 6, 3, 3, 2, 12, 2, 3, 2, 56, 2, 13, 2, 10,
                5, 3, 2]
    dn = galstatdict([nt_stats[(j + 1, dn_tlist[j])]
                      for j in range(len(dn_tlist))], n, dn_tlist)

    hdeg_stats = {j: nfstatdb.column_counts('degree', {'class_number': {
                                            '$lt': 1 + 10**j, '$gt': 10**(j - 1)}}) for j in range(1, 12)}
    hdeg_stats[0] = nfstatdb.column_counts('degree', {'class_number': 1})
    h = [sum(hdeg_stats[j].get(k + 1, 0) for k in range(max_deg))
         for j in range(12)]
    # if a count is missing it is because it is zero
    hdeg = [[hdeg_stats[j].get(deg + 1, 0)
             for j in range(12)] for deg in range(23)]
    has_hdeg_stats = nfstatdb.column_counts(
        'degree', {'class_number': {'$exists': True}})
    ...
    sighasclass = nfstatdb.column_counts(
        ['degree', 'r2'], {'class_number': {'$exists': True}})
...
def number_field_render_webpage():
    ...
    if not request.args:
        init_nf_count()
        discriminant_list_endpoints = [-10000, -1000, -
                                       100, 0, 100, 1000, 10000, 100000, 1000000]
...
def render_field_webpage(args):
    ...
    if nf.is_null():
        if re.match(r'^\d+\.\d+\.\d+\.\d+$', label):
            flash_error(
                "Number field %s was not found in the database.", label)
        else:
        ...
        if dirichlet_chars:
            data['dirichlet_group'] = [
                ...
                (url_for(
                    ...
                    number=j),
                    data['conductor'],
                    j) for j in dirichlet_chars]
            if len(data['dirichlet_group']) == 1:
                data['dirichlet_group'] = r'<span style="white-space:nowrap">$\lbrace$' + \
                    data['dirichlet_group'][0] + r'$\rbrace$</span>'
            else:
                data['dirichlet_group'] = r'$\lbrace$' + ', '.join(
                    data['dirichlet_group'][:-1]) + ', <span style="white-space:nowrap">' + data['dirichlet_group'][-1] + r'$\rbrace$</span>'
        if data['conductor'].is_prime() or data['conductor'] == 1:
            data['conductor'] = r"\(%s\)" % str(data['conductor'])
        else:
            factored_conductor = factor_base_factor(
                data['conductor'], ram_primes)
            factored_conductor = factor_base_factorization_latex(
                factored_conductor, cutoff=30)
            data['conductor'] = r"\(%s=%s\)" % (
                str(data['conductor']), factored_conductor)
    data['galois_group'] = group_pretty_and_nTj(n, t, True)
    data['aut_gp_knowl'] = abstract_group_display_knowl(
        db.gps_transitive.lookup(f'{n}T{t}', 'aut_label'))
    ...
    if D.abs().is_prime() or D == 1:
        ...
        data['discriminant'] = raw_typeset_int(
            D, extra=r"\(\medspace = %s\)" %
            data['disc_factor'])
    if nf.frobs():
            ...
            if ramified_algebras_data[j] is None:
                ...
                for mm in mydat:
                    ...
                    if len(mm) == 4:         # not in database
                        if mm[1] * mm[2] == 1:  # Q_p
                            loc_alg += '<td>$\\Q_{%s}$</td><td>$x$</td><td>$1$</td><td>$1$</td><td>$0$</td><td>%s</td><td>$%s$</td>' % (
                                pcomp, transitive_group_display_knowl("1T1", "Trivial"), show_slope_content([], 1, 1))
                        elif mm[1] * mm[2] == 2:  # quadratic
                            loc_alg += '<td></td><td>Deg $2$</td><td>${}$</td><td>${}$</td><td>${}$</td><td>{}</td><td>${}$</td>'.format(
                                mm[1], mm[2], mm[3], transitive_group_display_knowl("2T1", "$C_2$"), show_slope_content([], mm[1], mm[2]))
                            ...
                            loc_alg += '<td></td><td>Deg ${}$</td><td>${}$</td><td>${}$</td><td>${}$</td><td>{}</td><td>${}$</td>'.format(
                                mm[1] * mm[2], mm[1], mm[2], mm[3], transitive_group_display_knowl(f"{mm[2]}T{cyc}"), show_slope_content([], mm[1], mm[2]))
                        else:
                            loc_alg += '<td></td><td>Deg ${}$</td><td>${}$</td><td>${}$</td><td>${}$</td><td></td><td></td>'.format(
                        ...
                        loc_alg += '<td><a href="%s">%s</a></td><td>$%s$</td><td>$%d$</td><td>$%d$</td><td>$%d$</td><td>%s</td><td>%s</td>' % (
                            myurl, lab, mm[1], mm[2], mm[3], mm[4], mm[5], mysc)
            loc_alg += '</tr>\n'
        loc_alg += '</tbody></table>\n'

    ram_primes_raw = str(ram_primes).replace('L', '')[1:-1]
    ram_primes = [
        rf'\({compress_int(z,cutoff=30,sides=5)[0]}\)' for z in ram_primes]
    ...
    if nf.is_cm_field():
        ...
        for reflex_field in reflex_fields:
            if len(reflex_field['rf_coeffs']) > 1:
                reflex_fields_list.append(
                    ['', reflex_field['rf_coeffs'], reflex_field['multiplicity']])
                field_labels_dict[tuple(reflex_field['rf_coeffs'])] = "N/A"
        field_labels = db.nf_fields.search(
            {"$or": [{"coeffs": a[1]} for a in reflex_fields_list]}, ["label", "coeffs"])
        for field in field_labels:
            field_labels_dict[tuple(field["coeffs"])] = field["label"]
        for reflex_field in reflex_fields_list:
            reflex_field[0] = fake_label(
                field_labels_dict[tuple(reflex_field[1])], reflex_field[1])
        ...
        for reflex_field in reflex_fields_list:
            ...
            if field_labels_dict[tuple(reflex_field[1])] == "N/A":
                table = table + \
                    formatfield(reflex_field[1], data={
                                'label': field_labels_dict[tuple(reflex_field[1])]})
            else:
                table = table + \
                    formatfield(reflex_field[1], data={
                                'label': field_labels_dict[tuple(reflex_field[1])]})
    ...
    if nf.degree() == 1:
        info['friends'].append(
            ('L-function',
             url_for(
                 'l_functions.by_full_label',
                 label='1-1-1.1-r0-0-0')))
    info['friends'].append(
        ('Galois group', url_for(
            "galois_groups.by_label", label="%dT%d" %
            (n, t))))
    discrootfieldcoeffs = nf.discrootfieldcoeffs()[0]
    rf_label = db.nf_fields.lucky({'coeffs': discrootfieldcoeffs}, 'label')
    if rf_label:
        info['friends'].append(
            ('Discriminant root field', url_for(
                "number_fields.by_label", label=rf_label)))
    ...
    if galois_closure[0] > 0:
        if galois_closure[1]:
            ...
            if galois_closure[2]:
                info['friends'].append(
                    ('Galois closure', url_for(
                        ".by_label", label=galois_closure[2][0])))
        else:
    ...
    if arith_equiv[0] > 0:
        if arith_equiv[1]:
            ...
            for aelab in arith_equiv[2]:
                info['friends'].append(
                    ('Arithmetically equivalent sibling', url_for(
                        ".by_label", label=aelab)))
        else:
    ...
    try:
        ...
        pass
    return render_template("nf-show-field.html", properties=properties, title=title, bread=bread, code=nf.code, friends=info.pop(
        'friends'), downloads=downloads, learnmore=learnmore, info=info, formatfield=formatfield, KNOWL_ID="nf.%s" % label)
...
def by_label(label):
    ...
    try:
        ...
        flash_error(
            "%s is not a valid input for a <span style='color:black'>label</span>.  %s",
            label,
            str(err))
        return redirect(url_for(".number_field_render_webpage"))
...
def number_field_jump(info):
    ...
    try:
        ...
        # we only use the output of input_string_to_poly with single-letter
        # variable names
        if F1 and len(str(F1.parent().gen())) == 1 and F1.list(
        ) != db.nf_fields.lookup(query['label'], 'coeffs'):
            flash_info(r"The requested field $\Q[{}]/\langle {}\rangle$ is isomorphic to the field below, but uses a different defining polynomial.".format(
                str(F1.parent().gen()), latex(F1)))
...
# return render_template("number_field_algebra.html", info=info, title=t,
# bread=bread)
...
nf_columns = SearchColumns([
    ...
    MultiProcessedCol(
        "signature", "nf.signature", "Signature", [
            "r2", "degree"], lambda r2, degree: '[%s,%s]' %
        (degree - 2 * r2, r2), apply_download=False, align="center", default=False),
    DiscriminantCol("disc", "nf.discriminant", "Discriminant", [
                    'disc_sign', 'disc_abs'], func=None, align="left"),
    MathCol("num_ram", "nf.ramified_primes", "Ram. prime count",
            short_title="ramified prime count", default=False),
    ...
    SearchCol(
        ...
        "nf.galois_group",
        "Galois group",
        download_col="galois_label"),
    ClassGroupCol(
        "class_group_desc",
        "nf.ideal_class_group",
        "Class group",
        download_col="class_group"),
    MathCol(
        "torsion_order",
        "nf.unit_group",
        "Unit group torsion",
...
def nf_postprocess(res, info, query):
    galois_labels = [rec["galois_label"]
                     for rec in res if rec.get("galois_label")]
    cache = knowl_cache(list(set(galois_labels)))
    for rec in res:
        wnf = WebNumberField.from_data(rec)
        rec["poly"] = '$' + \
            compress_polynomial(wnf.poly().change_ring(ZZ), 30) + '$'
...
def number_field_search(info, query):
    ...
    parse_bracketed_posints(
        info, query, 'signature', qfield=(
            'degree', 'r2'), exactlength=2, allow0=True, extractor=lambda L: (
            L[0] + 2 * L[1], L[1]))
    parse_signed_ints(
        info,
        query,
    ...
    if fi == "cyc":
        ...
        if "degree" in info:
            opts = {
                n: opts[n] for n in integer_options(
                    info["degree"],
                    contained_in=list(opts),
                    lower_bound=1,
                    upper_bound=47) if n in opts}
        # Catch if signature is specified
        # We already parsed it, which sets degree to a single value
        if "signature" in info:
            opts = {n: opts[n] for n in integer_options(str(query["degree"]), contained_in=list(
                opts), lower_bound=1, upper_bound=47) if n in opts}
        ...
        if len(opts) == 0:
            # Incompatible with specified degree or galois labels, so we add an
...
class NFSearchArray(SearchArray):
    ...
    sorts = [("", "degree", ['degree', 'disc_abs', 'disc_sign', 'iso_number']),
             ("signature", "signature", [
              'degree', 'r2', 'disc_abs', 'disc_sign', 'iso_number']),
             ("rd", "root discriminant", [
              'rd', 'degree', 'disc_abs', 'disc_sign', 'iso_number']),
             ("grd", "Galois root discriminant", [
              'grd', 'degree', 'disc_abs', 'disc_sign', 'iso_number']),
             ("disc", "absolute discriminant", [
              'disc_abs', 'disc_sign', 'degree', 'iso_number']),
             ("num_ram", "ramified prime count", [
              'num_ram', 'disc_abs', 'disc_sign', 'degree', 'iso_number']),
             ("h",
              "class number",
              ['class_number',
               'degree',
