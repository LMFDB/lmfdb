...
def process_all():
    ...
    # This would prevent sharing some computations, like the poset of modular
    # curves, between different uploads
    with tempfile.NamedTemporaryFile("w", delete=False) as F:
        ...
        for rec in db.data_uploads.search(
                ...
                for table, newrow, line in section.process(rec["data"]):
                    if (table, newrow) in cols:
                        if cols[table, newrow] != set(line):
                            raise ValueError(
                                f"Schema for id {rec['id']}:{table}:{newrow} does not agree with previous schema")
                    else:
                    ...
                    line = "|".join(
                        copy_dumps(
                            line[col],
                            col_type[col]) for col in sorted(line))
            ...
            status_update[rec["section"]][rec["id"]] = (
                status, timestamp, comment)
