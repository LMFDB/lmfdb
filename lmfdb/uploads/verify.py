...
def verify_all():
    raise NotImplementedError(
        "This script has been disabled until the verify methods in modular curves are complete")
    reviewer = Reviewer()
    # TODO: it would be better to isolate each verification in its own process
    # to insulate against timeouts
    with tempfile.NamedTemporaryFile("w", delete=False) as F:
        ...
        for rec in db.data_uploads.search(
            ...
            _ = F.write(
                f"{rec['id']}|{status}|{timestamp}|{timestamp}|{comment}\n")
        F.close()
