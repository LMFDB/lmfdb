...
def codomain(algebraic_group, dimension, base_ring_order, base_ring_is_field):
    return "$" + _codomain(algebraic_group, dimension,
                           base_ring_order, base_ring_is_field) + "$"


def image_pretty(image_label, is_surjective, algebraic_group,
                 dimension, base_ring_order, base_ring_is_field, codomain=False):
    s = _codomain(
        algebraic_group,
        dimension,
        base_ring_order,
    ...
    if is_surjective:
        return "$" + s + "$"
    t = display_knowl('gl2.subgroup_data', title=image_label, kwargs={
                      'label': image_label}) if dimension == 2 else image_label
    return t + r" $< " + s + "$" if codomain else t


def image_pretty_with_abstract(image_label, is_surjective, algebraic_group, dimension,
                               base_ring_order, base_ring_is_field, image_abstract_group, codomain=False):
    s = _codomain(
        algebraic_group,
        dimension,
        base_ring_order,
    ...
    if image_abstract_group:
        t += r" $\ \cong$ " + \
            abstract_group_display_knowl(image_abstract_group)
    return t


def rep_pretty(algebraic_group, dimension,
               base_ring_order, base_ring_is_field):
    return r"$\rho\colon\Gal_\Q\to" + \
        _codomain(
            algebraic_group,
            dimension,
            base_ring_order,
...
def get_bread(tail=[]):
    base = [(r"Mod-$\ell$ Galois representations", url_for(".index")),
            (r"$\Q$", url_for(".index_Q"))]
...
class WebModLGalRep(WebObj):
    ...
    def friends(self):
        ...
        if kerfield and kerfield._data:
            friends.append(
                ("Number field " +
                 kerfield.field_pretty(),
                 url_for(
                     "number_fields.by_label",
                     label=kerfield.label)))
        projkerfield = WebNumberField.from_coeffs(
            self.projective_kernel_polynomial)
        if projkerfield and projkerfield._data and projkerfield.label != kerfield.label:
            friends.append(
                ("Number field " +
                 projkerfield.field_pretty(),
                 url_for(
                     "number_fields.by_label",
                     label=projkerfield.label)))

        if self.dimension > 1 and hasattr(self, "determinant_label"):
            friends.append(
                ("Determinant " +
                 self.determinant_label,
                 url_for_modlgal_label(
                     label=self.determinant_label)))
            ...
            friends.append(
                ("Dirichlet character " +
                 c[0] +
                    ...
                    c[1],
                    url_for(
                     "character.render_Dirichletwebpage",
                     modulus=c[0],
                     number=c[1])))
        return friends
    ...
    @lazy_attribute
    def codomain(self):
        return codomain(self.algebraic_group, self.dimension,
                        self.base_ring_order, self.base_ring_is_field)

    @lazy_attribute
    def image_pretty_with_abstract(self):
        return image_pretty_with_abstract(self.image_label, self.image_index == 1, self.algebraic_group, self.dimension,
                                          self.base_ring_order, self.base_ring_is_field, self.image_abstract_group, codomain=False)

    @lazy_attribute
    def image_pretty(self):
        return image_pretty(self.image_label, self.image_index == 1, self.algebraic_group,
                            self.dimension, self.base_ring_order, self.base_ring_is_field, codomain=False)

    @lazy_attribute
    def rep_pretty(self):
        return rep_pretty(self.algebraic_group, self.dimension,
                          self.base_ring_order, self.base_ring_is_field)
    ...
    def projective_image_abstract(self):
        if self.image_abstract_group:
            return abstract_group_display_knowl(
                self.projective_image_abstract_group)
        return ''
    ...
    def frobenius_generators(self):
        if not self.generating_primes:
            return None
        return ",".join(r"\mathrm{Frob}_{%s}" % (p)
                        for p in self.generating_primes)
    ...
    def frobenius_matrices_pretty(self):
        L = []
        F = GF(
            self.base_ring_order) if self.base_ring_is_field else Integers(
            self.base_ring_order)
        R = GL(
            self.dimension,
            F) if self.algebraic_group == "GL" else Sp(
            self.dimension,
            F)
        n = self.dimension
        ps = [
            p for p in prime_range(100) if (
                self.conductor *
                self.base_ring_characteristic) %
            p != 0]
        ...
        try:
            for i in range(len(ps)):
                ...
                M = R(frobs[i])
                p = r"\mathbf{%s}" % (
                    ps[i]) if self.generating_primes and ps[i] in self.generating_primes else ps[i]
                charpoly = m.charpoly()
                pol = web_latex(charpoly) if charpoly.is_irreducible(
                ) else web_latex(factor(charpoly))
                L.append([p, m.trace(), m.det(), M.order(), pol, web_latex(m)])
        except ValueError:
            print(
                f"Error occurred while attempting to parse frobenius_matrices for {self.label}")
