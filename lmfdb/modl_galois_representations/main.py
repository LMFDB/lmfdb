...
def modlgal_link(label):
    if int(label.split(".")[0]) <= 70:
        return '<a href=%s>%s</a>' % (url_for("modlgal.by_label",
                                              label=label), label)
    else:
        return label
...
def by_label(label):
    ...
    if rep.is_null():
        flash_error(
            r"There is no mod-$\ell$ Galois representation %s in the database",
            label)
        return redirect(url_for(".index"))
    return render_template(
        "modlgal_rep.html",
...
modlgal_columns = SearchColumns(
    [
        ...
        RationalCol(
            ...
            lambda x: x,
            align="center",
            default=lambda info: info.get("top_slope")),
        ...
        MultiProcessedCol("algebraic_group",
                          "modlgal.codomain",
                          "Codomain",
                          ["algebraic_group",
                           "dimension",
        ...
        SearchCol(
            ...
            "modlgal.image_index",
            "Index",
            short_title="image index",
            default=False),
        SearchCol(
            "image_order",
            "modlgal.image_order",
            "Order",
            short_title="image order",
            default=False),
        ...
        CheckCol(
            ...
            "Abs irred",
            short_title="absolutely irreducible",
            default=False),
        ...
        ProcessedCol(
            "determinant_index",
            "modlgal.det_surjective",
            "Det. surjective",
            lambda a: "&#x2713;" if a == 1 else "",
            align="center",
            default=False),
        ProcessedCol(
            "generating_primes",
            "modlgal.generating_primes",
            "Generators",
            lambda ps: "$" +
            ",".join(
                str(p) for p in ps) +
        ...
        ProcessedCol(
            ...
            formatfield),
        ProcessedCol(
            "projective_kernel_polynomial",
            "modlgal.projective_kernel_polynomial",
            "Projective splitting field",
            formatfield,
            default=False),
    ],
...
def modlgal_search(info, query):
    ...
    parse_primes(
        ...
        mode=info.get('conductor_primes_quantifier'))
    parse_rats(
        info,
        query,
        'top_slope',
        qfield='top_slope_real',
        name='Top slope')
    if 'top_slope_real' in query and '/' in info['top_slope']:
        ...
        if isinstance(a, dict):
            e = 0.000001
            query['top_slope_real'] = {k: float(QQ(a[k])) -
                                       e if '/' in a[k] and k[:2] == '$g' else (float(QQ(a[k])) +
                                                                                e if '/' in a[k] and k[:2] == '$l' else a[k]) for k in a}
        else:
            query['top_slope_rational'] = query.pop('top_slope_real')
    if info.get('codomain'):
        query['algebraic_group'], query['dimension'], query['base_ring_order'], query['base_ring_is_field'] = info.get(
            'codomain').split(',')
...
class ModLGalRep_stats(StatsDisplay):
    ...
    def short_summary(self):
        modlgal_knowl = display_knowl(
            "modlgal", title=r"mod-$\ell$ Galois representations")
        return (
    ...
    query_formatters = {
        'image_abstract_group': lambda x: "image_abstract_group=%s" % x}
    stat_list = [
...
def statistics():
    ...
    return render_template("display_stats.html", info=ModLGalRep_stats(
...
def download_modlgal_text(label):
    ...
    if data is None:
        return r"There is no mod-$\ell$ Galois representation %s in the database" % (
            label)
    data_list = [data]
...
def modlgal_data(label):
    ...
    if LABEL_RE.fullmatch(label):
        return datapage([label], ["modlgal_reps"],
                        title=fr"Mod-$\ell$ Galois representation data - {label}", bread=bread)
    else:
        return abort(404)
