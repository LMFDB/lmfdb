...
def H6(k, p):
    S = 0
    if ((p % 4) == 1):
        S = 5 * (2 * k - 3) * (p + 1) / (2 ** 7 * 3) + (-1) ** k * (p + 1) / (2 ** 7)
    if ((p % 4) == 3):
        S = (2 * k - 3) * (p - 1) / (2 ** 7) + 5 * (-1) ** k * (p - 1) / (2 ** 7 * 3)
    if p == 2:
        S = 3 * (2 * k - 3) / (2 ** 7) + 7 * (-1) ** k / (2 ** 7 * 3)
    return S
...
def H7(k, p):
    ...
    S1 = tink([0, -1, 1], k)
    if ((p % 3) == 1):
        S = (2 * k - 3) * (p + 1) / (2 * 3 ** 3) + S1 * (p + 1) / (2 ** 2 * 3 ** 3)
    if ((p % 3) == 2):
        S = (2 * k - 3) * (p - 1) / (2 ** 2 * 3 ** 3) + S1 * (p - 1) / (2 * 3 ** 3)
    if p == 3:
        S = 5 * (2 * k - 3) / (2 ** 2
                                                                   * 3 ** 3) + S1 / (3 ** 3)
    return S
