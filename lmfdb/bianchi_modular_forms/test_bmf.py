...
class BMFTest(LmfdbTest):
    ...
    def test_search(self):
        ...
        self.check_args(
            ...
            'ModularForm/GL2/ImaginaryQuadratic/2.0.1007.1/9.1/a/')
    ...
    def test_download_sage(self):
        ...
        L1_variables = self.check_sage_compiles_and_extract_variables(
            L1_sage_code)
        assert L1_variables['NN'].norm() == Integer(18333)
        a = L1_variables['a']
        ZF = L1_variables['ZF']
        assert L1_variables['NN'] == ZF.ideal((6111, 3 * a + 5052))
        for gens in [(27 * a - 22,), (-29 * a + 15,), (-29 * a + 14,),
                     (29 * a - 11,), (-29 * a + 18,), (-29 * a + 9,)]:
            assert ZF.ideal(gens) in L1_variables['hecke_eigenvalues']
        hecke_av_start = [0, -1, 2, -1, 1, -3, 4,
                          0, -2, -8, 7, -9, -8, -4, -9, 8, 10, -11]
        assert L1_variables['hecke_eigenvalues_array'][:len(
            hecke_av_start)] == hecke_av_start
        ...
        # assert L2_variables['NN'] == ZF.ideal((16*i - 11)), "level doesn't
        # match"  # the level displayed on BMF homepage
    ...
    def test_underlying_data(self):
        ...
        page = self.tc.get(
            "/ModularForm/GL2/ImaginaryQuadratic/2.0.7.1/88.5/a",
