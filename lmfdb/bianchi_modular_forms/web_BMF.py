...
# EllipticCurveSearch() returning "no elliptic curves have the given
# traces")
    ...
    def make_form(self, nap0=50):
        ...
        try:
            dims = db.bmf_dims.lucky(
                {'field_label': self.field_label, 'level_label': self.level_label}, projection='gl2_dims')
        ...
        if self.Lratio == '?':
            ...
            self.anrank = r"\(0\)" if self.Lratio != 0 else "odd" if self.sfe == - \
                1 else r"\(\ge2\), even"
        ...
        try:
            ...
            self.ec_status = None
        if self.ec_status not in [
                0, -1]:  # so it is +1 (curve does exist) or None (curve_status not set)
            self.ec_url = url_for(
                "ecnf.show_ecnf_isoclass",
                nf=self.field_label,
                conductor_label=self.level_label,
                class_label=self.label_suffix)
            curve_bc = db.ec_nfcurves.lucky(
                {'class_label': self.label}, projection="base_change")
            self.bc_forms = []
