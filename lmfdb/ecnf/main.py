...
def index():
    ...
    info['fields'] = []
    # Rationals
    # info['fields'].append(['the rational field', (('1.1.1.1',
    # [url_for('ec.rational_elliptic_curves'), '$\Q$']),)]) # Removed due to
    # ambiguity

    # Real quadratics (sample)
    rqfs = ['2.2.{}.1'.format(d)
            for d in [8, 12, 5, 24, 28, 40, 44, 13, 56, 60]]
    ...
    # , 56, 15, 68, 19, 84, 88, 23, 43, 67, 163]]
    iqfs = ['2.0.{}.1'.format(d) for d in [4, 8, 3, 20, 24, 7, 40, 11, 52]]
...
@redirect_no_cache
def random_curve():
    E = db.ec_nfcurves.random(
        projection=[
            'field_label',
            'conductor_label',
            'iso_label',
            'number'])
    return url_for(".show_ecnf", nf=E['field_label'], conductor_label=E['conductor_label'],
                   class_label=E['iso_label'], number=E['number'])
...
def statistics():
    ...
    return render_template("display_stats.html", info=ECNF_stats(
...
def show_ecnf1(nf):
    if LABEL_RE.fullmatch(nf):
        ...
        return redirect(url_for(".show_ecnf", nf=nf, conductor_label=cond_label,
                                class_label=iso_label, number=number), 301)
    if CLASS_LABEL_RE.fullmatch(nf):
        nf, cond_label, iso_label = split_class_label(nf)
        return redirect(url_for(".show_ecnf_isoclass", nf=nf,
                                conductor_label=cond_label, class_label=iso_label), 301)
    ...
    if nf_label == '1.1.1.1':
        return redirect(
            url_for("ec.rational_elliptic_curves", **request.args), 301)
...
def show_ecnf_conductor(nf, conductor_label):
    ...
    info['bread'] = [
        ('Elliptic curves', url_for(".index")), (nf_pretty, url_for(
            ".show_ecnf1", nf=nf)), (conductor_label, url_for(
                ".show_ecnf_conductor", nf=nf, conductor_label=conductor_label))]
    if len(request.args) > 0:
        # if requested field or conductor norm differs from nf or
        # conductor_lable, redirect to general search
...
def show_ecnf_isoclass(nf, conductor_label, class_label):
    ...
    if not CLASS_LABEL_RE.fullmatch(label):
        flash_error(
            "%s is not a valid elliptic curve isogeny class label",
            label)
        return redirect(url_for(".index"))
...
def show_ecnf(nf, conductor_label, class_label, number):
    ...
    try:
        ...
        return redirect(url_for(".index"))
    label = "".join(
        ["-".join([nf_label, conductor_label, class_label]), str(number)])
    ...
    if not isinstance(ec, ECNF):
        flash_error(
            "There is no elliptic curve with label %s in the database",
            label)
        return redirect(url_for(".index"))
    bread = [("Elliptic curves", url_for(".index"))]
    ...
    code = ec.code()
    code['show'] = {'magma': '', 'pari': '',
                    'sage': ''}  # use default show names
    info = {}
...
def elliptic_curve_jump(info):
    ...
    if LABEL_RE.fullmatch(label):
        ...
        flash_error(
            "%s is not a valid elliptic curve or isogeny class label.",
            label)
...
def url_for_label(label):
    ...
    nf, cond_label, iso_label, number = split_full_label(label.strip())
    return url_for("ecnf.show_ecnf", nf=nf, conductor_label=cond_label,
                   class_label=iso_label, number=number)


def make_cm_query(cm_disc_str):
    cm_list = parse_ints_to_list_flash(
        cm_disc_str, "CM discriminant", max_val=None)
    for d in cm_list:
        if not ((d < 0) and (d % 4 in [0, 1])):
            raise ValueError(
                "CM discriminants are negative and congruent to 0 or 1 mod 4.")
    return cm_list
...
ecnf_columns = SearchColumns([
    ...
    MathCol(
        ...
        "ec.isogeny",
        "Class size",
        short_title="isogeny class size",
        default=False),
    MathCol(
        "class_deg",
        "ec.isogeny",
        "Class degree",
        short_title="isogeny class degree",
        default=False),
    ProcessedCol(
        "field_label",
        "nf",
        "Base field",
        lambda field: nf_display_knowl(
            field,
            field_pretty(field)),
        align="center"),
    MathCol(
    ...
    MathCol(
        ...
        "nf.signature",
        "Field signature",
        short_title="base field signature",
        align="center",
        default=False),
    SearchCol(
        "conductor_label",
    ...
    ProcessedCol("bad_primes", "ec.bad_reduction", "Bad primes",
                 lambda primes: ", ".join(''.join(str(p.replace('w', 'a')).split(
                     '*')) for p in primes) if primes else r"\textsf{none}",
                 default=lambda info: info.get("bad_primes"), mathmode=True, align="center"),
    MultiProcessedCol("rank", "ec.rank", "Rank", ["rank", "rank_bounds"],
                      lambda rank, rank_bounds: rank if rank is not None else (r"%s \le r \le %s" % (
                          rank_bounds[0], rank_bounds[1]) if rank_bounds is not None else ""),
    ...
    ProcessedCol("sato_tate_group", "st_group.definition", "Sato-Tate",
                 lambda v: st_display_knowl(
                     '1.2.A.1.1a' if v == 0 else (
                         '1.2.B.2.1a' if v < 0 else '1.2.B.1.1a')),
...
modell_image_label_regex = re.compile(
    r'(\d+)(G|B|Cs|Cn|Ns|Nn|A4|S4|A5)(\.\d+)*(\[\d+\])?')
...
def elliptic_curve_search(info, query):
    ...
    parse_element_of(info, query, 'isodeg', split_interval=1000,
    ...
    parse_nf_jinv(info, query, 'jinv', 'j-invariant',
    ...
    # Keep include_base_change/include_Q_curves options for backward compat
    # with URLs
    if 'include_base_change' in info:
        if info['include_base_change'] in [
                'exclude', 'off']:  # off for backward compat
            query['base_change'] = []
...
def browse():
    ...
    bread = [('Elliptic curves', url_for("ecnf.index")),
             ('Browse', ' ')]
    return render_template("ecnf-stats.html", info=info,
...
def statistics_by_degree(d):
    ...
    def sig_counts(sig):
        return ['%s,%s' % sig, counts_by_sig[sig], [
    ...
    bread = [('Elliptic curves', url_for("ecnf.index")),
             ('Degree %s' % d, ' ')]
    return render_template("ecnf-by-degree.html", info=info,
                           title=t, bread=bread, learnmore=learnmore_list())
...
def statistics_by_signature(d, r):
    ...
    return render_template("ecnf-by-signature.html", info=info,
                           title=t, bread=bread, learnmore=learnmore_list())
...
def download_ECNF_all(nf, conductor_label, class_label, number):
    ...
    if data is None:
        flash_error(
            "%s is not the label of an elliptic curve in the database.",
            label)
        return redirect(url_for(".index"))
...
def ecnf_code(**args):
    label = "".join(["-".join([args['nf'],
                               args['conductor_label'],
                               args['class_label']]),
                     args['number']])
    if not LABEL_RE.fullmatch(label):
        return abort(404)
