...
class ECNF(object):
    ...
    def make_E(self):
        ...
        self.downloads = [('Download all stored data to text', url_for(".download_ECNF_all", nf=self.field_label, conductor_label=quote(self.conductor_label), class_label=self.iso_label, number=self.number))]
        for lang in [["Magma","magma"], ["SageMath","sage"], ["GP", "gp"]]:
            self.downloads.append(('Download code to {}'.format(lang[0]),
