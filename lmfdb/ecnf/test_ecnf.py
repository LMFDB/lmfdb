...
class EllCurveTest(LmfdbTest):
    ...
    def test_minimal_eqn(self):
        ...
        assert 'This is a <a title="Global minimal model' in L.get_data(
            as_text=True)
        L = self.tc.get('/EllipticCurve/2.2.229.1/9.3/a/2')
        assert 'This is not a <a title="Global minimal model' in L.get_data(
            as_text=True)
    ...
    def test_download(self):
        ...
        assert 'Magma code for working with elliptic curve 2.2.89.1-81.1-a1' in L.get_data(
            as_text=True)
        L = self.tc.get('EllipticCurve/2.2.89.1/81.1/a/1/download/sage')
        assert 'SageMath code for working with elliptic curve 2.2.89.1-81.1-a1' in L.get_data(
            as_text=True)
        L = self.tc.get('EllipticCurve/2.2.89.1/81.1/a/1/download/gp')
        assert 'Pari/GP code for working with elliptic curve 2.2.89.1-81.1-a1' in L.get_data(
            as_text=True)
    ...
    def test_search(self):
        ...
        # Conductor 1
        L = self.tc.get(
            '/EllipticCurve/?start=0&count=50&conductor_norm=1&include_isogenous=on&include_base_change=on')
        assert '2115 a - 13286543' in L.get_data(as_text=True)
        # 4*4 torsion
        L = self.tc.get(
            '/EllipticCurve/?start=0&count=50&include_isogenous=on&include_base_change=on&torsion=&torsion_structure=[4%2C4]')
        ...
        L = self.tc.get(
            '/EllipticCurve/?field=Qsqrt-11&include_base_change=on&conductor_norm=&include_isogenous=on&torsion=&torsion_structure=&count=')
        ...
        L = self.tc.get(
            '/EllipticCurve/?field=2.0.11.1&jinv=~-52893159101157376/11')
    ...
    def test_cm_disc_search(self):
        ...
        self.check_args(
            '/EllipticCurve/?cm_disc=-11&include_cm=PCMnoCM',
            '14641.1-a1')
        self.not_check_args(
            '/EllipticCurve/?cm_disc=-11&include_cm=PCMnoCM',
            '9.1-CMa1')
