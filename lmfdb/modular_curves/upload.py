...
class Points(UploadSection):
    ...
    inputs = [UReferenceBox("reference", "Reference", "upload.reference"),
        ...
        "curve",
        "Modular curve",
        "upload.modcurve.name_or_label",
        name_or_label_for="gps_gl2zhat_fine",
        label_linker=modcurve_link),
        UTextBox(
        "residue_field",
        "Residue field",
        "upload.modcurve.residue_field",
        label_for="nf_fields",
        label_linker=lambda label: nf_display_knowl(
            label,
            label)),
        UTextBox("jorig",
                 "$j$-invariant",
                 "upload.modcurve.jinv",
                 remove_spaces=True,
                 re=(jre,
                     "comma separated rationals"),
                 mathmode=True),
        UTextBox(
        "coordinates",
        "Coordinates",
        "upload.modcurve.coordinates",
        remove_spaces=True,
        re=(
            coordre,
            "semicolon separated points, each point giving (weighted) projective coordinates separated by colons, with each coordinate specified as a comma separated list of rational numbers"),
        mathmode=True),
    ...
    def validate(self, rec):
        ...
        if g == 0:
            raise ValueError(
                "Adding points on genus zero curves not supported")
        if rec["jorig"] != "oo" and rec["jorig"].count(",") != n - 1:
            raise ValueError(
                f"j-coordinate {rec['jorig']} has degree {rec['jorig'].count(',')+1} but residue field has degree {n}")

        # Check that each coordinate satisfies the equations for the model, and
        # combine model_type and coordinates into the right format
        model_type = rec["model_type"]
        edata = db.modcurve_models.search({"modcurve": rec["curve"], "model_type": model_type}, [
                                          "number_variables", "equation"])
        if edata is None:
            box = [x for x in self.inputs if x.name == "model_type"][0]
            model_name = [
                name for opt,
                name in box.options if opt == str(model_type)][0]
            raise ValueError(
                f"There is no {model_name} model for {rec['curve']} stored in the database")
        d = edata["number_variables"]
        equation = edata["equation"]
        if model_type in [2, 5]:
            if d != 3:
                raise ValueError(
                    f"Underlying data error: num vars {d} but it should be 3")
            vars = "xyz"
        elif model_type == 7:
            if d != 4:
                raise ValueError(
                    f"Underlying data error: num vars {d} but it should be 4")
            vars = "xyzw"
        else:
        ...
        try:
            ...
            raise ValueError(
                "Underlying data error: could not build polynomials from\n{equation}")
        if n != 1:
            R = ZZ['x']
        ...
        for pt in encoded:
            ...
            if len(coords) != d:
                raise ValueError(
                    f"Coordinates do not have the right size: there are {len(coords)} but should be {d}")
            for x in coords:
                if x.count(",") != n - 1:
                    raise ValueError(
                        f"Coordinate does not lie in field of correct degree: {x} does not have degree {n}")
            if n == 1:
            ...
            for g in equation:
                if g.subs(dict(zip(vars, coords))) != 0:
                    raise ValueError(
                        f"{pt} does not satisfy equation {str(g).replace(' ','')}=0")
        # TODO: Check that we don't already have this point
        return rec

    def verify(self, rec):
        # TODO: Call out to magma to create the model, the given point, the
        # other preimages of the same j-invariants, etc
        pass
    ...
    def process(self, rec):
        ...
        data = {
            ...
            # TODO: Is this right?  What if we're on a non-smooth model?
            "cardinality": len(coords),
            "cusp": (jorig == "oo"),
        ...
        # BIG TODO: Propagate this point to other modular curves (both up and
        # down, including fine models)
...
class PointCompleteness(UploadSection):
    ...
    def process(self, rec):
        return [("gps_gl2zhat_fine", False, {
                 "label": rec["curves"], "all_degree1_points_known": True})]
...
class GonalityBounds(UploadSection):
    ...
    inputs = [UReferenceBox("reference", "Reference", "upload.reference"),
        ...
        "q_gonality",
        r"$\mathbb{Q}$-gonality",
        "upload.modcurve.q_gonality",
        remove_spaces=True,
        natural_or_range=True),
        UTextBox(
        "qbar_gonality",
        r"$\bar{\mathbb{Q}}$-gonality",
        "upload.modcurve.qbar_gonality",
        remove_spaces=True,
        natural_or_range=True),
    ...
    def __init__(self, *args, **kwds):
        ...
        # This list will get populated when process() is called, and used when
        # final_process() is called
        self.delayed = []
    ...
    def validate(self, rec):
        ...
        cur = db.gps_gl2zhat_fine.lookup(
            rec["curve"], [
                "q_gonality_bounds", "qbar_gonality_bounds"])
        if not (cur["q_gonality_bounds"][0] <= q_low <=
                q_high <= cur["q_gonality_bounds"][1]):
            raise ValueError(
                f"Q-gonality bounds for {rec['curve']} inconsistent with current bounds {cur['q_gonality_bounds']}")
        if not (cur["qbar_gonality_bounds"][0] <= qbar_low <=
                qbar_high <= cur["qbar_gonality_bounds"][1]):
            raise ValueError(
                f"Qbar-gonality bounds for {rec['curve']} inconsistent with current bounds {cur['qbar_gonality_bounds']}")

        # TODO: If gonal map specified, we should ensure that it has the right
        # format for a map from the specified model to P1
        return rec

    def verify(self, rec):
        # TODO: If gonal map specified, we should confirm that its degree matches the provided upper gonality bound
        # We don't do anything further here, since propagation is expensive and
        # we want to wait until the processing stage
        pass
    ...
    def final_process(self, ids, F, by_table, cols):
        # ids as input needs to be overwritten; we will reconstruct it below as
        # we do actual work.
        lines = []
        ...
        ids = {upid: (status, timestamp, comment)
               for upid, data in self.delayed}

        # If other columns are added later, it's important that these be sorted
        # (see lmfdb/uploads/process.py)
        columns = sorted(['label',
                          'q_gonality',
                          'q_gonality_bounds',
                          'qbar_gonality',
                          'qbar_gonality_bounds'])
        by_table["gps_gl2zhat_fine", False] = [
            "|".join(line[col] for col in columns) for line in lines]
...
class Models(UploadSection):
    ...
    inputs = [UReferenceBox("reference", "Reference", "upload.reference"),
        ...
        "equation",
        "Equation(s)",
        "upload.modcurve.model_equation"),
        UTextBox(
        "curve",
        "Modular curve",
        "upload.modcurve.name_or_label",
        ...
        "map_coordinates",
        "Map Definition",
        "upload.modcurve.map_coordinates",
        mathmode=True),
    ]

    def validate(self, rec):
        rec = super().validate(rec)
        # We only do some simple checks that the other modular curve is
        # plausible
        if rec["curve"] != rec["other_curve"] and rec["maps"] == "from":
            raise ValueError(
                "Defining map from another curve only allowed if birational")
        if rec["model_type"] == 1:
            raise ValueError("P1 models are not stored in the database")
        if db.modcurve_models.exists(
                {"modcurve": rec["curve"], "model_type": rec["model_type"]}):
            raise ValueError(
                f"Model of type {rec['model_type']} already exists for {rec['curve']}")
        ...
        if N % oN != 0:
            raise ValueError(
                "Level of other curve not a divisor; map not possible")
        if i % oi != 0:
            raise ValueError(
                "Index of other curve not a divisor; map not possible")
        if rec["other_model_type"] == 1:
            if og != 0:
                raise ValueError(
                    f"Other curve {rec['other_curve']} does not have genus 0")
            if db.gps_gl2zhat_fine.lookup(
                    rec["other_curve"], "pointless") is not False:
            ...
            raise ValueError(
                f"Other model of type {rec['other_model_type']} for {rec['other_curve']} must exist")
        eq = rec["equation"]
        if eq != eq.lower():
            raise ValueError(
                f"All variables must be lower case (using variable order {VARORDER}")
        break_found = False
        for i, ch in enumerate(VARORDER):
            if break_found and ch in eq:
                raise ValueError(
                    f"Not in variable order {VARORDER}: {ch} out of order")
    ...
    def verify(self, rec):
        ...
        # TODO: Use point counts to check plausibility of model for the given
        # GL2Zhat-subgroup
        pass
    ...
    def process(self, rec):
        modeldata = {
            ...
            "model_type": rec["model_type"],
            # TODO: determine if smooth (only needed for plane model)
            "smooth": None if rec["model_type"] == 2 else True,
            "dont_display": False,
        ...
        return [("modcurve_models", True, modeldata),
                ("modcurve_modelamps", True, mapdata)]
...
class UniversalEC(UploadSection):
    ...
    inputs = [UReferenceBox("reference", "Reference", "upload.reference"),
        ...
        UTextBox(
        "universal_ec",
        "Universal Elliptic Curve Model",
        "upload.modcurve.universal_ec"),
    ]
    ...
    def validate(self, rec):
        ...
        if rec["universal_ec"].count(",") not in [1, 4]:
            raise ValueError(
                "Equation must be given as a list of a-invariants of length either 2 or 5")
        coarse_label, g = to_coarse_label(rec["curve"])
        if g == 0:
            pointless = db.gps_gl2zhat_fine.lookup(coarse_label, "pointless")
            # TODO: should we be using t as a coordinate here or xy?
            vars = "xyz" if pointless else "xy"
        else:
            numvars = db.modcurve_models.lucky(
                {"modcurve": rec["curve"], "model_type": rec["model_type"]}, "number_variables")
        ...
        for f in rec["universal_ec"].split(","):
            try:
                ...
                raise ValueError(
                    f"Invalid format (must be in Q({','.join(vars)})): {f}")
        return rec

    def verify(self, rec):
        # TODO: We should do some random tests to ensure that specializations have the right Galois image
        # If we ever get spot on, done.  If not, compute minimum upper bound in
        # lattice....
        pass

    def process(self, rec):
        return [("gps_gl2zhat_fine", False, {"label": rec["curve"], "universal_ec": rec["universal_ec"].split(
            ","), "universal_ec_model_type": rec["model_type"]})]
...
class MultiKnowl(UploadSection):
    ...
    def verify(self, rec):
        # We don't do anything; the verification is completely on the editor
        # checking that the knowl makes sense to associate to this set of
        # modular curves
        pass

    def process(self, rec):
        # We don't do anything; the modular curve pages use the data_uploads
        # table to find the appropriate knowls to include.
        return []
