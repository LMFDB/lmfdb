...
def by_label(label):
    if RSZB_LABEL_RE.fullmatch(label):
        label = db.gps_gl2zhat_fine.lucky(
            {"RSZBlabel": label}, projection="label")
    ...
    learnmore_mcurve_pic = (
        'Picture description',
        url_for(".mcurve_picture_page"))
    return render_template(
        "modcurve.html",
        curve=curve,
        dojs=dojs,
...
def diagram_js_string(curve):
    display_opts = {}
    graph, rank_lookup, num_layers = diagram_js(
        curve, curve.nearby_lattice, display_opts)
...
def url_for_CP_label(label):
    ...
    return "https://mathstats.uncg.edu/sites/pauli/congruence/csg" + \
        genus + ".html#group" + label
...
def modcurve_lmfdb_label(label):
    if LABEL_RE.fullmatch(label) or ISO_CLASS_RE.fullmatch(label):
        ...
        label_type = "name"
        lmfdb_label = db.gps_gl2zhat_fine.lucky(
            {"name": canonicalize_name(label)}, "label")
    else:
        label_type = "label"
        lmfdb_label = None
...
def modcurve_jump(info):
    ...
    for label in labels:
        ...
        if lmfdb_label is None:
            flash_error(
                "There is no modular curve in the database with %s %s",
                label_type,
                label)
            return redirect(url_for(".index"))
    ...
    if len(lmfdb_labels) == 1:
        ...
        if len(factors) != len(lmfdb_labels_not_X1):
            flash_error(
                "Fiber product decompositions cannot contain repeated terms")
            return redirect(url_for(".index"))
        ...
        if label is None:
            flash_error(
                "There is no modular curve in the database isomorphic to the fiber product %s",
                info["jump"])
            return redirect(url_for(".index"))
        else:
            return redirect(url_for_modcurve_label(label))
...
modcurve_columns = SearchColumns(
    [
        ...
        ProcessedCol(
            ...
            "modcurve.standard",
            "Name",
            lambda s: name_to_latex(s) if s else "",
            align="center"),
        ...
        ProcessedCol(
            ...
            "modcurve.rank",
            "Rank",
            lambda r: "" if r is None else r,
            default=lambda info: info.get(
                "rank") or info.get("genus_minus_rank"),
            align="center",
            mathmode=True),
        ProcessedCol(
            "q_gonality_bounds", "modcurve.gonality", r"$\Q$-gonality", lambda b: r'$%s$' %
            (b[0]) if b[0] == b[1] else r'$%s \le \gamma \le %s$' %
            (b[0], b[1]), align="center", short_title="Q-gonality"),
        ...
        CheckCol(
            ...
            "Contains -1",
            short_title="contains -1",
            default=False),
        MultiProcessedCol("dims",
        ...
        ProcessedCol(
            ...
            r"$\operatorname{GL}_2(\mathbb{Z}/N\mathbb{Z})$-generators",
            lambda gens: ", ".join(
                r"$\begin{bmatrix}%s&%s\\%s&%s\end{bmatrix}$" %
                tuple(g) for g in gens) if gens else "trivial subgroup",
            short_title="generators",
            default=False),
    ],
...
def parse_family(inp, query, qfield):
    if inp not in ["X0", "X1", "Xpm1", "X", "Xsp", "Xspplus", "Xns",
                   "Xnsplus", "XS4", "Xarith1", "Xarithpm1", "Xsym", "Xarith", "any"]:
        raise ValueError
    ...
    if inp == "any":
        ...
        query[qfield] = {
            "$or": [{"$like": inp + "(%"}, {"$in": ["X(1)", "X(2)"]}]}
    elif inp == "Xarith1":  # X_{arith,1}(m,mn); add X(2)
        query[qfield] = {
            "$or": [{"$like": inp + "(%,%"}, {"$like": "X1(2,%"}, {"$in": ["X(2)"]}]}
    elif inp == "Xarithpm1":  # X_{\pm1}(2,2n); add X(2)
        query[qfield] = {
            "$or": [{"$like": inp + "(%,%"}, {"$like": "Xpm1(2,%"}, {"$in": ["X(2)"]}]}
    elif inp == "Xpm1":  # Add X(1) and X0(N) for N=2,3,4,6
        query[qfield] = {"$or": [{"$like": "Xpm1(%", "$not": {"$like": "%,%"}}, {
            "$in": ["X(1)", "X0(2)", "X0(3)", "X0(4)", "X0(6)"]}]}
    elif inp == "X1":
        query[qfield] = {"$or": [{"$like": "X1(%", "$not": {"$like": "%,%"}}, {
            "$in": ["X(1)", "X0(2)"]}]}
...
class ModCurve_download(Downloader):
    ...
    def download_modular_curve_magma_str(self, label):
        ...
        s += "// Isomorphic to P^1?\n"
        is_P1 = "true" if (
            rec['genus'] == 0 and rec['pointless'] is False) else "false"
        s += "is_P1 := %s;\n" % is_P1
        model_id = 0
        for m in models:
            if m["model_type"] == 0:
        ...
        # This was using t twice when the genus was large enough that t was
        # used above. Even when t is not defined above, t is not used below.
        for m in maps:
            ...
            if has_codomain_equation:
                s += "// Codomain equation:\n"
                eq = [eq for eq in codomain_models if eq["modcurve"] ==
                      m["codomain_label"] and eq["model_type"] == m["codomain_model_type"]][0]
                s += prefix + "codomain := " + \
                    "[%s];\n" % ",".join(eq["equation"])
            map_id += 1
        return s
...
def modcurve_search(info, query):
    ...
    if not info.get("points_type"):  # default, which is non-cuspidal
        parse_ints(
            ...
            query,
            "points",
            qfield="num_known_degree1_noncusp_points")
    elif info["points_type"] == "noncm":
        parse_ints(
            info,
            query,
            "points",
            qfield="num_known_degree1_noncm_points")
    ...
    if "cm_discriminants" in info:
        if info["cm_discriminants"] == "yes":
            ...
            query["cm_discriminants"] = {
                "$or": [{"$contains": int(D)} for D in [-3, -12, -27]]}
        elif info["cm_discriminants"] == "-4,-16":
            query["cm_discriminants"] = {
                "$or": [{"$contains": int(D)} for D in [-4, -16]]}
        elif info["cm_discriminants"] == "-7,-28":
            query["cm_discriminants"] = {
                "$or": [{"$contains": int(D)} for D in [-7, -28]]}
        else:
            query["cm_discriminants"] = {
                "$contains": int(
                    info["cm_discriminants"])}
    ...
    if "covered_by" in info:
        ...
        if lmfdb_label is None:
            ...
            if "-" in lmfdb_label:
                # fine label
                rec = db.gps_gl2zhat_fine.lookup(
                    lmfdb_label, ["parents", "coarse_label"])
...
@embed_wrap(
    ...
    # which prioritizes values found in info (which are set in family_page()
    # before calling render_family)
    bread=lambda: None,
...
ratpoint_columns = SearchColumns([
    ...
    LinkCol(
        ...
        "Elliptic curve",
        lambda Elabel: url_for_ECNF_label(Elabel) if "-" in Elabel else url_for_EC_label(Elabel)),
    ProcessedCol(
        "residue_field",
...
def ratpoint_postprocess(res, info, query):
    ...
    RSZBlabels = {rec["label"]: rec["RSZBlabel"] for rec in db.gps_gl2zhat_fine.search(
        {"label": {"$in": labels}}, ["label", "RSZBlabel"])}
    for rec in res:
...
def statistics():
    ...
    return render_template("display_stats.html", info=ModCurve_stats(
    ), title=title, bread=get_bread('Statistics'), learnmore=learnmore_list())
...
def modcurve_data(label):
    ...
    if not LABEL_RE.fullmatch(label):
        ...
        if label != coarse_label:
            label_tables_cols.append(
                (coarse_label, "gps_gl2zhat_fine", "label"))
        # modcurve_models
        label_tables_cols.append((coarse_label, "modcurve_models", "modcurve"))
        # modcurve_modelmaps
        label_tables_cols.append(
            (coarse_label, "modcurve_modelmaps", "domain_label"))
        # modcurve_points
        label_tables_cols.append(
            (coarse_label, "modcurve_points", "curve_label"))

    labels, tables, label_cols = map(
        list, zip(*label_tables_cols))  # transpose
    return datapage(
        labels, tables, title=f"Modular curve data - {label}", bread=bread, label_cols=label_cols)
