...
class XnsplusN(ModCurveFamily_base):
    ...
    sl2level = "N"
    index = "N+1"
    psl2index = r"i = N \cdot \varphi(N) \cdot 2^{-\omega(N)}"
    genus = "g"
    nu2 = r"$\nu_2 = N \cdot 2^{-\omega(N)} \cdot \prod_{p\mid N} \begin{cases}1 - \frac{1}{p} & \text{if } p \equiv 1 \bmod 4, \\ 1 & \text{if } p=2, \\ 1 + \frac{1}{p} + \frac{2}{p^r} & \text{if } p \equiv 3 \bmod 4 \text{ (}r \text{ is the highest power of } p\mid N). \end{cases}$"
    nu3 = r"$\nu_3 = \begin{cases}1 & \text{if } p \equiv 2 \bmod 3 \text{ for every } p \mid N, \\ 0 & \text{otherwise}. \end{cases}$"
    cusps = r"$\nu_\infty = \varphi(N) \cdot 2^{-\omega(N)}$"
    rational_cusps = r"$\nu_\infty = 0$"
...
class XarithN(ModCurveFamily_base):
    ...
    sl2level = "N"
    index = "N+1"
    psl2index = r"i = \begin{cases}6, & \text{if } N=1, \\ \frac{N^3}{2}\prod_{p\mid N, \text{prime}}(1-p^{-2}), & \text{if } N>2. \end{cases}"
    genus = "g"
    nu2 = r"$\nu_2 = \begin{cases}1, & \text{if } N=1, \\ 0, & \text{if } N>1. \end{cases}$"
    nu3 = r"$\nu_3 = \begin{cases}1, & \text{if } N=1, \\ 0, & \text{if } N>1. \end{cases}$"
    cusps = r"$\nu_\infty = \begin{cases}1, & \text{if } N=1,\\ i/N, & \text{if } N>1.\end{cases}$"
