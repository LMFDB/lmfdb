...
class Xarith1MMN(ModCurveFamily_base):
    ...
    moduli_description = fr"$X_{{\mathrm{{arith}},1}}(M,MN)$ is the {display_knowl('modcurve','modular curve')} $X_H$ for $H\le \GL_2(\widehat\Z)$ the inverse image of $\begin{{pmatrix}} 1 & M* \\ 0 & * \end{{pmatrix}} \subset \GL_2(\Z/MN\Z)$. As a moduli space it parameterizes triples $(E,P,C)$, where $E$ is an elliptic curve over $k$, $P \in E[MN](k)$ is a point of order $MN$, and $C\leq E[M](\overline{{k}})$ is a $\Gal_k$-stable cyclic subgroup of order $M$ such that $E[M]=\langle NP\rangle + C$. <p> The {display_knowl('modcurve.canonical_field', 'canonical field of definition')} of $X_{{\mathrm{{arith}},1}}(M,MN)$ is $\Q$.</p>"
    genus_formula = r"$$g = 1 + \frac{i}{12} - \frac{\nu_2}{4} - \frac{\nu_3}{3} - \frac{\nu_\infty}{2}$$"
    #hypell_description = "Unknown."
    #biell_description = "Unknown."
