...
class XnsN(ModCurveFamily_base):
    ...
    cusps = "N/2"
    rational_cusps = "1"
    moduli_description = r"$X_{\text{ns}}(N)$ is the {display_knowl('modcurve','modular curve')} $X_H$ for $H\le \GL_2(\widehat\Z)$ the inverse image of a {display_knowl('gl2.cartan', 'Cartan subgroup')} of $\GL_2(\Z/N\Z)$ that is nonsplit at every prime dividing $N$."
...
class XnsplusN(ModCurveFamily_base):
    ...
    cusps = "N/2"
    rational_cusps = "1"
    moduli_description = r"$X_{\text{ns}}^+(N)$ is the {display_knowl('modcurve','modular curve')} $X_H$ for $H\le \GL_2(\widehat\Z)$ the inverse image of an {display_knowl('gl2.cartan', 'extended Cartan subgroup')} of $\GL_2(\Z/N\Z)$ that is nonsplit at every prime dividing $N$."
    genus_formula = r"The genus of $X_{\text{ns}}^+(N)$ is $g$."
...
class XarithN(ModCurveFamily_base):
    ...
    cusps = "N/2"
    rational_cusps = "1"
    moduli_description = r"$X_{\textup{arith}}(N)$ is the {display_knowl('modcurve','modular curve')} $X_H$ for $H\leq \GL_2(\widehat{\Z})$ the inverse image of $\begin{pmatrix}1&0\\0&*\end{pmatrix}\leq \GL_2(\Z/N\Z)$. As a moduli space, $X_{\textup{sym}}$ parametrizes isomorphism classes of triples $(E,\phi,P)$, where $E$ is a generalized elliptic curve, $P$ is a point of exact order $N$, and $\phi \colon E \to E'$ is a cyclic $N$-isogeny such that $E[N]$ is generated by $P$ and $\ker\phi$. Alternatively, it parametrizes isomorphism classes of pairs $(E,\psi)$ where $E$ is a generalized elliptic curve and $\psi \colon \mu_N\times \Z/N\Z\xrightarrow{\sim} E[N]$ is a symplectic isomorphism."
    genus_formula = r"The genus of $X_{\textup{arith}}(N)$ is $g$."
