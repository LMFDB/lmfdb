...
class X0N(ModCurveFamily_base):
    ...
    cusps = r"$\nu_\infty = \sum\limits_{d|N, d>0} \varphi((d,N/d))$"
    rational_cusps = "1"
...
class X1N(ModCurveFamily_base):
    ...
    sl2level = "N"
    index = "N+1"
    psl2index = r"i = \begin{cases} 1, & \text{ if } N = 1, \\ 3, & \text{ if } N = 2, \\ \frac{N^2}{2}\prod\limits_{p|N, \text{ prime}} \left( 1 - \frac{1}{p^2} \right), & \text{ otherwise.}\end{cases}"
    genus = "g"
    nu2 = r"$\nu_2 = \begin{cases} 1, & \text{ if } N =1,2, \\ 0, & \text{ otherwise.}\end{cases}$"
    nu3 = r"$\nu_3 = \begin{cases} 1, & \text{ if } N =1,3, \\ 0, & \text{ otherwise.}\end{cases}$"
    cusps = r"$\nu_\infty = \begin{cases} 1, & \text{ if } N =1, \\ 2, & \text{ if } N = 2, \\ 3, & \text{ if } N = 4, \\ \frac{1}{2} \sum\limits_{d|N,d>0} \varphi(d)\varphi(N/d), & \text{ otherwise.} \end{cases}$"
    rational_cusps = "1"
...
class XN(ModCurveFamily_base):
    ...
    sl2level = "N"
    index = "N+1"
    psl2index = r"i = \begin{cases} 6, & \text{ if } N = 2, \\ \frac{N^3}{2}\prod\limits_{p|N, \text{ prime}} \left( 1 - \frac{1}{p^2} \right), & \text{ if } N >2. \end{cases}"
    genus = "g"
...
class Xarith1MMN(ModCurveFamily_base):
    ...
    sl2level = "N"
    index = "N"
    psl2index = r"i = \frac{M^3N^2}{2} \cdot \prod\limits_{p|MN} \left(1 - \frac{1}{p^2}\right)"
    genus = "g"
    nu2 = r"$\nu_2 = 0$"
    nu3 = r"$\nu_3 = 0$"
    cusps = r"$\nu_\infty = \sum\limits_{d \mid MN } \varphi (MN/d) \varphi(d) \gcd (M,MN/d)$"
...
class XspN(ModCurveFamily_base):
    ...
    nu3 = r"$\nu_3 = \begin{cases}0 & \text{if $2\mid N$,} \\ \prod\limits_{p|N} \left(1 + \left(\frac{-3}{p}\right) \right) & \text{otherwise.}\end{cases}$"
    ...
    genus_formula = r"$$ g = 1 + \frac{ i }{12} - \frac{ \nu_2 }{4} - \frac{ \nu_3}{3} - \frac{ \nu_\infty}{2}$$"
    hypell_description = r'[<a href="https://mathscinet.ams.org/mathscinet/article?mr=3532138">MR:3532138</a>] showed that there is only 1 hyperelliptic curve, namely $X_{\mathrm{sp}}(11)$. See the [<a href="https://beta.lmfdb.org/ModularCurve/Q/?level=11&family=Xsp">following table</a>] for the full list.'
    biell_description = "Unknown."
...
class XspplusN(ModCurveFamily_base):
    ...
    genus = "g"
    nu2 = r"$\nu_2 = \begin{cases}0 & \text{if $2\mid N$, } \\ \frac{1}{2}\cdot \prod\limits_{p|N} \left( 1 + \left(\frac{-1}{p}\right) \right) & \text{otherwise.}\end{cases} + \left(2^{-\delta} \frac{N}{2}\cdot \prod\limits_{p\mid N, p \equiv 1 \bmod 4}\left( 1 - \frac{1}{p}\right) \cdot \prod\limits_{p\mid N, p \equiv 3 \bmod 4}\left(1 + \frac{1}{p}\right)\right)$"
    nu3 = r"$\nu_3 = \begin{cases}0 & \text{if $2\mid N$,} \\ \frac{1}{2}\cdot \prod\limits_{p|N} \left(1 + \left(\frac{-3}{p}\right) \right) & \text{otherwise.}\end{cases} $"
...
class XnsN(ModCurveFamily_base):
    ...
    genus = "g"
    nu2 = r"$\nu_2 = \prod\limits_{p\mid N} \left( 1 - \left( \frac{-1}{p} \right) \right)$"
    nu3 = r"$\nu_3 = \prod\limits_{p|N} \left( 1 - \left( \frac{-3}{p} \right) \right)$"
    cusps = r"$\nu_\infty = \varphi(N)$"
    rational_cusps = "1"    
...
class XnsplusN(ModCurveFamily_base):
    ...
    nu3 = r"$\nu_3 = \frac{1}{2}\cdot \prod\limits_{p|N} \left( 1 - \left( \frac{-3}{p} \right) \right)$"
    cusps = r"$\nu_\infty = \varphi(N)/2$"
    rational_cusps = r"$\nu_\infty = 0$"
    ...
    hypell_description = "Unknown."
    biell_description = "Unknown."
    notation = r"<ul> <li>$S = \{a+b\alpha \in (\mathbb{Z}/N\mathbb{Z})[\alpha]^\times/\pm 1 : N(a+b\alpha) = -1, \operatorname{gcd}(b,N) >1\}.$ </li> </ul>"
...
class XarithN(ModCurveFamily_base):
    ...
    sl2level = "N"
    index = "N+1"
    psl2index = r"i = \begin{cases}6, & \text{if } N=1, \\ \frac{N^3}{2}\prod\limits_{p\mid N, \text{prime}}\left(1-\frac{1}{p^2}\right), & \text{if } N>2. \end{cases}"
    genus = "g"
    nu2 = r"$\nu_2 = \begin{cases}1, & \text{if } N=1, \\ 0, & \text{if } N>1. \end{cases}$"
    nu3 = r"$\nu_3 = \begin{cases}1, & \text{if } N=1, \\ 0, & \text{if } N>1. \end{cases}$"
    cusps = r"$\nu_\infty = \begin{cases}1, & \text{if } N=1,\\ i/N, & \text{if } N>1.\end{cases}$"
