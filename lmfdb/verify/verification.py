...
class TableChecker():
    ...
    def get_checks(self, typ):
        return [MethodType(f, self) for fname, f in inspect.getmembers(
            self.__class__) if isinstance(f, typ)]
    ...
    def _make_join(self, join1, join2):
        ...
        return SQL(" AND ").join(SQL("{0} = {1}").format(
            j1, j2) for j1, j2 in zip(join1, join2))

    def _run_query(self, condition=None, constraint={},
                   values=None, table=None, query=None, ratio=1):
        """
    ...
    def _run_crosstable(self, quantity, other_table, col, join1, join2=None, constraint={
    }, values=[], subselect_wrapper="", extra=None):
        """
        ...
        condition = SQL("{0} != {1}(SELECT {2} FROM {3} t2 WHERE {4}{5})").format(
            ...
            join,
            extra)
        return self._run_query(condition, constraint, values, table=SQL(
            "{0} t1").format(Identifier(self.table.search_table)))
    ...
    def check_eq(self, col1, col2, constraint={}):
        return self._run_query(SQL("{0} != {1}").format(
            Identifier(col1), Identifier(col2)), constraint)
    ...
    def check_array_dotproduct(
            self, array_column1, array_column2, value_column, constraint={}):
        """
    ...
    def check_values(self, values, constraint={}):
        ...
        if vstr is None:
            ...
            return self._run_query(SQL("NOT ({0})").format(
                vstr), constraint, values=vvalues)
    ...
    def check_array_len_eq_constant(
            self, column, limit, constraint={}, array_dim=1):
        """
    ...
    def check_array_len_col(self, array_column, len_column,
                            constraint={}, shift=0, array_dim=1):
        """
    ...
    def check_array_bound(self, array_column, bound,
    ...
    def check_string_concatenation(self,
        ...
        oc = [oc_converted[i // 2] if i % 2 ==
              0 else Literal(sep[i // 2]) for i in range(2 * len(oc_converted) - 1)]
    ...
    def check_string_startswith(self, col, head, constraint={}):
        ...
        return self._run_query(SQL("NOT ({0} LIKE %s)").format(
            Identifier(col)), constraint=constraint, values=[value])

    def check_sorted(self, column):
        return self._run_query(
            SQL("{0} != sort({0})").format(Identifier(column)))

    def check_crosstable(self, other_table, col1, join1,
                         col2=None, join2=None, constraint={}):
        """
        ...
        return self._run_crosstable(
            col2, other_table, col1, join1, join2, constraint=constraint)

    def check_crosstable_count(
            self, other_table, col, join1, join2=None, constraint={}):
        """
        ...
        """
        return self._run_crosstable(
            SQL("COUNT(*)"), other_table, col, join1, join2, constraint)

    def check_crosstable_sum(self, other_table, col1,
                             join1, col2=None, join2=None, constraint={}):
        """
    ...
    def check_crosstable_dotprod(
            self, other_table, col1, join1, col2, join2=None, constraint={}):
        """
    ...
    def check_crosstable_aggregate(self, other_table, col1, join1,
                                   col2=None, join2=None, sort=None, truncate=None, constraint={}):
        """
        ...
        return self._run_crosstable(
            col2, other_table, col1, join1, join2, constraint, subselect_wrapper="ARRAY", extra=sort)

    def check_letter_code(self, index_column,
                          letter_code_column, constraint={}):
        return self._run_query(SQL("{0} != to_base26({1} - 1)").format(
            Identifier(letter_code_column), Identifier(index_column)), constraint)
    ...
    def check_uniqueness_constraints(self):
        ...
        """
        constraints = {tuple(sorted(D['columns'])) for D in self.table.list_constraints(
        ).values() if D['type'] == 'UNIQUE'}
        return [constraint for constraint in self.uniqueness_constraints if tuple(
