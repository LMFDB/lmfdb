...
class mf_gamma1(MfChecker):
    ...
    def check_box_count(self):
        ...
        def make_query(box):
            ...
            # Have to remove small N if there are restrictions on the character
            # order
            if 'omin' in box:
                if box['omin'] == 2:
    ...
    def check_newspaces_numforms(self):
        ...
        """
        # TIME about 2s
        return self.check_crosstable_sum(
            'mf_newspaces', 'num_forms', ['level', 'weight'])
    ...
    def check_newspaces_hecke_orbit_dims(self):
        ...
        """
        # TIME about 10s
        return self.check_crosstable_aggregate('mf_newforms', 'hecke_orbit_dims', [
                                               'level', 'weight'], 'dim', sort=['char_orbit_index', 'hecke_orbit'])
    ...
    def check_newspaces_newspace_dims(self):
        ...
        """
        # TIME about 5s
        return self.check_crosstable_aggregate('mf_newspaces', 'newspace_dims', [
                                               'level', 'weight'], 'dim', sort=['char_orbit_index'])
    ...
    def check_newspaces_num_spaces(self):
        ...
        """
        # TIME about 2s
        # TODO: check that the number of char_orbits of level N and weight k is
        # the same as the number of rows in mf_newspaces with this weight and
        # level.  The following doesn't work since num_spaces counts spaces
        # with positive dimension
        return self._run_crosstable(SQL("COUNT(*)"), 'mf_newspaces', 'num_spaces', [
                                    'level', 'weight'], extra=SQL(" AND t2.dim > 0"))
    ...
    def check_portraits_count(self):
        ...
        """
        return self.check_crosstable_count('mf_gamma1_portraits', 1, 'label', constraint={
                                           'dim': {'$gt': 0}, 'level': {'$lte': 4000}})

    ### slow ###
    @slow(projection=['level', 'level_radical', 'level_primes', 'level_is_prime',
                      'level_is_prime_power', 'level_is_squarefree', 'level_is_square'])
    ...
    def check_sturm_bound(self, rec, verbose=False):
        ...
        """
        return self._test_equality(rec['sturm_bound'], sturm_bound1(
            rec['level'], rec['weight']), verbose)

    @fast(constraint={'weight': {'$gt': 1}},
    ...
    def check_Sk_dim_formula(self, rec, verbose=False):
        ...
        """
        # TIME about 60s
        return self._test_equality(rec['dim'], dimension_new_cusp_forms(
            Gamma1(rec['level']), rec['weight']), verbose)

    @fast(constraint={'weight': {'$gt': 1}},
          projection=['level', 'weight', 'eis_dim', 'cusp_dim', 'mf_dim'])
