...
class mf_newforms(MfChecker):
    ...
    def check_box_count(self):
        ...
        for box in db.mf_boxes.search({'newform_count': {'$exists': True}}):
            bad_label = self.check_count(
                box['newform_count'], self._box_query(box))
            if bad_label:
                return bad_label
    ...
    def check_space_label(self):
        ...
        return self.check_string_concatenation(
            'space_label', ['level', 'weight', 'char_orbit_label'])
    ...
    def check_newspaces_overlap(self):
        ...
        # TIME > 120s
        bad_labels = []
        labels = self.check_crosstable_count(
            'mf_newspaces', 1, 'space_label', 'label')
        bad_labels.extend([label + " (count)" for label in labels])
        for col in ['Nk2', 'analytic_conductor', 'char_conductor', 'char_degree', 'char_is_real', 'char_orbit_index', 'char_orbit_label', 'char_order', 'char_parity', 'char_values', 'conrey_index',
                    'level', 'level_is_prime', 'level_is_prime_power', 'level_is_square', 'level_is_squarefree', 'level_primes', 'level_radical', 'prim_orbit_index', 'weight', 'weight_parity']:
            labels = self.check_crosstable(
                'mf_newspaces', col, 'space_label', col, 'label')
    ...
    def check_polredabs_set(self):
        ...
        """
        return self.check_values({'is_polredabs': True}, {
                                 'nf_label': {'$exists': True}})
    ...
    def check_field_poly(self):
        ...
        return self._run_query(SQL(
            'array_length(field_poly, 1) = 1 AND field_poly[dim + 1]  = 1'), {'field_poly': {'$exists': True}})
    ...
    def check_traces_length(self):
        ...
        return (self.check_non_null(['traces'])
                ...
                + self.check_array_len_gte_constant(
            'traces', 2000, constraint={
                'level': {
                    '$gt': 1000, '$lte': 4000}})
    ...
    def check_number_field(self):
        ...
        return (self.check_crosstable_count('nf_fields', 1, 'nf_label', 'label', constraint=nfyes)
                + self.check_crosstable('nf_fields',
                                        'field_poly',
                                        'nf_label',
                                        'coeffs',
                                        'label',
    ...
    def check_field_disc(self):
        ...
        return self.check_non_null(
            ['field_disc'], {'hecke_ring_index_proved': True})
    ...
    def check_analytic_rank_proved(self):
        ...
        """
        # TIME about 5s
        return list(self.table.search(
            {'analytic_rank_proved': False, 'analytic_rank': {'$exists': True}}, 'label'))
    ...
    def check_self_twist_type(self):
        ...
        return (self.check_non_null(['is_cm', 'is_rm'])
                + self.check_iff({'self_twist_type': 0},
                                 {'is_cm': False, 'is_rm': False})
                + self.check_iff({'self_twist_type': 1},
                                 {'is_cm': True, 'is_rm': False})
                + self.check_iff({'self_twist_type': 2},
                                 {'is_cm': False, 'is_rm': True})
    ...
    def check_fricke_eigenval(self):
        ...
        """
        # TIME about 3s
        return self._run_query(SQL('fricke_eigenval != prod2(atkin_lehner_eigenvals)'), {
                               'fricke_eigenval': {'$exists': True}})
    ...
    def check_trivial_character_cols(self):
        ...
        # TIME about 1s
        yes = {'$exists': True}
        return self.check_iff({'atkin_lehner_eigenvals': yes, 'atkin_lehner_string': yes,
                               'fricke_eigenval': yes}, {'char_orbit_index': 1})
    ...
    def check_has_non_self_twist(self):
        ...
        return (self.check_iff({'inner_twist_count': -1}, {'has_non_self_twist': -1}) +
                self.check_values({'inner_twist_count': 1}, {
                                  'has_non_self_twist': 0, 'self_twist_type': 0})
                + self.check_values({'inner_twist_count': 2},
                                    {'has_non_self_twist': 0, 'self_twist_type': 1})
                + self.check_values({'inner_twist_count': 2},
                                    {'has_non_self_twist': 0, 'self_twist_type': 2})
                + self.check_values({'inner_twist_count': 4},
                                    {'has_non_self_twist': 0, 'self_twist_type': 3})
                + self.check_values({'inner_twist_count': {'$gt': 1}},
                                    {'has_non_self_twist': 1, 'self_twist_type': 0})
                + self.check_values({'inner_twist_count': {'$gt': 2}},
                                    {'has_non_self_twist': 1, 'self_twist_type': 1})
                + self.check_values({'inner_twist_count': {'$gt': 2}},
                                    {'has_non_self_twist': 1, 'self_twist_type': 2})
    ...
    def check_field_disc_factorization(self):
        ...
        """
        # TIME about 3s
        return self._run_query(SQL('field_disc != prod_factorization(field_disc_factorization)'), {
                               'field_disc': {'$exists': True}})
    ...
    def check_hecke_ring_index_factorization(self):
        ...
        """
        # TIME about 2s
        return self._run_query(SQL('hecke_ring_index != prod_factorization(hecke_ring_index_factorization)'), {
                               'hecke_ring_index_factorization': {'$exists': True}})
    ...
    def check_self_dual_by_embeddings(self):
        ...
        db._execute(
            SQL("CREATE TEMP TABLE tmp_cc AS SELECT t1.hecke_orbit_code, every(0 = all(t1.an_normalized[:][2:2] )) self_dual FROM mf_hecke_cc t1, mf_newforms t2 WHERE t1.hecke_orbit_code=t2.hecke_orbit_code AND t2.is_self_dual AND t2.field_poly is NULL GROUP BY t1.hecke_orbit_code"))
        query = SQL(
            "SELECT t1.label FROM mf_newforms t1, tmp_cc t2 WHERE NOT t2.self_dual AND t1.hecke_orbit_code = t2.hecke_orbit_code")
    ...
    def check_self_dual_lfunctions(self):
        ...
        # TIME > 1200s
        db._execute(
            SQL("CREATE TEMP TABLE temp_mftbl AS SELECT label, string_to_array(label,'.'), is_self_dual FROM mf_newforms"))
        db._execute(
            SQL("CREATE TEMP TABLE temp_ltbl AS SELECT (string_to_array(origin,'/'))[5:8], every(self_dual) self_dual FROM lfunc_lfunctions WHERE origin LIKE 'ModularForm/GL2/Q/holomorphic%' and degree=2 GROUP BY (string_to_array(origin,'/'))[5:8]"))
        db._execute(
            SQL("CREATE INDEX temp_ltbl_string_to_array_index on temp_ltbl using HASH(string_to_array)"))
        db._execute(
            SQL("CREATE INDEX temp_mftbl_string_to_array_index on temp_mftbl using HASH(string_to_array)"))
        query = SQL(
            "SELECT t1.label FROM temp_mftbl t1, temp_ltbl t2 WHERE t1.is_self_dual != t2.self_dual AND t2.string_to_array = t1.string_to_array")
        res = self._run_query(query=query)
        db._execute(SQL("DROP TABLE temp_mftbl"))
        db._execute(SQL("DROP TABLE temp_ltbl"))
        return res

    @fast(constraint={'projective_field': {'$exists': True}}, projection=[
          'projective_field', 'projective_image', 'projective_image_type'])
    ...
    @fast(constraint={'inner_twists': {'$exists': True}},
          projection=['self_twist_discs', 'inner_twists'])
    def check_self_twist_disc(self, rec, verbose=False):
        ...
        """
        return self._test_equality(set(rec['self_twist_discs']), {
                                   elt[6] for elt in rec['inner_twists'] if elt[6] not in [None, 0, 1]}, verbose)
    ...
    @fast(constraint={'field_poly': {'$exists': True}},
          projection=['field_poly', 'field_poly_is_cyclotomic'])
    ...
    @fast(constraint={'nf_label': None, 'field_poly': {'$exists': True}},
          projection=['field_poly', 'is_self_dual'])
    ...
    @slow(projection=['level', 'weight',
                      'char_orbit_index', 'dim', 'related_objects'])
    def check_related_objects(self, rec, verbose=False):
        ...
        if len(names) != len(rec['related_objects']):
            if verbose:
                print(
                    "Length failure", len(names), len(
                        rec['related_objects']))
            return False
        # if related_objects contains an Artin rep, check that k=1 and that
        # conductor of artin rep matches level N
        ...
        # if k=2, char_orbit_index=1 and dim=1 check that elliptic curve
        # isogeny class of conductor N is present in related_objects
            if url.startswith('/EllipticCurve/Q/'):
                ...
                if rec['dim'] == 1:
                    ...
                    if rec['level'] != int(name.split()[-1].split('.')[0]):
                        if verbose:
                            print("EC level failure", url, rec['level'], int(
    ...
    @slow(ratio=1, constraint={'artin_image': {'$exists': True}},
          projection=['projective_image', 'artin_image', 'artin_degree'])
    ...
    def check_traces_count(self):
        ...
        return self.check_crosstable_count(
            'mf_hecke_traces', 1000, 'hecke_orbit_code')
    ...
    def check_traces_match(self):
        ...
        """
        # TIME > 600s
        return self.check_crosstable_aggregate(
            'mf_hecke_traces', 'traces', 'hecke_orbit_code', 'trace_an', sort=['n'], truncate=1000)
    ...
    def check_lpoly_count(self):
        ...
        """
        # TIME about 200s
        return self.check_crosstable_count('mf_hecke_lpolys', 25, 'hecke_orbit_code', constraint={
                                           'field_poly': {'$exists': True}})
    ...
    def check_embeddings_count(self):
        ...
        """
        # TIME > 1000s
        return accumulate_failures(self.check_crosstable_count('mf_hecke_cc', 'dim', 'hecke_orbit_code',
                                                               constraint=self._box_query(box)) for box in db.mf_boxes.search({'embeddings': True}))
    ...
    def check_embeddings_count_boxcheck(self):
        ...
        return [str(box['embedding_count']) for box in db.mf_boxes.search({'embeddings': True}) if sum(
            self.table.search(self._box_query(box), 'dim')) != box['embedding_count']]
    ...
    @slow(constraint={'field_poly': {'$exists': True}},
          projection=['field_poly', 'hecke_orbit_code'])
    def check_roots_are_roots(self, rec, verbose=False):
        ...
        dbroots = db.mf_hecke_cc.search({'hecke_orbit_code': rec['hecke_orbit_code']}, [
                                        "embedding_root_real", "embedding_root_imag"])
        dbroots = [CCC(root["embedding_root_real"],
                       root["embedding_root_imag"]) for root in dbroots]
        if len(dbroots) != poly.degree():
            if verbose:
                print("Wrong number of roots")
            return False
        for r in dbroots:
            # f is irreducible, so all roots are simple and checking relative
            # error is the way to go
            if poly(r) / dpoly(r) > 1e-11:
                # It's still possible that the roots are correct; it could just
        ...
        # greedily match.  The degrees are all at most 20, so it's okay to use
        ...
        while len(roots) > 0:
            ...
            # The dim 1 case where poly=x is handled correctly in the earlier
            # loop, so r != 0.
    ...
    def check_traces(self):
        ...
        query = SQL(
            "WITH foo AS (  SELECT hecke_orbit_code, traces(array_agg(an_normalized[1:%s])) traces FROM mf_hecke_cc GROUP BY hecke_orbit_code) SELECT t1.label FROM mf_newforms t1, foo WHERE t1.hecke_orbit_code = foo.hecke_orbit_code AND NOT compare_traces(t1.traces[1:%s], foo.traces, -0.5*(t1.weight - 1)) AND t1.weight <= 272")
