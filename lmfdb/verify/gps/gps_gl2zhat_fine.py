...
def bkm_bounds(p, dims, mults):
    ...
    if p == 2:
        return sum([m * d * (bkm_const(p, d) + 1)
...
class gps_gl2zhat_fine(TableChecker):
    ...
    # We can't use the default check_label, since our labels come in two
    # flavors
    @overall
    def check_label(self):
        ...
        """
        bad_labels = self.check_string_concatenation(
            "label", [
                "coarse_class", "coarse_num"], {
    ...
    def check_genus_equals_total_newform_dim(self):
        return self.check_array_dotproduct("dims", "mults", "genus", {
                                           "newforms": {"$exists": True}})
