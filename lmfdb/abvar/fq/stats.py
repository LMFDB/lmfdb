...
class AbvarFqStats(StatsDisplay):
    ...
    buckets = {'q': ['2', '3', '4', '5-8', '9-16', '17-32', '37-64', '67-128', '131-211', '223-1024'],
    ...
    def short_summary(self):
        return r'The database currently contains %s %s of %s of dimension up to %s over finite fields.' % (self.counts['nclasses_c'], self.isogeny_knowl, self.abvar_knowl, max(self.gs))

    @lazy_attribute
    def summary(self):
        return r"The database currently contains %s %s of %s of dimension up to %s over finite fields.  In addition to the statistics below, you can also <a href='%s'>create your own</a>." % (self.counts['nclasses_c'], self.isogeny_knowl, self.abvar_knowl, max(self.gs), url_for("abvarfq.dynamic_statistics"))
