...
class AVTest(LmfdbTest):
    ...
    def test_polynomial(self):
        ...
        """
        self.check_args(
            "/Variety/Abelian/Fq/2/9/aj_bl",
            "1 - 9 x + 37 x^{2} - 81 x^{3} + 81 x^{4}")
    ...
    def test_is_simple(self):
        ...
        self.check_args(
            "/Variety/Abelian/Fq/3/4/e_q_bg",
            '<a title="Simple abelian variety [av.simple]" knowl="av.simple" kwargs="">not simple</a>')
    ...
    def test_newton_polygon_plot(self):
        ...
        """
        page = self.tc.get(
            "/Variety/Abelian/Fq/2/4/ad_g").get_data(as_text=True)
    ...
    def test_property_box(self):
        ...
        """
        page = self.tc.get("/Variety/Abelian/Fq/2/4/ad_g").get_data(
            as_text=True).replace(
            "\n", "").replace(
            " ", "")
    ...
    def test_av_download(self):
        ...
        """
        data = self.tc.get(
            "Variety/Abelian/Fq/5/2/?Submit=sage&download=1&query=%7B%27q%27%3A+2%2C+%27g%27%3A+5%7D").get_data(as_text=True)
        self.assertTrue("Each entry in the following data list" in data)
        self.assertTrue(
            "[1, -10, 50, -160, 360, -592, 720, -640, 400, -160, 32]" in data)
        response = self.tc.get(
            "Variety/Abelian/Fq/5/2/?Submit=gp&download=1&query=%7B%27q%27%3A+2%2C+%27g%27%3A+5%7D")
        self.assertTrue(
            "Each entry in the following data list" in response.get_data(
                as_text=True))
        self.assertTrue(
            "[1, -10, 50, -160, 360, -592, 720, -640, 400, -160, 32]" in response.get_data(as_text=True))
        response = self.tc.get(
    ...
    def test_download_all(self):
        ...
        page = self.tc.get(
            'Variety/Abelian/Fq/download_all/1.81.r',
            follow_redirects=True)
        assert '"abvar_counts": [99, 6435, 532224, 43043715,' in page.get_data(
            as_text=True)

        page = self.tc.get(
            'Variety/Abelian/Fq/download_all/3.17.d_b_act',
            follow_redirects=True)
        assert '"curve_counts": [21, 283, 4719, 84395' in page.get_data(
            as_text=True)

        text = self.tc.get(
            'Variety/Abelian/Fq/data/3.17.d_b_act',
    ...
    def test_download_curves(self):
        ...
        page = self.tc.get(
            'Variety/Abelian/Fq/2.19.ae_w',
            follow_redirects=True)
        assert 'Curves to text' in page.get_data(as_text=True)

        page = self.tc.get(
            'Variety/Abelian/Fq/download_curves/2.19.ae_w',
            follow_redirects=True)
        assert 'y^2=3*x^6+18*x^5+15*x^4+12*x^3+x^2+5*x+18' in page.get_data(
            as_text=True)

        page = self.tc.get(
            'Variety/Abelian/Fq/5/3/ac_e_ai_v_abl',
            follow_redirects=True)
        assert 'Curves to text' not in page.get_data(as_text=True)

        page = self.tc.get(
            'Variety/Abelian/Fq/download_curves/5.3.ac_e_ai_v_abl',
            follow_redirects=True)
        assert 'No curves for abelian variety isogeny class 5.3.ac_e_ai_v_abl' in page.get_data(
            as_text=True)
