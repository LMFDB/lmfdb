...
# The function from_cycle_type_to_conjugacy_class_index_dict is called once for each number field (and thus once for each artin representation)
...
def are_conjugate_in_alternating(rho, sigma):
    ...
    # Because sort bases itself first on the first argument of the tuple, the cycles match up with their lengths
...
def from_cycle_type_to_conjugacy_class_index_dict(defining_polynomial, frobenius_resolvents):
    ...
    for technique in frobenius_resolvents:
        ...
        if technique["Algorithm"] == "CYC":
            tmp = CYC_from_cycle_type_to_conjugacy_class_index_fn(technique)   # a bit convoluted but nice to have the same structure, helps understand
