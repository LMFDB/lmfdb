...
class HomePageTest(LmfdbTest):
    ...
    def test_lattice_classnumber_large_download(self):
        L = self.tc.get(
            "/Lattice/3.1942.3884.56.13/download/sage/genus_reps").get_data(as_text=True)
        # test download genus representatives
        assert 'Matrix([[2, 0, 0], [0, 14, -3], [0, -3, 70]]),' in L

    def test_lattice_search(self):
        L = self.tc.get(
            "/Lattice/?dim=&det=&level=&gram=&minimum=&class_number=1&aut=&count=50").get_data(as_text=True)
        assert '56' in L  # search

    def test_lattice_search_next(self):
        L = self.tc.get(
            "/Lattice/?start=50&dim=&det=&level=&gram=&minimum=&class_number=&aut=2&count=50").get_data(as_text=True)
    ...
    def test_lattice_searchlevel(self):
        L = self.tc.get(
            "/Lattice/?start=&dim=&det=&level=90&gram=&minimum=&class_number=&aut=").get_data(as_text=True)
        assert '16' in L  # level search

    def test_lattice_searchminvectlength(self):
        L = self.tc.get(
            "/Lattice/?dim=&det=&level=&gram=&minimum=3&class_number=&aut=").get_data(as_text=True)
        assert '2.42.84.1.3' in L  # search minimum vector length

    def test_lattice_searchGM(self):
        L = self.tc.get(
            "/Lattice/?dim=&det=&level=&gram=[17%2C6%2C138]&minimum=&class_number=&aut=").get_data(as_text=True)
        assert '4620' in L  # gram matrix search

    def test_lattice_searchGM_2(self):
        L = self.tc.get(
            "/Lattice/?dim=&det=&level=&gram=5%2C3%2C2&minimum=&class_number=&aut=").get_data(as_text=True)
    ...
    def test_lattice_thetadisplay(self):
        L = self.tc.get(
            "/Lattice/theta_display/7.576.18.1.1/40").get_data(as_text=True)
    ...
    def test_lattice_random(self):
        ...
        assert 'Normalized minimal vectors' in L.get_data(
            as_text=True)  # check redirection
    ...
    def test_download_shortest(self):
        L = self.tc.get(
            "/Lattice/13.14.28.8.1/download/magma/shortest_vectors").get_data(as_text=True)
        assert 'data := ' in L

    def test_download_genus(self):
        L = self.tc.get(
            "/Lattice/4.5.5.1.1/download/gp/genus_reps").get_data(as_text=True)
        assert ']~)' in L
