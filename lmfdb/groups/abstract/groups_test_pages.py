...
class GroupsTest(LmfdbTest):
    ...
    def abstract_groups_of_order(self, N, imin, imax):
        ...
        res = []
        n = 0
        for label in self.db.gps_groups.search(
                {"order": N, "counter": {"$gte": imin, "$lte": imax}}, "label"):
            n += 1
        ...
        if errors:
            print(
                f"Tested {n} pages ({N}.{imin} to {N}.{imax}) with {len(errors)} errors occurring on the following pages:")
            for url in errors:
                print(url)
        else:
            print(
                f"No errors while running {n} tests ({N}.{imin} to {N}.{imax})!")
        return res
    ...
    def all_small_groups(self, maxord=None, chunksize=1000):
        ...
        for n in range(1, maxord + 1):
            ...
            for i in range(numchunks):
                inputs.append((n,
                               1 + floor(i / numchunks * numgps),
                               floor((i + 1) / numchunks * numgps)))
        res = sum(
            (outp for inp, outp in self.abstract_groups_of_order(inputs)), [])
        ...
        if errors:
            print(
                f"Tested {total + len(errors)} pages with {len(errors)} errors occurring on the following pages:")
        ...
        print("Quartiles: %.2f %.2f %.2f" %
              tuple(times[max(0, int(total * f) - 1)] for f in [0.25, 0.5, 0.75]))
        print("Slowest pages:")
