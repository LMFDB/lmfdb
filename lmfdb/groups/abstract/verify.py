...
def test_small_gps(sample_gp):
    ...
    # 1024 doesn't have standard labels, need to construct another way
    if sample_gp['order'] != 1024:
        ...
        print("Group orders match: " +
              str(sample_gp['order'] == libgap.Order(G)))
        # Confirm IsAbelian returns same value
        print("Groups abelian-ness match: " +
              str((sample_gp['abelian']) == bool(libgap.IsAbelian(G))))
        # Confirm IsSimple returns same value
        print("Groups simple-ness match: " +
              str((sample_gp['simple']) == bool(libgap.IsSimple(G))))
        # Confirm IsPerfect returns same value
        print("Groups perfect-ness match: " +
              str((sample_gp['perfect']) == bool(libgap.IsPerfect(G))))
        # Confirm IsMonomial returns same value
        print("Groups monomial-ness match: " +
              str((sample_gp['monomial']) == bool(libgap.IsMonomial(G))))
        ...
        if sample_gp['number_subgroup_classes']:
            ...
            print("Number of subgroup classes match: " +
                  str(libgap.Size(Cons) == sample_gp['number_subgroup_classes']))
        if sample_gp['number_normal_subgroups']:
            NormLat = libgap.NormalSubgroups(G)
            print("Number of normal subgroups match: " +
                  str(libgap.Size(NormLat) == sample_gp['number_normal_subgroups']))
        ...
        if sample_gp['permutation_degree']:
            ...
            print("Minimal permutation degrees match: " +
                  str(minpermdeg_gap == sample_gp['permutation_degree']))

        # check order stats
        stupid_str = 'Set(ConjugacyClasses(SmallGroup(' + \
            id_nums[0] + ',' + id_nums[1] + ")), z->Order(Representative(z))) "
        ...
        stupid_str_2 = 'List(Irr(SmallGroup(' + \
            id_nums[0] + ',' + id_nums[1] + ')), z -> z[1])'
