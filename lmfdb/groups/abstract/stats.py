...
ftype_lookup = {
    ...
    33: "$p^{3+}q^{3+}$",
    51: "$p^{5+}q$",
    # at least three distinct prime divisors each with valuation at most 2,
    # not squarefree
    222: r"$p^{1,2}q^{1,2}r^{1,2}\cdots$",
    # at least three distinct prime divisors, exactly one of which has
    # valuation at least 3
    311: r"$p^{3+}qr\cdots$",
...
stype_klookup = {
    stype: stype_insert_knowls(desc) for stype,
    desc in stype_lookup.items()}
...
class GroupStats(StatsDisplay):
    ...
    @lazy_attribute
    def summary(self):
        # In addition to the statistics below, you can also <a
        # href="{url_for(".dynamic_statistics")}">create your own</a>.'
        return fr'The database currently contains {comma(db.gps_groups.count())} {display_knowl("group", "groups")} from {display_knowl("rcs.source.groups.abstract", "many different sources")}, the largest of which is $S_{{47}}$ of {display_knowl("group.order", "order")} $47!$.  In addition, it contains {comma(db.gps_subgroups.count())} of their {display_knowl("group.subgroup", "subgroups")} and {comma(db.gps_char.count())} of their {display_knowl("group.representation.character", "irreducible complex characters")}.'
