# This file is used to determine positions for discs in the image
...
class ThetaRay:
    ...
    def max_theta_span(self):
        # This is the largest difference between thetas for two touching
        # circles in the annulus
...
class Circle:
    ...
    def distance(self, C):
        if isinstance(C, ThetaRay):
            ...
            # Also note that this ignores the radius of the circle: we only use
            # the center
    ...
    def ok(self, others, thetaray):
        if self.R < thetaray.R0 + self.r - eps or self.R > thetaray.R1 - self.r + eps:
            return False
        # We need to worry near the beginning, since if thetaray.theta=0 then
        # every theta works
    ...
    def hole_degree(self, others):
        # We will want to use a manual cache so that we don't have to hash the
        # inputs and can control when it gets updated
        dmin = None
        for C in others:
...
# The HSV color coordinates are obtained from the following dictionary:
# the odd part gives the key, and the valuation is used to index into each
# list.
hsv = {
    # red; 1 is handled separately
    1: (0, 95, [None, 100, 82, 66, 52, 40, 30, 22, 16]),
...
def find_touching_centers(C1, C2, r, o):
    ...
    if isinstance(C2, Outside):
        ...
        if d > rS + eps:
            return []
    # we solve for a point c3 that has distance r1S from the center of C1 and
    # r2S from the center of C2
    if (d - rS).abs() < eps:
    ...
    # Expand for quadratic formula: ((y2-y1)^2 + (x2-x1)^2)x'^2 - 2E(x2-x1) x'
    # + E^2 - (r2S)^2 (y2-y1)^2 = 0
    xD = C2.x - C1.x
    yD = C2.y - C1.y
...
def place_segment(segment, R0, R1, thetamin, placed, last):
    ...
    to_check = []
    # We filter the placed circles to eliminate ones that can't possibly
    # interfere with this segment.
    for C in placed:
        ...
        if d < C.r + rmax:
            ...
            if d < C.r + rmax:
                if last:
                    ...
                    to_check.append(C)
    # We start by constructing corner placements from the circles in
    # constructive, together with the thetamin ray and the outside of the
    # annulus
    corners = []
...
def pack(rdata, R0, rmax):
    ...
    # We approximate the intersections as happening at radius R0+rmax, then
    # check that this doesn't cause problems
    rdata = sorted(
        rdata, key=lambda pair: (-pair[1].valuation(2), -pair[1].valuation(3), pair[1], -pair[0]))
    ...
    if thetasum < 2 * pi:
        ...
        for i in range(len(rdata) - 1):
            thetas.append(
                thetas[i] + (radii[i] + radii[i + 1]) / Rc + thetaspace)
        pos = [(Rc * theta.cos(), Rc * theta.sin()) for theta in thetas]
        if all(distxy(pos[i], pos[(i + 1) % len(pos)]) >= radii[i] +
               radii[(i + 1) % len(pos)] for i in range(len(pos))):
            return [(x, y, r, get_color(o))
                    for (r, o), (x, y) in zip(rdata, pos)], R0 + 2 * rmax
    ...
    while True:
        ...
        for sctr, segment in enumerate(segments):
            ok, placements = place_segment(
                segment, R0, R1, thetamin, placed, sctr == len(segments) - 1)
            if not ok:
                break
...
def arrange_ring(radii, colors, R0, rmax):
    ...
    n = sum(radii.values())
    thetadiff = (2 * pi / n)
    # distance between centers in equal space case
    dist = Rc * (2 - 2 * thetadiff.cos()).sqrt()
    if len(radii) == 1:
        r = next(iter(radii))
        ...
        while len(placed) < n:
            ...
            cur = heapq.heappushpop(
                remaining, (cur[0] + invcnt[srad[cur[1]]], cur[1]))
    #print("radii", radii)
    #print("colors", colors)
    # print(placed)
    ...
    for r in radii:
        ...
        if len(colors[r]) == 1:
            ...
            while len(color_placed[r]) < radii[r]:
                cur = heapq.heappushpop(
                    remaining, (cur[0] + invcnt[cur[1]], cur[1]))
...
def arrange_rings(radii, colors, R0, rmax):
    ...
    num_rings = 1
    thetaleft = 2 * pi
    # We first need to get a count of how many rings to use.  We greedily
    # assign discs to annuli
    ...
    while True:
        ...
        for r, cnt in sorted(radii.items(), reverse=True):
            ...
            while thetaneeded * cnt > thetaleft:
                ...
                thetaleft = 2 * pi * (1 if len(rings) ==
                                      num_rings else utilization)
                Rc += rmax + r
                rmax = r
                thetaneeded = (1 - 2 * r**2 / Rc**2).arccos()
            rings[-1][0][r] += cnt
...
def arrange(rdata, R0, rmax):
    ...
    # rmin = min(radii)
    if True:  # rmax < 3 * rmin:
        # the circles are close to the same size.  We divide them up into
        # concentric rings greedily
...
def find_packing(ccdata):
    ...
    if R > 10000000:
        scale = RR(10000000) / R
        R = RR(10000000)
        circles = [(x * scale, y * scale, rad * scale, (r, g, b))
