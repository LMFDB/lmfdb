...
def set_info_for_start_page():
    ...
    modulus_list = ["%s-%s" % (start, end - 1) for start, end in zip(modulus_list_endpoints[:-1],
                                                                     modulus_list_endpoints[1:])]
    info = {'modulus_list': modulus_list, 'conductor_list': range(1, 181), 'order_list': range(2, 21),
            'type_table': tt, 'l': [1]}
    info['credit'] = 'Sage'
...
def initCharacterInfo(web_chi, args, request):
    ...
    if args['type'] == 'dirichlet':
        ...
        indices = []
        info['bread'] = [('Dirichlet Characters', '/Character/Dirichlet'), (
            'Character ' + snum + ' modulo ' + smod, '/Character/Dirichlet/' + smod + '/' + snum)]
        ...
        # info['learnmore'] = [('Dirichlet Characters', url_for("knowledge.show",
        # ID="character.dirichlet.learn_more_about"))]
        if web_chi.primitive == "False":
            info['friends'] = []
        else:
