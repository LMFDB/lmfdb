...
from sage.all import gcd, ZZ, Rational, Integers, cached_method, euler_phi, latex
...
class WebDirichlet(WebCharObject):
    ...
    @lazy_attribute
    def codegauss(self):
        return {"sage": ["chi.gauss_sum(a)"], "pari": "znchargauss(g,chi,a)"}
    ...
    def jacobi_sum(self, val):
        ...
        try:
            ...
            raise Warning(
                "n must be a positive integer coprime to the modulus {} and no greater than it".format(
                    mod
                )
            )
        if gcd(mod, val) > 1:
        ...
        psi_values_data = db.char_dir_values.lookup("{}.{}".format(self.modulus, val))
...
class WebChar(WebCharObject):
    ...
    @lazy_attribute
    def codeorder(self):
        return {"sage": "chi.multiplicative_order()", "pari": "charorder(g,chi)"}
    ...
    def kerfield(self):
        ...
        if kerpoly and self.order <= 100:
            ...
            return (
                "Number field defined by a degree %d polynomial (not computed)"
                % self.order
            )
    ...
    def friends(self):
        ...
        f = []
        cglink = url_character(
            type=self.type, number_field=self.nflabel, modulus=self.modlabel
        )
...
class WebCharGroup(WebCharObject):
    ...
    def friends(self):
        if self.nflabel:
            return [
                ("Number field", "/NumberField/" + self.nflabel),
            ]
...
class WebDirichletGroup(WebCharGroup, WebDirichlet):
    ...
    @lazy_attribute
    def codegen(self):
        return {"sage": "H.gens()", "pari": "g.gen"}

    @lazy_attribute
    def codestruct(self):
        return {"sage": "H.invariants()", "pari": "g.cyc"}
...
class WebDBDirichletGroup(WebDirichletGroup, WebDBDirichlet):
    ...
    def _determine_values(self, valuepairs, order):
        ...
        """
        raw_values = [int(v) for g, v in valuepairs]
        values = [self._tex_value(v, order, texify=True) for v in raw_values]
        return values
...
class WebDBDirichletCharacter(WebChar, WebDBDirichlet):
    ...
    def friends(self):
        ...
        gal_orb_link = url_character(
            type=self.type, modulus=self.modulus, orbit_label=self.orbit_label
        )
        ...
        if self.type == "Dirichlet" and self.isprimitive == bool_string(False):
            friendlist.append(
                (
                    "Primitive character " + self.inducing,
                    url_for(
    ...
    def codeisprimitive(self):
        return {"sage": "chi.is_primitive()", "pari": "#znconreyconductor(g,chi)==1"}

    @lazy_attribute
    def codecond(self):
        return {"sage": "chi.conductor()", "pari": "znconreyconductor(g,chi)"}

    @lazy_attribute
    def codeparity(self):
        return {"sage": "chi.is_odd()", "pari": "zncharisodd(g,chi)"}
...
class WebDBDirichletOrbit(WebChar, WebDBDirichlet):
    ...
    def _set_galoisorbit(self, orbit_data):
        ...
        self.galoisorbit = list(
            self._char_desc(num, prim=orbit_data["is_primitive"])
            for num in self.galorbnums
        )
    ...
    def get_orbit_data(self, orbit_label):
        ...
        self.parity = parity_string(int(orbit_data["parity"]))
        self._set_kernel_field_poly(orbit_data)
        self.ind_orbit_label = cremona_letter_code(
            int(orbit_data["prim_orbit_index"]) - 1
        )
    ...
    def friends(self):
        ...
        if self.type == "Dirichlet" and self.isprimitive == bool_string(False):
            friendlist.append(
                (
                    "Primitive orbit " + self.inducing,
                    url_for(
                        "characters.render_Dirichletwebpage",
                        modulus=self.conductor,
                        orbit_label=self.ind_orbit_label,
                    ),
                )
            )
    ...
    def add_row(self, c):
        ...
        valuepairs = db.char_dir_values.lookup(
            "{}.{}".format(mod, num), projection="values"
        )
    ...
    def codeinit(self):
        ...
        values_gens = db.char_dir_values.lookup(
            "{}.{}".format(self.modulus, self.exnum), projection="values_gens"
        )

        vals = [int(v) for g, v in values_gens]
        sage_zeta_order = self.exchi.sage_zeta_order(self.order)
        self._genvalues_for_code = get_sage_genvalues(
            self.modulus, self.order, vals, sage_zeta_order
        )
