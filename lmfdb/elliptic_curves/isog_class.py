...
class ECisog_class():
    ...
    def by_label(label):
        ...
        try:
            ...
            data = db.ec_curvedata.lucky(
                {"lmfdb_iso": label, 'lmfdb_number': 1})
            if data is None:
    ...
    def make_class(self):
        ...
        self.optimality_known = (
            self.conductor < OPTIMALITY_BOUND) or (
            (self.conductor < CREMONA_BOUND) and (
                (self.optimality == 1) or (
                    self.Ciso == '990h')))
        ...
        for c in self.curves:
            c['ai'] = c['ainvs']
            c['curve_url_lmfdb'] = url_for(
                ".by_ec_label", label=c['lmfdb_label'])
            c['curve_url_cremona'] = url_for(
                ".by_ec_label", label=c['Clabel']) if self.conductor < CREMONA_BOUND else "N/A"
            ...
            # convert [num,den] to rational for display
            c['j_inv'] = QQ(tuple(c['jinv']))
        ...
        if self.label_type == 'Cremona':
            def perm(i): return next(
                c for c in self.curves if c['Cnumber'] == i + 1)['lmfdb_number'] - 1
        ...
        self.newform_label = ".".join(
            [str(self.conductor), str(2), 'a', self.iso_label])
        ...
        self.downloads = [('q-expansion to text', url_for(".download_EC_qexp", label=self.lmfdb_iso, limit=1000)),
                          ('All stored data to text', url_for(
                              ".download_EC_all", label=self.lmfdb_iso)),
                          ('Underlying data', url_for(".EC_data", label=self.lmfdb_iso))]

        self.bread = [('Elliptic curves', url_for("ecnf.index")),
                      (r'$\Q$', url_for(".rational_elliptic_curves")),
                      ('%s' % self.conductor, url_for(
                          ".by_conductor", conductor=self.conductor)),
                      ('%s' % self.iso_label, ' ')]
        self.code = {}
