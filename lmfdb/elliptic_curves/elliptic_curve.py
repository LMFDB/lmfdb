...
def get_bread(tail=[]):
    base = [('Elliptic curves', url_for("ecnf.index")),
...
def rational_elliptic_curves(err_args=None):
    ...
    if err_args is None:
        if request.args:
            ...
            err_args = {}
            for field in ['conductor', 'jinv', 'torsion', 'rank',
                          'sha', 'optimal', 'torsion_structure', 'msg']:
                err_args[field] = ''
            err_args['count'] = '50'
    ...
    conductor_list_endpoints = [
        1, 100, 1000, 10000, 100000, int(
            counts.max_N_Cremona) + 1]
    conductor_list = {r: r for r in ["%s-%s" % (start, end - 1) for start, end in zip(conductor_list_endpoints[:-1],
                                                                                      conductor_list_endpoints[1:])]}
    conductor_list[">{}".format(counts.max_N_Cremona)
                   ] = "{}-".format(counts.max_N_Cremona)
...
def random_curve():
    ...
    return url_for(".by_triple_label", conductor=cond,
                   iso_label=iso, number=num)
...
class ECstats(StatsDisplay):
    ...
    def short_summary(self):
        ...
        return r'The database currently includes %s %s defined over $\Q$, in %s %s, with %s at most %s.  Here are some further <a href="%s">statistics and completeness information</a>.' % (
            self.ncurves_c, self.ec_knowl, self.nclasses_c, self.cl_knowl, self.cond_knowl, self.max_N_c, stats_url)

    @property
    def summary(self):
        return r'Currently, the database includes ${}$ {} over $\Q$ in ${}$ {}, with {} at most ${}$.'.format(
            self.ncurves_c, self.ec_knowl, self.nclasses_c, self.cl_knowl, self.cond_knowl, self.max_N_c)
...
# NB the context processor wants something callable and the summary is a
# *property*
...
def ctx_ec_reduction_type():
    return {'ec_reduction_type': lambda x: ["nonsplit multiplicative", "additive",
                                            "split multiplicative", "good"][x + 1] if x >= -1 and x <= 2 else "unknown"}
...
def statistics():
    ...
    return render_template("display_stats.html", info=ECstats(
...
def by_conductor(conductor):
    ...
    info['bread'] = get_bread(
        [('%s' % conductor, url_for(".by_conductor", conductor=conductor))])
    info['title'] = r'Elliptic curves over $\Q$ of conductor %s' % conductor
    if request.args:
        # if conductor changed, fall back to a general search
        if 'conductor' in request.args and request.args['conductor'] != str(
                conductor):
            return redirect(
                url_for(".rational_elliptic_curves", **request.args), 307)
...
def elliptic_curve_jump_error(label, args, missing_curve=False,
                              missing_class=False, invalid_class=False, invalid_poly=False):
    err_args = {}
    for field in ['conductor', 'torsion', 'rank',
                  'sha', 'optimal', 'torsion_structure']:
    ...
    if missing_curve:
        ...
        err_args[
            'err_msg'] = r"%s is not a valid label for an isogeny class of elliptic curves over $\mathbb{Q}$"
    elif invalid_poly:
        ...
        err_args[
            'err_msg'] = r"%s is not a valid label for an elliptic curve or isogeny class over $\mathbb{Q}$"
...
def ec_parse_coeff_vec(label, info):
    ...
    try:
        ...
        lmfdb_label = db.ec_curvedata.lucky(
            {'ainvs': EC_ainvs(E)}, 'lmfdb_label')
...
def ec_lookup_equation(input_str):
    ...
    if ec_defining_poly.coefficient(x**3) == -1:
        try:
            E = EllipticCurve_from_Weierstrass_polynomial(
                ec_defining_poly).minimal_model()
        except ValueError:
    ...
    if label is None:
        ...
        if label:
            ...
            if fail_reason[0] == "invalid_poly":
                return elliptic_curve_jump_error(
                    fail_reason[1], info, invalid_poly=True)
            return elliptic_curve_jump_error(
                fail_reason[1], info, missing_curve=True)
    elif label.count('=') == 1:
        ...
        try:
            E = EllipticCurve_from_Weierstrass_polynomial(
                main_poly).minimal_model()
        except ValueError:
            C_str_latex = fr"\({latex(main_poly)}\)"
            return elliptic_curve_jump_error(
                C_str_latex, info, invalid_poly=True)
        lmfdb_label = db.ec_curvedata.lucky(
            {'ainvs': EC_ainvs(E)}, 'lmfdb_label')
        if lmfdb_label is None:
            return elliptic_curve_jump_error(
                EC_ainvs(E), info, missing_curve=True)
        return by_ec_label(lmfdb_label)
    else:
        return elliptic_curve_jump_error(label, info)
...
modell_image_label_regex = re.compile(
    r'(\d+)(G|B|Cs|Cn|Ns|Nn|A4|S4|A5)(\.\d+)*')

modm_full = r'(\d+)\.(\d+)\.(\d+)\.([a-z]+)\.(\d+)'
modm_not_computed = r'(\d+)\.(\d+)\.(\d+)\.(\?)'
modm_no_negative = r'(\d+)\.(\d+)\.(\d+)-(\d+)\.([a-z]+)\.(\d+)\.(\d+)'
modm_image_label_regex = re.compile(
    modm_full +
    "|" +
    modm_not_computed +
    "|" +
...
def ec_postprocess(res, info, query):
    ...
    mwgens = {rec["lmfdb_label"]: rec["gens"] for rec in db.ec_mwbsd.search(
        {"lmfdb_label": {"$in": labels}}, ["lmfdb_label", "gens"])}
    for rec in res:
...
ec_columns = SearchColumns([
    ...
    MultiProcessedCol("cremona_label", "ec.q.cremona_label", "Cremona label",
                      ["Clabel", "conductor"],
                      lambda label, conductor: '<a href="%s">%s</a>' % (url_for(
                          ".by_ec_label", label=label), label) if conductor < CREMONA_BOUND else " - ",
    ...
    MathCol(
        ...
        "Class size",
        align="center",
        default=lambda info: info.get("class_size") or info.get("optimal") == "on"),
    MathCol("class_deg", "ec.isogeny_class_degree", "Class degree",
            align="center", default=lambda info: info.get("class_deg")),
    ProcessedCol(
        "conductor",
        "ec.q.conductor",
        "Conductor",
        lambda v: web_latex_factored_integer(
            ZZ(v)),
        align="center"),
    ...
    ProcessedCol("geom_end_alg", "ag.endomorphism_algebra", r"$\textrm{End}^0(E_{\overline\Q})$",
                 lambda v: r"$\Q$" if not v else r"$\Q(\sqrt{%d})$" % (
                     integer_squarefree_part(v)),
    ...
    MathCol("adelic_level", "ec.galois_rep", "Adelic level", default=lambda info: info.get(
        "adelic_level") or info.get("adelic_index") or info.get("adelic_genus")),
    MathCol("adelic_index", "ec.galois_rep", "Adelic index", default=lambda info: info.get(
        "adelic_level") or info.get("adelic_index") or info.get("adelic_genus")),
    MathCol("adelic_genus", "ec.galois_rep", "Adelic genus", default=lambda info: info.get(
        "adelic_level") or info.get("adelic_index") or info.get("adelic_genus")),
    ProcessedCol(
        "regulator",
        "ec.regulator",
    ...
    MathCol(
        ...
        short_title="Weierstrass coeffs",
        align="left",
        default=False),
    ProcessedCol(
        "equation",
        ...
        latex_equation,
        short_title="Weierstrass equation",
        align="left",
        orig="ainvs",
        download_col="ainvs"),
...
class ECDownloader(Downloader):
    ...
    def postprocess(self, row, info, query):
        ...
        if "mwgens" in info.get("showcol", "").split("."):
            gens = db.ec_mwbsd.lucky(
                {"lmfdb_label": row["lmfdb_label"]}, "gens")
            if gens is not None:
...
def elliptic_curve_search(info, query):
    ...
    if info.get('conductor_type'):
        if info['conductor_type'] == 'prime':
            ...
            if not isinstance(query.get('conductor'), int):
                ...
                query['conductor'] = {
                    '$in': integer_divisors(ZZ(query['conductor']))}
            else:
    ...
    parse_element_of(
        ...
        'isogeny_degrees',
        split_interval=200,
        contained_in=get_stats().isogeny_degrees)
    ...
    if info.get('galois_image'):
        ...
        elladic_labels = [a for a in labels if elladic_image_label_regex.fullmatch(
            a) and is_prime_power(elladic_image_label_regex.match(a)[1])]
        modell_labels = [a for a in labels if modell_image_label_regex.fullmatch(
            a) and is_prime(modell_image_label_regex.match(a)[1])]
        modm_labels = [
            a for a in labels if modm_image_label_regex.fullmatch(a)]
        if len(elladic_labels) + len(modell_labels) + \
                len(modm_labels) != len(labels):
        ...
        if 'cm' not in query:
            ...
            info['cm'] = "noCM"
        if query['cm']:
            # try to help the user out if they specify the normalizer of a
            # Cartan in the CM case (these are either maximal or impossible
            if any(a.endswith("Nn") for a in modell_labels) or any(
                    a.endswith("Ns") for a in modell_labels):
                err = "To search for maximal images, exclude non-maximal primes"
                flash_error(err)
                raise ValueError(err)
        else:
            # if the user specifies full mod-ell image with ell > 3,
            # automatically exclude nonmax primes (if possible)
            max_labels = [a for a in modell_labels if a.endswith(
                "G") and int(modell_image_label_regex.match(a)[1]) > 3]
            if max_labels:
                if info.get(
                        'nonmax_primes') and info['nonmax_quantifier'] != 'exclude':
                    err = "To search for maximal images, exclude non-maximal primes"
                    flash_error(err)
                    raise ValueError(err)
                else:
                    modell_labels = [
                        a for a in modell_labels if a not in max_labels]
                    max_primes = [
                        modell_image_label_regex.match(a)[1] for a in max_labels]
                    if info.get('nonmax_primes'):
                        max_primes += [l.strip() for l in info['nonmax_primes'].split(',')
                                       if l.strip() not in max_primes]
                    max_primes.sort(key=int)
                    info['nonmax_primes'] = ','.join(max_primes)
                    info['nonmax_quantifier'] = 'exclude'
                    parse_primes(info, query, 'nonmax_primes', name='non-maximal primes',
                                 qfield='nonmax_primes', mode=info.get('nonmax_quantifier'), radical='nonmax_rad')
                    info['galois_image'] = ','.join(
                        modell_labels + elladic_labels)
                query['modell_images'] = {'$contains': modell_labels}
    ...
    info['curve_url_LMFDB'] = lambda dbc: url_for(
        ".by_triple_label",
        conductor=dbc['conductor'],
        iso_label=split_lmfdb_label(
            dbc['lmfdb_iso'])[1],
        number=dbc['lmfdb_number'])
    info['iso_url_LMFDB'] = lambda dbc: url_for(
        ".by_double_iso_label",
        conductor=dbc['conductor'],
        iso_label=split_lmfdb_label(
            dbc['lmfdb_iso'])[1])
    info['cremona_bound'] = CREMONA_BOUND
    info['curve_url_Cremona'] = lambda dbc: url_for(
        ".by_ec_label", label=dbc['Clabel'])
    info['iso_url_Cremona'] = lambda dbc: url_for(
        ".by_ec_label", label=dbc['Ciso'])
...
def by_ec_label(label):
    ...
    try:
        ...
        if number:
            return redirect(url_for(".by_triple_label",
                                    conductor=N, iso_label=iso, number=number))
        else:
            return redirect(url_for(".by_double_iso_label",
                                    conductor=N, iso_label=iso))
...
def by_weierstrass(eqn):
    ...
    if label is None:
        N = E.conductor()
        return elliptic_curve_jump_error(
            eqn, {'conductor': N}, missing_curve=True)
...
def render_isogeny_class(iso_class):
    ...
    class_data.modform_display = url_for(
        ".modular_form_display",
        label=class_data.lmfdb_iso + "1",
        number="")
    learnmore_isog_picture = (
        'Picture description',
        url_for(".isog_picture_page"))
...
def render_curve_webpage_by_label(label):
    ...
    code['show'] = {
        ...
        'pari': '',
        'sage': '',
        'oscar': ''}  # use default show names
    learnmore_curve_picture = (
        'Picture description',
        url_for(".curve_picture_page"))
    T = render_template("ec-curve.html",
                        properties=data.properties,
                        data=data,
                        # set default show names but actually code snippets are
...
def EC_data(label):
    ...
    if match_lmfdb_label(label):
        ...
        if not number:  # isogeny class
            return datapage(label, ["ec_classdata", "ec_padic", "ec_curvedata"], title=f"Elliptic curve isogeny class data - {label}", bread=bread, label_cols=[
                            "lmfdb_iso", "lmfdb_iso", "lmfdb_iso"], sorts=[[], ["p"], ['conductor', 'iso_nlabel', 'lmfdb_number']])
        ...
        sorts = [[], [], [], [], ["degree", "field"],
                 ["prime"], ["prime"], ["p"]]
        return datapage(labels, ["ec_curvedata", "ec_classdata", "ec_mwbsd", "ec_iwasawa", "ec_torsion_growth", "ec_localdata",
                                 "ec_galrep", "ec_padic"], title=f"Elliptic curve data - {label}", bread=bread, label_cols=label_cols, sorts=sorts)
...
def padic_data(label, p):
    try:
        ...
        return abort(404)
    info = {'p': p}
    if db.ec_curvedata.lookup(
            label, label_col='lmfdb_label', projection="rank") == 0:
        info['reg'] = 1
    elif number == '1':
...
def download_EC_qexp(label, limit):
    ...
    if ainvs is None:
        return elliptic_curve_jump_error(label, {})
    if limit > 100000:
        return redirect(url_for('.download_EC_qexp',
                                label=label, limit=10000), 301)
    E = EllipticCurve(ainvs)
    response = make_response(
        ','.join(
            str(an) for an in E.anlist(
                int(limit),
                python_ints=True)))
...
def download_EC_all(label):
    ...
    if number:
        ...
        data_list = list(db.ec_curvedata.search(
            {'lmfdb_iso': label}, sort=['lmfdb_number']))
        if not data_list:
            return elliptic_curve_jump_error(label, {})
...
def render_congruent_number_data():
    ...
    if 'lookup' in info:
        return redirect(
            url_for(".render_single_congruent_number", n=info['lookup']))
    ...
    if 'filename' in info:
        filepath = os.path.join(
            congruent_number_data_directory,
            info['filename'])
    ...
    return render_template("congruent_number_data.html",
                           info=info, title=t, bread=bread, learnmore=learnmore)
...
def render_single_congruent_number(n):
    ...
    bread = get_bread() + [("Congruent numbers",
                            url_for(".render_congruent_number_data")), (n, "")]
    return render_template("single_congruent_number.html", info=info,
                           title=t, bread=bread, learnmore=learnmore_list())
...
def render_sw_ecdb():
    ...
    if 'Fetch' in info:
        ...
        if 'k' in info and not errors:
            ...
            if k.isdigit():
                ...
                errors.append(
                    f"k must be a nonnegative integer at most {kmax}")
        if not errors:
    ...
    return render_template("sw_ecdb.html", info=info,
                           comma=comma, title=t, bread=bread, learnmore=learnmore)
...
def render_bhkssw():
    ...
    return render_template("bhkssw.html", info=info, comma=comma,
...
def tor_struct_search_Q(prefill="any"):
    ...
    return "\n".join(["<select name='torsion_structure', style='width: 155px'>"] +
                     ["<option value={}>{}</option>".format(a, b) for a, b in gps] + ["</select>"])
...
def modm_reduce():
    ...
    galois_image = db.ec_galrep.lucky(
        {"lmfdb_label": label, "prime": 0}, "adelic_gens")
...
# the following allows the preceding function to be used in any template
# via {{...}}
...
class ECSearchArray(SearchArray):
    ...
    sorts = [("", "conductor", ["conductor", "iso_nlabel", "lmfdb_number"]),
             ...
             ("rank", "rank", ["rank", "conductor",
                               "iso_nlabel", "lmfdb_number"]),
             ("torsion",
              "torsion",
              ["torsion",
               "conductor",
               "iso_nlabel",
               "lmfdb_number"]),
             ("cm_discriminant", "CM discriminant", [
                 ("cm", -1), "conductor", "iso_nlabel", "lmfdb_number"]),
             ("regulator", "regulator", [
                 "regulator", "conductor", "iso_nlabel", "lmfdb_number"]),
             ("sha", "analytic &#1064;", [
                 "sha", "conductor", "iso_nlabel", "lmfdb_number"]),
             ("class_size", "isogeny class size", [
                 "class_size", "conductor", "iso_nlabel", "lmfdb_number"]),
             ("class_deg", "isogeny class degree", [
                 "class_deg", "conductor", "iso_nlabel", "lmfdb_number"]),
             ("num_int_pts", "integral points", [
                 "num_int_pts", "conductor", "iso_nlabel", "lmfdb_number"]),
             ("degree", "modular degree", [
                 "degree", "conductor", "iso_nlabel", "lmfdb_number"]),
             ("adelic_level", "adelic level", [
                 "adelic_level", "adelic_index", "adelic_genus"]),
             ("adelic_index", "adelic index", [
                 "adelic_index", "adelic_level", "adelic_genus"]),
             ("adelic_genus", "adelic genus", [
                 "adelic_genus", "adelic_level", "adelic_index"]),
             ("faltings_height", "Faltings height", [
                 "faltings_height", "conductor", "iso_nlabel", "lmfdb_number"]),
             ("abc_quality", "$abc$ quality", [
                 "abc_quality", "conductor", "iso_nlabel", "lmfdb_number"]),
    ...
    def __init__(self):
        ...
        cm_opts = ([('', ''), ('noCM', 'no potential CM'), ('CM', 'potential CM')]
                   + [('-4,-16', 'CM field Q(sqrt(-1))'), ('-3,-12,-27',
                                                           'CM field Q(sqrt(-3))'), ('-7,-28', 'CM field Q(sqrt(-7))')]
        ...
        bad_primes = TextBoxWithSelect(
            name="bad_primes",
            label="Bad primes $p$&emsp;",
            # trailing &emsp; prevents caption moving when toggling advanced
            # search opts
            short_label=r"Bad$\ p$",
