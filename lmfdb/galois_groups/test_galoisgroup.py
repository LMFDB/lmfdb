...
class GalGpTest(LmfdbTest):
    ...
    def test_search_deg(self):
        L = self.tc.get(
            '/GaloisGroup/?start=0&parity=0&cyc=0&solv=0&prim=0&n=7&t=&count=50')
        assert '7 matches' in L.get_data(as_text=True)

    def test_search_t_solv_prim(self):
        L = self.tc.get(
            '/GaloisGroup/?start=0&parity=-1&cyc=0&solv=1&prim=-1&n=&t=18&count=50')
    ...
    def test_search_range(self):
        L = self.tc.get(
            'GaloisGroup/?start=0&parity=0&cyc=0&solv=0&prim=0&n=8-11&t=3-5&count=50')
    ...
    def test_bad_jump(self):
        ...
        assert 'not a valid label for a Galois group' in L.get_data(
            as_text=True)
