...
### of listing the a(n)'s rather than the a(n)/sqrt(n)'s."""
    ...
    if hasattr(L, 'normalize_by'):
        ...
        thefile += "### Normalize, below, the n-th Dirichlet coefficient by n^(" +str(L.normalize_by) + ")\n\n"
    else:
...
def createLcalcfile_ver1(L):
    ...
    for n in range(0,len(L.poles)):
        ...
        thefile += str(real_part(L.residues[n])) + " " + str(imag_part(L.residues[n])) + "\n" #residue at pole
    ...
    for n in range(0,len(L.dirichlet_coefficients)):
        ...
        if not L.selfdual:  # if not selfdual
            thefile += " " + str(imag_part(L.dirichlet_coefficients[n]))   # add imaginary part of Dirichlet coefficient
        thefile += "\n"
