...
def set_info_for_start_page():
    ...
    tt = [[{'title':'Riemann zeta function','link': url_for('.l_function_riemann_page')},
          ...
          [ {'title':'', 'link': ''},
            {'title':'GL3 Maass form', 'link': url_for('.l_function_maass_gln_browse_page', degree= 'degree3')},
          ...
          [{'title':'GSp4 Maass form', 'link': url_for('.l_function_maass_gln_browse_page', degree= 'degree4') + '#GSp4_Q_Maass'},
           {'title':'GL4 Maass form', 'link': url_for('.l_function_maass_gln_browse_page', degree= 'degree4')},
...
def initLfunction(L, args, request):
    ...
    if L.Ltype() == 'maass':
        ...
        info['friends'].append(('Symmetric square L-function', url_for(".l_function_ec_sym_page", power='2', label=label)))
        info['friends'].append(('Symmetric cube L-function', url_for(".l_function_ec_sym_page", power='3', label=label)))
...
def processSymPowerEllipticCurveNavigation(startCond, endCond, power):
    ...
    for label in iso_list:
        ...
        counter += 1
        s += '<td><a href="' + url_for('.l_function_ec_sym_page', power = str(power), label = label)+ '">%s</a></td>\n' % label
