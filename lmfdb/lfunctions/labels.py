...
def make_label(L):
    ...
    rs = ''.join(['r' + str(round_to_half_str(elt.real())) for elt in GR])
    cs = ''.join(['c' + str(round_to_half_str(elt.real())) for elt in GC])
    ...
    beginning = "{}.{}.{}".format(L['degree'], conductor,
L['central_character'])
    gammas = "-" + rs + cs + "-"
    if L['algebraic']:
        end = "0"
...
def break_label(label):
    ...
    L = {'degree':int(deg),'conductor': cond,'central_character': ".".join([char_mod, char_index])}
    GR = []
    GC = []
...
L = {'algebraic': False, 'central_character': '1.1',  'conductor': 4,  'degree': 3,  'gamma_factors': [[u'18.960155*I', u'1.76101718*I', u'-20.721172*I'], []]}
label =  make_label(L)
from_label =  break_label(make_label(L))
print L
print label
print from_label