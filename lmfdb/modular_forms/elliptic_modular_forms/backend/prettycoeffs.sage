...
def Modf_changevar(f,Bfacto=10^6):
    ######
    # Usage : f a hecke_orbit
    ...
    if Klabel:
        ...
        if len(gp.nfcertify(pKP)):
            ...
            for s in faD:
                p=int(s.split(',')[0].replace(' ',''))
                if p<Bfacto:
                    ram.append(p)
            print ram
            subquery = {}
            if ram:
                subquery['$contains'] = ram
            if ur:
                subquery['$notcontains'] = ur
            if ram or ur:
                query['ramps'] = subquery
        else:
            ...
            query['disc_abs']=D
        print "querying"
        LK=list(db.nf_fields.search(query))
        print len(LK),"matches"
        Klabel=''
        for K in LK:
