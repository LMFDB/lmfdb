...
class EmfTest(LmfdbTest):
    ...
    def test_gamma0_pages(self):
        ...
        wmax = 40; Nmax = 25
        data = spaces.find({'weight': {'$gte':int(2), '$lt':int(wmax+1)},'level':{'$lt':int(Nmax+1)},'character':int(1),'version':float(version)})
    ...
    def test_gamma1_pages(self):
        ...
        spaces = getDBConnection().modularforms2.webmodformspace
        wmax = 10; Nmax = 50
        data = spaces.find({'weight':{'$gte': int(2), '$lt':int(wmax+1)}, 'level':{'$lt':int(Nmax+1)},'version':float(version)})
