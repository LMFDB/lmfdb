...
def decimal_pretty(s, min_chars_before_decimal=1, max_chars_after_decimal=6,
    ...
    n = min(min(max_chars, len(s)), min_chars_before_decimal +
            max_chars_after_decimal + 1)
    # truncate for the moment (as we do elsewhere in the LMFDB, revisit if we
    # switch to rounding
    return s[:n]
...
def min_eqns_pretty(fh):
    ...
    lhs = yR([0, polys[1], 1])
    slist.append(str(lhs).replace("*", "") + " = " +
                 str(polys[0]).replace("*", ""))
    slist.append("y^2 = " +
                 str(xR(simplify_hyperelliptic(fh))).replace("*", ""))
    return slist
...
def url_for_ec_class(ec_label):
    ...
    return url_for('ecnf.show_ecnf_isoclass', nf=nf,
                   conductor_label=cond, class_label=isog)
...
def ring_pretty(L, f):
    ...
    if f % 2 == 0:
        return r'\Z [' + (str(f // 2) if f != 2 else "") + \
            r'\sqrt{' + str(D) + r'}]'
...
def eqn_list_to_curve_plot(L, rat_pts):
    ...
    X0 = [real(z[0]) for z in g.base_extend(CC).roots()] + [real(z[0])
                                                            for z in g.derivative().base_extend(CC).roots()]
...
def end_statement(factorsQQ, factorsRR, field='', ring=None):
    ...
    # endomorphism ring is an invariant of the curve but not the isogeny
    # class, so we make it optional
    if ring:
        ...
        if ring[0] == 1:
            ...
            # and we can prettify them
                ...
                statement += r'\(' + r' \times '.join(ring_pretty(
                    factorQQ[1], 1) for factorQQ in factorsQQ) + r'\)'
        ...
        # can describe the maximal order here
            ...
            statement += r"an order of index \(%s\) in \(%s\)" % (
                ring[0], r' \times '.join(ring_pretty(factorQQ[1], 1) for factorQQ in factorsQQ))
        # End of first row:
        statement += "</td></tr>"

    # Second row: description of endomorphism algebra factors (this is the
    # first row if ring=None)
    ...
    if factorsQQ_number == 1:
        ...
        if factorsQQ[0][2] == -1:
            # Prettify if labels available, otherwise return defining
            # polynomial:
            if factorsQQ_pretty:
                statement += "<a href=%s>%s</a>" % (
                    url_for("number_fields.by_label", label=factorsQQ[0][0]), factorsQQ_pretty[0])
            else:
        ...
        # labels and full prettification always available:
            ...
            statement += r"\(\mathrm{M}_2(\)<a href=%s>%s</a>\()\)" % (
                url_for("number_fields.by_label", label=factorsQQ[0][0]), factorsQQ_pretty[0])
    ...
    # Third row: description of algebra tensored with RR (this is the second
    # row if ring=None)
...
def end_lattice_statement(lattice):
    ...
    for ED in lattice:
        ...
        statement += "&nbsp;&nbsp;Sato Tate group: %s" % st_display_knowl(
            convert_label(ED[4]))
        statement += "<br>&nbsp;&nbsp;"
...
def split_statement(coeffs, labels, condnorms):
    ...
    for n in range(len(coeffs)):
        ...
        if label:
            statement += "<br>&nbsp;&nbsp;Elliptic curve isogeny class <a href=%s>%s</a>" % (
                url_for_ec_class(label), ec_label_class(label))
        # Otherwise give defining equation:
        else:
            statement += r"<br>&nbsp;&nbsp;\(y^2 = x^3 - g_4 / 48 x - g_6 / 864\) with"
            statement += r"<br>&nbsp;&nbsp;\(g_4 = %s\)<br>&nbsp;&nbsp;\(g_6 = %s\)" % tuple(
                strlist_to_nfelt(x, 'b') for x in coeffs[n])
...
def gsp4_subgroup_data(label):
    ...
    def row_wrap(
        ...
        val): return "<tr><td>%s: </td><td>%s</td></tr>\n" % (cap,
                                                              val)
    def matrix(m): return r'$\begin{bmatrix}%s&%s&%s&%s\\%s&%s&%s&%s\\%s&%s&%s&%s\\%s&%s&%s&%s\end{bmatrix}$' % (
    ...
    e = N.valuation(ell)
    if e == 1:
        info += row_wrap("(%s,%s)-isogeny field degree" % (ell, ell),
        ...
        fulltorsflddeg = ell**4 * (ell**4 - 1) * \
            (ell**2 - 1) * (ell - 1) // data['index']
        info += row_wrap("Full %s-torsion field degree" %
                         (ell), fulltorsflddeg)
    info += "</table>\n"
    return info
...
# add new friend to list of friends, but only if really new (don't add an
# elliptic curve and its isogeny class)
def add_friend(friends, friend):
    for oldfriend in friends:
        if oldfriend[0] == friend[0] or oldfriend[1] in friend[1] or friend[1] in oldfriend[1]:
            return
        # compare again with slashes converted to dots to deal with minor
        # differences in url/label formatting
...
def th_wrap(kwl, title, colspan=1):
    if kwl:
        if colspan > 1:
            return ' <th colspan=%s>%s</th>' % (colspan,
                                                display_knowl(kwl, title=title))
        else:
...
def mw_gens_table(invs, gens, hts, pts,
                  comp=PolynomialRing(QQ, ['x', 'y', 'z'])('y')):
    def divisor_data(P):
        ...
        xD = sum([ZZ(xden) * ZZ(xP[i][0]) / ZZ(xP[i][1]) * x **
                  i * z**(len(xP) - i - 1) for i in range(len(xP))])
        if str(xD.factor())[:4] == "(-1)":
            xD = -xD
        yD = sum([ZZ(yden) * ZZ(yP[i][0]) / ZZ(yP[i][1]) * x **
                  i * z**(len(yP) - i - 1) for i in range(len(yP))])
        yD = comp(x, yD, z)
        return [make_bigint(elt, 10) for elt in [str(xD.factor()).replace("**", "^").replace("*", ""), str(
            yden) + "y" if yden > 1 else "y", str(yD).replace("**", "^").replace("*", "")]], xD, yD, yden
    if not invs:
        return ''
    gentab = ['<table class="ntdata">', '<thead>', '<tr>',
              th_wrap('g2c.mw_generator', 'Generator'),
              th_wrap(
                  'g2c.mw_generator', '$D_0$'), '<th></th>', '<th></th>', '<th></th>', '<th></th>', '<th></th>',
    ...
    for i in range(len(invs)):
        ...
        D0 = [
            P for P in pts if P[2] and xD(
                P[0],
                P[2]) == 0 and yD(
                P[0],
                P[2]) == yden *
            P[1]]
        Dinf = [P for P in pts if P[2] == 0 and not (
            xD(P[0], P[2]) == 0 and yD(P[0], P[2]) == yden * P[1])]
        div = (
            r'2 \cdot' +
            point_string(
                D0[0]) if len(D0) == 1 and len(Dinf) != 1 else ' + '.join(
                point_string(P) for P in D0)) if D0 else 'D_0'
        div += ' - '
        div += (r'2 \cdot' + point_string(Dinf[0]) if len(Dinf) == 1 and len(
            D0) != 1 else ' - '.join(point_string(P) for P in Dinf)) if Dinf else r'D_\infty'
...
def local_table(N, D, tama, bad_lpolys, cluster_pics):
    ...
    for p in integer_prime_divisors(D):
        ...
        if Cluslist:
            ClusThmb = '<img src="' + \
                Cluslist[0][2] + \
                '" height=19 style="position: relative; top: 50%; transform: translateY(10%);" />'
            Clus = cp_display_knowl(Cluslist[0][1], img=ClusThmb)
        else:
            Clus = ''
        loctab.extend([td_wrapr(p), td_wrapc(N.ord(p)), td_wrapc(
            D.ord(p)), td_wrapc(cp), td_wrapl(Lp), td_wrapcn(Clus)])
        loctab.append('  </tr>')
...
def galrep_table(galrep, torsion_order):
    ...
    for i in range(len(galrep)):
        ...
        if modellimage_lbl == 'not computed':
            ...
            modellimg = display_knowl(
                'gsp4.subgroup_data',
                title=modellimage_lbl,
                kwargs={
                    'label': modellimage_lbl})
        galtab.extend([td_wrapc(p), td_wrapcn(
            modellimg), td_wrapcn(is_torsion)])
        galtab.append('  </tr>')
...
def ratpts_table(pts, pts_v):
    def sorted_points(pts):
        return sorted(pts, key=lambda P: (
            max([abs(x) for x in P]), sum([abs(x) for x in P])))
    if len(pts) > 1:
        # always put points at infinity first, regardless of height
        pts = sorted_points([P for P in pts if P[2] == 0]) + \
            sorted_points([P for P in pts if P[2] != 0])
    kid = 'g2c.all_rational_points' if pts_v else 'g2c.known_rational_points'
    if len(pts) == 0:
        if pts_v:
            return 'This curve has no %s.' % display_knowl(
                kid, 'rational points')
        else:
            return 'No %s for this curve.' % display_knowl(
    ...
    if len(pts) <= tabcols + 1:
        return r'%s: \(%s\)' % (display_knowl(
            kid, caption), r',\, '.join(spts))
    ptstab = [
        '<table class="ntdata">',
        '<thead>',
        '<tr>',
    ...
    for i in range(0, len(pts), 6):
        ...
        if i + 6 > len(pts):
            # pad last line
            ptstab.extend(['<td></td>' for i in range(i + 6 - len(pts))])
        ptstab.append('</tr>')
...
class WebG2C():
    ...
    def __init__(self, curve, endo, tama, ratpts,
                 clus, galrep, nonsurj, is_curve=True):
        self.make_object(
            curve,
            endo,
            tama,
    ...
    def by_label(label):
        ...
        if not curve:
            if len(slabel) == 2:
                ...
                raise KeyError(
                    "Genus 2 curve %s not found in database." %
                    label)
        ...
        for x in tama:
            if x['p'] != 2:
                try:
                    clusentry = db.cluster_pictures.lucky(
                    ...
                    g2c_logger.error(
                        "Cluster picture data for genus 2 curve %s not found in database." %
                        label)
                    raise KeyError(
                        "Cluster picture data for genus 2 curve %s not found in database." %
                        label)
        nonsurj = curve.get('non_maximal_primes')
        galrep = list(db.g2c_galrep.search(
            {'lmfdb_label': curve['label']}, ['prime', 'modell_image']))
        galrep = augment_galrep_and_nonsurj(galrep, nonsurj)
        return WebG2C(curve, endo, tama, ratpts, clus, galrep,
                      nonsurj, is_curve=(len(slabel) == 4))

    def make_object(self, curve, endo, tama, ratpts,
                    clus, galrep, nonsurj, is_curve):
        ...
        data['lfunc_url'] = url_for(
            "l_functions.l_function_genus2_page",
            cond=data['slabel'][0],
            x=data['slabel'][1])
        data['bad_lfactors'] = literal_eval(curve['bad_lfactors'])
        data['bad_lfactors_pretty'] = [
            (c[0], list_to_factored_poly_otherorder(
                c[1])) for c in data['bad_lfactors']]
        if is_curve:
            ...
            data['disc_factor_latex'] = web_latex_factored_integer(
                data['disc'])
            data['igusa_clebsch'] = [
                ZZ(a) for a in literal_eval(
                    curve['igusa_clebsch_inv'])]
            ...
            data['igusa_clebsch_factor_latex'] = [
                web_latex_factored_integer(i) for i in data['igusa_clebsch']]
            data['igusa_factor_latex'] = [
                web_latex_factored_integer(j) for j in data['igusa']]
            data['aut_grp'] = abstract_group_display_knowl(
                curve['aut_grp_label'], f"${curve['aut_grp_tex']}$")
            data['geom_aut_grp'] = abstract_group_display_knowl(
                curve['geom_aut_grp_label'], f"${curve['geom_aut_grp_tex']}$")
            ...
            data['regulator'] = decimal_pretty(
                str(curve['regulator'])) if curve['regulator'] > -0.5 else 'unknown'
            if data['mw_rank'] == 0 and data['mw_rank_proved']:
                data['regulator'] = '1'  # display an exact 1 when we know this

            data['tamagawa_product'] = ZZ(
                curve['tamagawa_product']) if curve.get('tamagawa_product') else 0
            data['analytic_sha'] = ZZ(
                curve['analytic_sha']) if curve.get('analytic_sha') else 0
            data['leading_coeff'] = decimal_pretty(
                str(curve['leading_coeff'])) if curve['leading_coeff'] else 'unknown'

            data['rat_pts'] = ratpts['rat_pts']
            data['rat_pts_v'] = ratpts['rat_pts_v']
            data['rat_pts_table'] = ratpts_table(
                ratpts['rat_pts'], ratpts['rat_pts_v'])
            data['rat_pts_simple_table'] = ratpts_simpletable(
                ratpts['rat_pts'], ratpts['rat_pts_v'], data['min_eqn'])

            data['mw_gens_v'] = ratpts['mw_gens_v']
            lower = len([n for n in ratpts['mw_invs'] if n == 0])
            upper = data['analytic_rank']
            invs = ratpts['mw_invs'] if data['mw_gens_v'] or lower >= upper else [
                0 for n in range(upper - lower)] + ratpts['mw_invs']
            if len(invs) == 0:
                data['mw_group'] = 'trivial'
            else:
                data['mw_group'] = r'\(' + r' \oplus '.join(
                    (r'\Z' if n == 0 else r'\Z/{%s}\Z' % n) for n in invs) + r'\)'
            if lower >= upper:
                data['mw_gens_table'] = mw_gens_table(
                    ratpts['mw_invs'],
                    ratpts['mw_gens'],
                    ratpts['mw_heights'],
                    ratpts['rat_pts'])
                data['mw_gens_simple_table'] = mw_gens_simple_table(
                    ratpts['mw_invs'],
                    ratpts['mw_gens'],
                    ratpts['mw_heights'],
                    ratpts['rat_pts'],
            ...
            if curve['two_torsion_field'][0]:
                data['two_torsion_field_knowl'] = nf_display_knowl(
                    curve['two_torsion_field'][0], field_pretty(
                        curve['two_torsion_field'][0]))
            else:
                t = curve['two_torsion_field']
                data['two_torsion_field_knowl'] = r"splitting field of \(%s\) with Galois group %s" % (
                    intlist_to_poly(t[1]), transitive_group_display_knowl(f"{t[2][0]}T{t[2][1]}"))
            ...
            self.downloads = [  # ('Frobenius eigenvalues to text', url_for(".download_G2C_fouriercoeffs", label=self.lmfdb_label, limit=1000)),
                ('All stored data to text', url_for(
                    ".download_G2C_all", label=lmfdb_label)),
                ('Code to Magma',
                 url_for(".g2c_code_download",
                         conductor=cond,
                         iso=alpha,
                         discriminant=disc,
                         number=num,
            ...
            curves_data = list(db.g2c_curves.search(
                {"class": curve['class']}, ['label', 'eqn']))
            if not curves_data:
                raise KeyError(
                    "No curves found in database for isogeny class %s of genus 2 curve %s." %
                    (curve['class'], curve['label']))
            data['curves'] = [{"label": c['label'], "equation_formatted": min_eqn_pretty(
                literal_eval(c['eqn'])), "url": url_for_curve_label(c['label'])} for c in curves_data]
            ...
            if lfunc_data and lfunc_data.get('euler_factors'):
                data['good_lfactors'] = [
                    [
                        nth_prime(
                            n +
                            1),
                        lfunc_data['euler_factors'][n]] for n in range(
                        len(
                            lfunc_data['euler_factors'])) if nth_prime(
                        n +
                        1) < 30 and (
                            data['cond'] %
                            nth_prime(
                ...
                data['good_lfactors_pretty'] = [
                    (c[0], list_to_factored_poly_otherorder(
                        c[1])) for c in data['good_lfactors']]
        ...
        # Endomorphism data over intermediate fields not already treated (only
        # for curves, not necessarily isogeny invariant):
        if is_curve:
        ...
        # Field over which the Jacobian decomposes (base field if Jacobian is
        ...
        data['split_field_statement'] = split_field_statement(
            data['is_simple_geom'], data['split_field_label'], data['split_field_poly'])
        ...
        if not data['is_simple_geom']:
            ...
            data['split_statement'] = split_statement(
                data['split_coeffs'], data.get('split_labels'), data['split_condnorms'])
        ...
        if 'split_labels' in data:
            for friend_label in data['split_labels']:
                if is_curve:
                    ecs.append(
                        ("Elliptic curve " + friend_label,
                         url_for_ec(friend_label)))
                else:
                    ecs.append(
                        ("Elliptic curve " +
                         ec_label_class(friend_label),
        ...
        for elt in db.lfunc_instances.search(
                {'Lhash': data['Lhash'], 'type': 'ECQP'}, 'url'):
        ...
        self.bread = bread = [
            ...
            (r'$\Q$', url_for(".index_Q")),
            ('%s' % data['slabel'][0], url_for(
                ".by_conductor", cond=data['slabel'][0])),
            ('%s' % data['slabel'][1],
        ...
        # Title
        self.title = "Genus 2 " + \
            ("curve " if is_curve else "isogeny class ") + data['label']
        ...
        if data['abs_disc'] % 4096 == 0:
            ...
            magma_cond_option = ''
        code['cond'] = {
            'magma': 'Conductor(LSeries(C%s)); Factorization($1);' % magma_cond_option}
        code['disc'] = {
        ...
        if ratpts:
            code['rat_pts'] = {
                'magma': '[' + ','.join(
                    "C![%s,%s,%s]" %
                    (p[0], p[1], p[2]) for p in ratpts['rat_pts']) + ']; // minimal model'}
            code['rat_pts_simp'] = {
                'magma': '[' + ','.join(
                    [
                        "C![%s,%s,%s]" %
                        (p[0], p[1], p[2]) for p in [
                            simplify_hyperelliptic_point(
                                data['min_eqn'], pt) for pt in ratpts['rat_pts']]]) + ']; // simplified model'}
