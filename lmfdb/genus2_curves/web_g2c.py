...
def endo_statement(factorsQQ, factorsRR, ring, fieldstring):
    ...
    if ring[0] == 1:
        ...
        if factorsQQ_number == 1:
            # Number field or not:
            if factorsQQ[0][2] == -1:
                # Use pretty if available:
                if len(factorsQQ[0][1]) in [2, 3]:
                    statement += """\(%s\)""" % ring_pretty(factorsQQ[0][1])
                else:
        ...
        # and we can prettify them
            ...
            statement += r'\(' + ' \\times '.join([ ring_pretty(factorQQ) for
    ...
    # Finally the most generic case:
        ...
        statement += "an order of index \(%s\) in \(\End (J_{%s}) \otimes \Q\)"""\
            % (ring[0], fieldstring)
    # End of first row:
    statement += """</td></tr>"""
