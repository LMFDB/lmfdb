...
def mw_gens_table(invs,gens,hts):
    ...
    for i in range(len(invs)):
        gentab.append('  <tr>')
        if invs[i] == 0:
            gentab.extend([td_wrapl(D[i]), td_wrapl("%.9f"%(hts[i])), td_wrapc('\\infty'), td_wrapc('\\Z')])
        else:
...
class WebG2C(object):
    ...
    def make_object(self, curve, endo, tama, ratpts, is_curve):
        ...
        if is_curve:
            ...
            data['real_period'] = "%.9f"%(curve['real_period'])
            if (curve['regulator'] > -0.5):
                data['regulator'] = "%.9f"%(curve['regulator'])
            else:
            ...
            data['leading_coeff'] = "%.9f"%(curve['leading_coeff']) if curve['leading_coeff'] else '\\text{unknown}'
            if ratpts:
