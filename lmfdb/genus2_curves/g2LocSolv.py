...
def IsSolvableAt(F,p): # Tests whether y²=F(x) has a Qp-point, assuming F integral of degree 6 and lc(F) squarefree
...
def NonSolublePlaces(f,h=0): # List of primes at which the curve y²+h(x)*y=f(x) is not solvable
