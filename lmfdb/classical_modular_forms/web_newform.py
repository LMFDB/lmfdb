    ...
    def downloads(self):
        ...
        if self.embedding_label is None:
            if self.hecke_cutters or self.has_exact_qexp:
                downloads.append(('Download modular form to Magma', url_for('.download_newform_to_magma', label=self.label)))
            if self.has_exact_qexp:
                downloads.append(('Download q-expansion to Sage', url_for('.download_qexp', label=self.label)))
            downloads.append(('Download trace form to text', url_for('.download_traces', label=self.label)))
            if self.has_complex_qexp:
                downloads.append(('Download embeddings to text', url_for('.download_cc_data', label=self.label)))
                downloads.append(('Download Satake angles to text', url_for('.download_satake_angles', label=self.label)))
            downloads.append(('Download all stored data to text', url_for('.download_newform', label=self.label)))
        else:
            downloads.append(('Download coefficient data to text', url_for('.download_embedded_newform', label='%s.%s'%(self.label, self.embedding_label))))
        return downloads
