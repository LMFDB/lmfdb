...
def set_info_funcs(info):
    ...
    info['download_spaces'] = lambda results: any(
        space['dim'] > 1 for space in results)
...
def index():
    ...
    if len(request.args) > 0:
        ...
        if search_type in ['List', '', 'Random']:
            ...
            if bad_keys:
                flash_error(
                    "%s invalid for searching spaces",
                    ", ".join(bad_keys))
...
def parse_n(info, newform, primes_only):
    ...
    try:
        ...
        if err.args and err.args[0] == 'Too many options':
            errs.append(
                r"Only \(a_n\) up to %s are available" %
                (newform.an_cc_bound))
        else:
            errs.append(
    ...
    if info['CC_n'] and max(info['CC_n']) > newform.an_cc_bound:
        errs.append(
            r"Only \(a_n\) up to %s are available; limiting to \(n \le %d\)" %
            (newform.an_cc_bound, newform.an_cc_bound))
        info['CC_n'] = [n for n in info['CC_n'] if n <= newform.an_cc_bound]

    if primes_only:
        info['CC_n'] = [n for n in info['CC_n'] if ZZ(
            n).is_prime() and newform.level % n != 0]
        if len(info['CC_n']) == 0:
            errs.append("No good primes within n range; resetting to default")
            info['CC_n'] = [
                n for n in prime_range(
                    maxp +
                    1) if newform.level %
                n != 0]
...
def parse_m(info, newform):
    ...
    if '.' in mrange:
        ...
        try:
            mrange = re.sub(
                r'\d+\.\d+',
                newform.embedding_from_embedding_label,
                mrange)
        except ValueError:
    ...
    try:
        ...
        if err.args and err.args[0] == 'Too many options':
            ...
            errs.append(
                "<span style='color:black'>Embeddings</span> must consist of integers or embedding codes")
...
def render_newform_webpage(label):
    ...
    return render_template("cmf_newform.html",
                           ...
                           bread=newform.bread,
                           learnmore=learnmore_list_add(
                               *learnmore_cmf_picture),
                           title=newform.title,
...
def mf_data(label):
    ...
    return datapage(labels, tables, title=title,
                    bread=bread, label_cols=label_cols)
...
def by_url_space_conreylabel(level, weight, conrey_index):
    label = convert_spacelabel_from_conrey(
        str(level) + "." + str(weight) + "." + str(conrey_index))
    if label is None:
...
def by_url_newform_conrey5(
        level, weight, char_orbit_label, hecke_orbit, embedding_label):
    ...
    return redirect(url_for("cmf.by_url_embedded_newform_label", level=level, weight=weight, char_orbit_label=char_orbit_label,
                            hecke_orbit=hecke_orbit, conrey_index=conrey_index, embedding=embedding), code=301)

# Embedded modular form


@cmf.route("/<int:level>/<int:weight>/<char_orbit_label>/<hecke_orbit>/<int:conrey_index>/<int:embedding>/")
def by_url_embedded_newform_label(
        level, weight, char_orbit_label, hecke_orbit, conrey_index, embedding):
    if conrey_index <= 0 or embedding <= 0:
        return abort(404, "Invalid embedding label: negative values")
    newform_label = ".".join(
        map(str, [level, weight, char_orbit_label, hecke_orbit]))
...
def jump_box(info):
    ...
    if re.match(r'^\#\d+$', jump) and ZZ(jump[1:]) < 2**61:
        label = db.mf_newforms.lucky(
            {'trace_hash': ZZ(jump[1:].strip())}, projection="label")
        if label:
...
def parse_character(inp, query, qfield, prim=False):
    # qfield will be set to something by the logic in SearchParser.__call__,
    # but we want it determined by prim
    if prim:
    ...
    if orbit.isalpha():
        ...
        if prim:
            ...
            conductor = db.char_dirichlet.lucky(
                {'modulus': level, 'orbit': orbit}, 'conductor')
            if conductor is None:
                raise ValueError("No character orbit with this label exists.")
            if conductor != level:
                raise ValueError(
                    "It has level %d and is thus not primitive." %
                    conductor)
        query[qfield] = orbit
    else:
        if prim:
        ...
        query['conrey_index'] = ConreyCharacter(
            modulus=level, number=orbit).min_conrey_conj
...
def newform_parse(info, query):
    ...
    parse_subset(
        ...
        query,
        'self_twist_discs',
        name="CM/RM discriminant",
        parse_singleton=parse_discriminant)
...
def newspace_parse(info, query):
    ...
    if 'dim' not in info and 'hst' not in info:
        # When coming from browse page, add dim condition to only show
        # non-empty spaces
        info['dim'] = '1-'
    ...
    if info['search_type'] != 'SpaceDimensions':
        ...
        if 'num_forms' not in query and info.get('all_spaces') != 'yes':
            # Don't show spaces that only include dimension data but no
            # newforms
            query['num_forms'] = {'$exists': True}


def _trace_col(i):
    return ProcessedCol("traces", None, rf"$a_{{{nth_prime(i+1)}}}$", lambda tdisp: bigint_knowl(
        tdisp[i], 12), orig="trace_display", align="right")


def _AL_col(i, p):
    return ProcessedCol("atkin_lehner", None, str(
        p), lambda evs: "+" if evs[i][1] == 1 else "-", orig="atkin_lehner_eigenvals", align="center", mathmode=True)
...
newform_columns = SearchColumns([
    ...
    MultiProcessedCol("character", "cmf.character", "Char",
                      ...
                      lambda level, orb: display_knowl(
                          'character.dirichlet.orbit_data', title=f"{level}.{orb}", kwargs={
                              "label": f"{level}.{orb}"}),
    ...
    MultiProcessedCol("prim", "character.dirichlet.primitive", "Prim",
                      ...
                      lambda cond, ind: display_knowl(
                          'character.dirichlet.orbit_data', title=f"{cond}.{num2letters(ind)}", kwargs={
                              "label": f"{cond}.{num2letters(ind)}"}),
    ...
    MathCol(
        ...
        "Dim",
        align="right",
        short_title="dimension"),
    MathCol(
        "relative_dim",
        "cmf.relative_dimension",
        "Rel. Dim",
        align="right",
        short_title="relative dimension",
        default=False),
    FloatCol(
        "analytic_conductor",
    ...
    MultiProcessedCol("field",
                      "cmf.coefficient_field",
                      "Field",
                      ["field_poly_root_of_unity",
                       "dim",
                       "field_poly_is_real_cyclotomic",
                       "nf_label",
                       "field_poly",
                       "field_disc_factorization"],
                      nf_link,
                      download_col="nf_label"),
    ProcessedCol("projective_image", "cmf.projective_image", "Image",
                 lambda img: (
                     '' if img == '?' else '$%s_{%s}$' % (img[:1], img[1:])),
                 contingent=lambda info: any(
                     mf.get('weight') == 1 for mf in info["results"]),
                 default=lambda info: all(
                     mf.get('weight') == 1 for mf in info["results"]),
                 align="center", short_title="projective image"),
    MultiProcessedCol("cm", "cmf.self_twist", "CM",
                      ["is_cm", "cm_discs"],
                      lambda is_cm, cm_discs: ", ".join(map(quad_field_knowl, cm_discs)) if is_cm else (
                          "None" if is_cm is False else "not computed"),
                      short_title="CM",
                      download_col="cm_discs"),
    MultiProcessedCol("rm", "cmf.self_twist", "RM",
                      ["is_rm", "rm_discs"],
                      lambda is_rm, rm_discs: ", ".join(
                          map(quad_field_knowl, rm_discs)) if is_rm else "None",
                      contingent=lambda info: any(
                          mf.get('weight') == 1 for mf in info["results"]),
                      short_title="RM",
    ...
    ColGroup("atkin_lehner", "cmf.atkin-lehner", "A-L signs",
             lambda info: [
                 _AL_col(
                     i, pair[0]) for i, pair in enumerate(
                     info["results"][0]["atkin_lehner_eigenvals"])],
    ...
    ['analytic_conductor',
     ...
     'field_poly_root_of_unity',
     'fricke_eigenval',
     'hecke_ring_index_factorization',
     'inner_twist_count',
     'is_cm',
     'is_largest',
     'is_maximal',
...
def trace_postprocess(res, info, query):
    if res:
        ...
        for rec in table.search({'n': {'$in': info['Tr_n']}, 'hecke_orbit_code': {
                                '$in': hecke_codes}}, projection=['hecke_orbit_code', 'n', 'trace_an'], sort=[]):
            if q:
                trace_dict[rec['hecke_orbit_code']
                           ][rec['n']] = (rec['trace_an'] % q)
            else:
...
def process_an_constraints(info, query, qfield='traces', nshift=None):
    ...
    if q:
        ...
        parse_equality_constraints(info, query, 'an_constraints', qfield=qfield,
                                   parse_singleton=(
                                       lambda x: {'$mod': [int(x), q]}),
                                   nshift=nshift)
    else:
        parse_equality_constraints(
            info, query, 'an_constraints', qfield=qfield)
        if info.get('view_modp') == 'reductions':
...
def set_rows_cols(info, query):
    ...
    if 'weight_parity' in query:
        if query['weight_parity'] == -1:
            info['weight_list'] = [
                k for k in info['weight_list'] if k %
                2 == 1]
        else:
            info['weight_list'] = [
                k for k in info['weight_list'] if k %
    ...
    if 'char_orbit_index' in query:
        ...
        try:
            info['level_list'] = integer_options(
                info['level'], max_opts=100000)
        except ValueError:
    ...
    if 'char_conductor' in query:
        info['level_list'] = [
            N for N in info['level_list'] if (
                N %
                query['char_conductor']) == 0]
    if info.get('prime_quantifier') == 'exactly':
        rad = query.get('level_radical')
        if rad:
            info['level_list'] = [
                N for N in info['level_list'] if ZZ(N).radical() == rad]
    else:
        primes = info.get('level_primes', '').strip()
        if primes:
            try:
                rad = prod(ZZ(p) for p in primes.split(','))
                if info.get('prime_quantifier') in [
                        'subset', 'subsets']:  # subsets for backward compat in urls
                    info['level_list'] = [
                        N for N in info['level_list'] if (
                            rad %
                            ZZ(N).radical()) == 0]
                # append for backward compat in urls
                elif info.get('prime_quantifier') in ['supset', 'append']:
                    info['level_list'] = [
                        N for N in info['level_list'] if (
                            N %
                            rad) == 0]
                    ...
                    info['level_list'] = [
                        N for N in info['level_list'] if gcd(
                            N, rad) == 1]
    ...
    if not info['level_list']:
        raise ValueError(
            "No valid levels consistent with specified characters and bad primes")
...
def dimension_common_postprocess(
        info, query, cusp_types, newness_types, url_generator, pick_table, switch_text=None):
...
def delete_false(D):
    for key, val in list(
            D.items()):  # for py3 compat: can't iterate over items while deleting
        if val is False:
            del D[key]
...
def dimension_space_postprocess(res, info, query):
    ...
    if 'char_orbit_index' in query or 'prim_orbit_index' in query:
        ...
        def url_generator(N, k):
            return url_for(".by_url_space_label", level=N,
                           weight=k, char_orbit_label="a")
    elif 'char_order' in query:
        url_generator = url_generator_list
    else:
        def url_generator(N, k):
            return url_for(".by_url_full_gammma1_space_label",
                           level=N, weight=k)
...
def dimension_form_postprocess(res, info, query):
    ...
    dim_dict = {}
    for rec in db.mf_newspaces.search(
            na_query, ['level', 'weight', 'num_forms']):
        N = rec['level']
        k = rec['weight']
...
space_columns = SearchColumns([
    ...
    MultiProcessedCol("character", "cmf.character", r"$\chi$", ["level", "conrey_index"],
                      lambda level, number: r'<a href="%s">\( \chi_{%s}(%s, \cdot) \)</a>' % (url_for(
                          "characters.render_Dirichletwebpage", modulus=level, number=number), level, number),
                      short_title="character"),
    MathCol(
        "char_order",
        "character.dirichlet.order",
        r"$\operatorname{ord}(\chi)$",
        short_title="character order"),
    ...
    MathCol(
        ...
        "cmf.galois_orbit",
        "Orbits",
        short_title="Galois orbits"),
    MultiProcessedCol("decomp",
                      "cmf.dim_decomposition",
                      "Decomposition",
                      ["level",
                       "weight",
                       "char_orbit_label",
...
def level_primes_formatter(x):
    ...
    if subset:
        return 'level_primes=%s&prime_quantifier=subset' % (
            ','.join(map(str, subset)))
    supset = x.get('$contains')
    if supset:
        return 'level_primes=%s&prime_quantifier=supset' % (
            ','.join(map(str, supset)))
    raise ValueError
...
class CMF_stats(StatsDisplay):
    ...
    def __init__(self):
        ...
        self.galois_orbit_knowl = display_knowl(
            'cmf.galois_orbit', title='Galois orbits')
    ...
    def short_summary(self):
        return r'The database currently contains %s (%s of) %s, and %s of their embeddings into the complex numbers. You can <a href="%s">browse further statistics</a> or <a href="%s">create your own</a>.' % (
            self.nforms, self.galois_orbit_knowl, self.newform_knowl, self.ndim, url_for(".statistics"), url_for(".dynamic_statistics"))

    @lazy_attribute
    def summary(self):
        return r"The database currently contains %s (%s of) %s in %s %s, and %s of their embeddings into the complex numbers.  In addition to the statistics below, you can also <a href='%s'>create your own</a>." % (
            self.nforms, self.galois_orbit_knowl, self.newform_knowl, self.nspaces, self.newspace_knowl, self.ndim, url_for(".dynamic_statistics"))
...
def statistics():
    ...
    return render_template("display_stats.html", info=CMF_stats(
    ), title=title, bread=get_bread(other='Statistics'), learnmore=learnmore_list())
...
def dynamic_statistics():
    ...
    return render_template("dynamic_stats.html", info=info, title=title, bread=get_bread(
        other='Dynamic Statistics'), learnmore=learnmore_list())
...
class CMFSearchArray(SearchArray):
    ...
    _sort = [
        ...
        ('prim', 'primitive character', [
         'char_conductor', 'prim_orbit_index', 'level', 'weight']),
        ('char_order', 'character order', [
         'char_order', 'level', 'char_orbit_index', 'weight']),
        ('Nk2', 'Nk^2', ['Nk2', 'level']),
        ('dim', 'dimension', ['dim', 'level', 'weight']),
        ('relative_dim', 'relative dimension',
         ['relative_dim', 'level', 'weight']),
        ('analytic_rank', 'analytic rank', [
         'analytic_rank', 'level', 'weight']),
        ('inner_twist_count', 'inner twist count',
         ['inner_twist_count', 'level', 'weight']),
        ('hecke_ring_index_factorization', 'coeff ring index',
         ['hecke_ring_index', 'level', 'weight']),
    ]
    for name, disp, sord in _sort:
        if 'char_orbit_index' not in sord:
            sord.append('char_orbit_index')
    _sort_spaces = _sort[:-3]
    _sort_forms = [(name, disp, sord + ['hecke_orbit'])
                   for name, disp, sord in _sort]
    sorts = {'': _sort_forms,
             'Traces': _sort_forms,
             'Spaces': _sort_spaces}
    ...
    def __init__(self):
        ...
        self.refine_array = [
            ...
            [level_primes, character, char_primitive,
                char_order, is_maximal_largest],
            [coefficient_field, self_twist, self_twist_discs,
