...
def get_bread(**kwds):
    # Should be called with either search=True or an initial segment of the
    # links below
...
def newform_search_link(text, title=None, **kwd):
    ...
    return "<a href='%s'%s>%s</a>" % (link,
                                      "" if title is None else " title='%s'" % title, text)
...
def ALdim_new_cusp_table(al_dims, level, weight):
    ...
    if num_primes > 1:
        header.append(
            r"<th class='center'>%s</th>" %
            (display_knowl(
                'cmf.fricke',
                title='Fricke').replace(
                '"',
                "'")))
    ...
    for i, dim in enumerate(al_dims):
        ...
        query = {'level': level, 'weight': weight, 'char_order': 1,
                 'atkin_lehner_string': "".join(map(url_sign_char, b))}
    ...
    if num_primes > 1:
        plus_knowl = display_knowl(
            'cmf.plus_space',
            title='Plus space').replace(
            '"',
            "'")
        plus_link = newform_search_link(
            r'\(%s\)' %
            fricke[0],
            level=level,
            weight=weight,
            char_order=1,
            fricke_eigenval=1)
        minus_knowl = display_knowl(
            'cmf.minus_space',
            title='Minus space').replace(
            '"',
            "'")
        minus_link = newform_search_link(
            r'\(%s\)' %
            fricke[1],
            level=level,
            weight=weight,
            ...
            1)
        rows.append(
            r"<tr><td colspan='%s'>%s</td><td class='center'>\(+\)</td><td>%s</td></tr>" %
            (num_primes, plus_knowl, plus_link))
        rows.append(
            r"<tr><td colspan='%s'>%s</td><td class='center'>\(-\)</td><td>%s</td></tr>" %
            (num_primes, minus_knowl, minus_link))
...
def ALdim_table(al_dims, level, weight):
    ...
    if num_primes > 1:
        header.append(
            r"<th rowspan=2 class='center'>%s</th>" %
            (display_knowl(
                'cmf.fricke',
                title='Fricke').replace(
                '"',
    ...
    if num_primes > 1:
        ...
        minus_knowl = display_knowl(
            ...
            '"',
            "'")
        plus_row = r"<tr><td colspan='%s'>%s</td><td class='center'>\(+\)</td>" % (
            num_primes, plus_knowl)
        minus_row = r"<tr><td colspan='%s'>%s</td><td class='center'>\(-\)</td>" % (
            num_primes, minus_knowl)
        ...
        for X in ['M', 'S', 'E']:
            ...
            for typ in ['all', 'new', 'old']:
                ...
                if (X == 'S') and (typ == 'new'):
                    plus_dim = newform_search_link(
                        plus_dim, level=level, weight=weight, char_order=1, fricke_eigenval=1)
                    minus_dim = newform_search_link(
                        minus_dim, level=level, weight=weight, char_order=1, fricke_eigenval=-1)
...
def common_latex(level, weight, conrey=None, S="S",
                 t=0, typ="", symbolic_chi=False):
...
def convert_spacelabel_from_conrey(spacelabel_conrey):
    ...
    try:
        return db.mf_newspaces.lucky({'conrey_index': ConreyCharacter(
            N, n).min_conrey_conj, 'level': N, 'weight': k}, projection='label')
...
def trace_expansion_generic(space, prec_max=10):
    ...
    # return web_latex(coeff_to_power_series([0] +
    # space.traces[:prec-1],prec=prec),enclose=True)
...
class DimGrid():
    ...
    def __add__(self, other):
        if isinstance(
                other, int) and other == 0:  # So that we can do sum(grids)
            return self
...
def new_lambda(r, s, p):
    # Formula worked out by Kevin Buzzard in
    # http://wwwf.imperial.ac.uk/~buzzard/maths/research/notes/dimension_of_spaces_of_eisenstein_series.pdf,
    # with one obvious typo corrected (2*s==r, p>2 case)
    assert r > 0 and s <= r
    if 2 * s > r:
        if r == s:
            return 2
...
def QDimensionNewEisensteinForms(chi, k):
    # The Q-dimension of the new subspace of E_k(N,chi), the space of
    # Eisenstein series of weight k, level N, and character chi, where N is
    # the modulus of chi.
    ...
    D = prod([new_lambda(N.valuation(p), M.valuation(p), p)
...
def make_newspace_data(level, char_data, k=2):
    # Makes the data needed for creating newspace pages in cases without a
    # corresponding entry in mf_newspaces
    data = {}
    ...
    data['cusp_dim'] = int(gp('mfdim([%i, %i, znchar(Mod(%i,%i))], 1)' % (level, k, data['conrey_index'], level))) * \
        char_data['degree']  # https://pari.math.u-bordeaux.fr/pub/pari/manuals/2.15.4/users.pdf  p.595
    data['dim'] = int(
        gp(
            'mfdim([%i, %i, znchar(Mod(%i,%i))], 0)' % (level,
                                                        k,
                                                        data['conrey_index'],
                                                        level))) * char_data['degree']  # mfdim returns the dimension over Q(chi), not over Q
    data['eis_dim'] = int(gp('mfdim([%i, %i, znchar(Mod(%i,%i))], 3)' % (
        level, k, data['conrey_index'], level))) * char_data['degree']
...
def make_oldspace_data(newspace_label, char_conductor, prim_orbit_index):
    # This creates enough of data to generate the oldspace decomposition on a
    # newspace page
    level = int(newspace_label.split('.')[0])
    weight = int(newspace_label.split('.')[1])
    sub_level_list = [
        sub_level for sub_level in divisors(level) if (
            sub_level %
            char_conductor == 0) and sub_level != level]
    sub_chars = {
        char['modulus']: char for char in db.char_dirichlet.search(
            {
                'modulus': {
                    '$in': sub_level_list},
                'conductor': char_conductor,
                'primitive_orbit': prim_orbit_index})}
    if weight == 1:
        newspace_dims = {rec['level']: rec['dim'] for rec in db.mf_newspaces.search({'weight': weight, '$or': [
                                                                                    {'level': sub_level, 'char_orbit_index': sub_chars[sub_level]['orbit']} for sub_level in sub_level_list]}, ['level', 'dim'])}
    oldspaces = []
    for sub_level in sub_level_list:
        ...
        # only include subspaces with positive dimension (computed on the fly
        # unless with weight is 1)
        if weight == 1:
            if newspace_dims[sub_level] > 0:
                oldspaces.append(entry)
        else:
            if int(gp('mfdim([%i, %i, znchar(Mod(%i,%i))], 0)' % (
                    sub_level, weight, entry['sub_conrey_index'], sub_level))) > 0:
                oldspaces.append(entry)
    return oldspaces
...
class WebNewformSpace():
    def __init__(self, data):
        ...
        self.has_projective_image_types = all(
            typ + '_dim' in data for typ in ('dihedral', 'a4', 's4', 'a5'))
        # The following can be removed once we change the behavior of lucky to
        # include Nones
        ...
        self.char_conrey_str = r'\chi_{%s}(%s,\cdot)' % (
            self.level, self.char_conrey)
        self.newforms = list(db.mf_newforms.search(
            {'space_label': self.label}, projection=2))
        oldspaces = make_oldspace_data(
            self.label,
            self.char_conductor,
            self.prim_orbit_index)
        self.oldspaces = [
            (old['sub_level'],
             old['sub_char_orbit_index'],
                old['sub_conrey_index'],
                old['sub_mult']) for old in oldspaces]
        self.dim_grid = DimGrid.from_db(data)
        self.plot = db.mf_newspace_portraits.lookup(
        ...
        self.properties += [
            ...
            ('Character field', r'$\Q%s$' %
             ('' if self.char_degree == 1 else r'(\zeta_{%s})' %
              self.char_order)),
            ...
            self.properties.append(
                ('Newform subspaces',
                 prop_int_pretty(
                     self.num_forms)))
        self.properties.append(
            ('Sturm bound', prop_int_pretty(
                self.sturm_bound)))
        if data.get('trace_bound') is not None:
            self.properties.append(
                ('Trace bound', prop_int_pretty(
                    self.trace_bound)))
        ...
        self.downloads = [
            ...
            ('All stored data to text', url_for(
                '.download_newspace', label=self.label)),
            ('Underlying data', url_for('.mf_data', label=self.label)) if self.__dict__.get(
                'has_mf_newspaces_entry', True) else ('No underlying data', None),
        ]
        ...
        gamma1_link = '/ModularForm/GL2/Q/holomorphic/%d/%d' % (
            self.level, self.weight)
        self.friends = [
            ('Newspace %d.%d' %
             (self.level, self.weight), gamma1_link)]
    ...
    def display_character(self):
        if self.char_order == 1:
            ...
            ord_deg = r" (of %s \(%d\) and %s \(%d\))" % (
                ord_knowl, self.char_order, deg_knowl, self.char_degree)
    ...
    def old_latex_symbolic(self):
        return common_latex(
            *(self._vec() + ["S", 0, "old"]), symbolic_chi=True)

    def subspace_latex(self, new=False):
        return common_latex("M", self.weight, self.conrey_index,
                            "S", 0, "new" if new else "", symbolic_chi=True)

    def oldspace_decomposition(self):
        # Returns a latex string giving the decomposition of the old part.
        # These come from levels M dividing N, with the conductor of the
        # character dividing M.
        ...
        return r"\(\oplus\)".join(template.format(old=common_latex(N, self.weight, conrey, typ="new"),
            ...
            level=N,
            weight=self.weight,
            char_orbit_label=cremona_letter_code(
                i - 1)),
            mult=mult)
    ...
    def ALdim_table(self):
        ...
        aldims_data['mf_dim'] = aldims_data['cusp_dim'] + \
            aldims_data['eis_dim']
...
class WebGamma1Space():
    def __init__(self, level, weight):
        ...
        newspaces = list(db.mf_newspaces.search(
            {'level': level, 'weight': weight, 'char_parity': self.weight_parity}))
        self.oldspaces = [(sublevel, number_of_divisors(level / sublevel))
                          for sublevel in divisors(level) if sublevel != level]
        self.oldspaces.sort()
        self.dim_grid = DimGrid.from_db(data)
        self.decomp = []
        newforms = list(db.mf_newforms.search({'level': level, 'weight': weight}, [
                        'label', 'space_label', 'dim', 'level', 'char_orbit_label', 'hecke_orbit', 'char_degree']))
        self.has_uncomputed_char = False
        if len(newspaces) == len(
                [dim for dim in self.newspace_dims if dim != 0]):
            for space in newspaces:
                if space.get('num_forms') is None:
                    ...
                    self.decomp.append(
                        (space, [form for form in newforms if form['space_label'] == space['label']]))
        else:
            char_orbits = list(db.char_dirichlet.search({'modulus': level}))
            # to match the full character orbit label
            newspaces_by_label = {
                str(level) +
                '.' +
                ns['char_orbit_label']: ns for ns in newspaces}
            # This relies on the fact that newspaces are sorted by
            # char_orbit_index, which is the default at the time of writing.
            newspace_dims_by_label = {
                char_orbits[i]['label']: self.newspace_dims[i] for i in range(
                    len(char_orbits))}
            for char in char_orbits:
                if char['label'] in newspaces_by_label:
                    ...
                    space['label'] = "%s.%s.%s" % (
                        level, weight, space['char_orbit_label'])
                    space['char_degree'] = char['degree']
                    space['dim'] = newspace_dims_by_label[char['label']]
                    space['generate_link'] = (
                        self.weight == 2) and (
                        space['char_orbit_label'] != 'a')
                    # generate_link is used in self.decomposition() as a marker of pages which can be generated dynamically.
                    # len(newspaces) == len([dim for dim in self.newspace_dims
                    # if dim != 0]) when there is an associated database entry,
                    # so the line above doesn't come up in those cases.
                    self.decomp.append((space, None))
                    self.has_uncomputed_char = True
        self.plot = db.mf_gamma1_portraits.lookup(
            self.label, projection="portrait")
        self.properties = [('Label', self.label), ]
        if self.plot is not None and self.dim > 0:
            self.properties += [
                (None, '<a href="{0}"><img src="{0}" width="200" height="200"/></a>'.format(self.plot))]
        self.properties += [
        ...
        self.downloads = [
            ('Trace form to text', url_for('cmf.download_traces', label=self.label)),
            ('All stored data to text', url_for(
                'cmf.download_full_space', label=self.label)),
    ...
    def subspace_latex(self, new=False):
        return common_latex("M", self.weight, None, "S",
                            1, "new" if new else "")

    def summand_latex(self, symbolic_chi=True):
        return common_latex(self.level, self.weight, 1, "S",
                            0, "new", symbolic_chi=symbolic_chi)

    def old_latex(self):
        return common_latex(*(self._vec() + ["S", 1, "old"]))

    def header_latex(self):
        return r'\(' + common_latex(*(self._vec() +
                                      ["S", 0, "new", True])) + r'\)'
    ...
    def decomposition(self):
        # returns a list of 6-tuples chi_rep, num_chi, space, firstform,
        # firstdim, forms
        ans = []
        for i, (space, forms) in enumerate(self.decomp):
            rowtype = "oddrow" if i % 2 else "evenrow"
            chi_str = r"\chi_{%s}(%s, \cdot)" % (
                space['level'], space['conrey_index'])
            ...
            if space.get('generate_link', True):
                ...
                link = "{N}.{k}.{i}".format(
                    N=space['level'],
                    k=self.weight,
                    i=space['char_orbit_label'])  # Not actually a link
            if forms is None:
                ans.append(
                    (rowtype, chi_rep, num_chi, link, "n/a", space['dim'], []))
            elif not forms:
                ans.append(
                    (rowtype, chi_rep, num_chi, link, "None", space['dim'], []))
            else:
                dims = [form['dim'] for form in forms]
                forms = [
                    self._link(
                        form['level'],
                        form['char_orbit_label'],
                        form['hecke_orbit']) for form in forms]
                ans.append((rowtype, chi_rep, num_chi, link,
                            forms[0], dims[0], list(zip(forms[1:], dims[1:]))))
        return ans
