...
class ArtinRepTest(LmfdbTest):
    ...
    def test_search_deg_condrange(self):
        L = self.tc.get(
            '/ArtinRepresentation/?dimension=3&conductor=1988-2015&group=&ramified=&unramified=&root_number=&frobenius_schur_indicator=&count=15')
        # Only 1 result at the time this test was written, which has conductor
        # 2009 = 7^2.41
        assert '41' in L.get_data(as_text=True)

    def test_search_gal_ram_rn_fn(self):
        L = self.tc.get(
            '/ArtinRepresentation/?dimension=&conductor=&group=11T5&ramified=43&unramified=&root_number=1&frobenius_schur_indicator=1&count=15')
        # prime divisor of one of the conductors in the result
        assert '2898947' in L.get_data(as_text=True)
    ...
    def test_underlying_data(self):
        data = self.tc.get(
            '/ArtinRepresentation/data/2.3806401.120.b').get_data(as_text=True)
