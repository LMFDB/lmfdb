...
def by_data(degree, size, index):
    nfgg_logger.debug("Asked for the Galois group of size %s on %s points, with index %s"%(size, degree, index))
...
def render_nfgg_webpage(degree,size,index):
  ...
  bread = get_bread([(str("Degree %s, size %s, index %s"%(the_nfgg.degree(),the_nfgg.size(),the_nfgg.index())), ' ')])
