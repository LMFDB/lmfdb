...
class MaassDB(object):
    ...
    def check_data(self,data):
        ...
        if not isinstance(data,dict):
            return 0
        if not data.has_key('level') or not data.has_key('R'):
            return 0
        if not data.has_key('err'):
            return 0
...
#              'R2':float(R2)}
        ...
        try:
            ...
            data['R1']=float(R1); data['R2']=float(R2)
    ...
    def deregister_coeff_work(self,data={}):
        ...
        if self._ongoing_coeff_work_id<>None:
    ...
    def show_history(self,sort=[]):
        if sort<>[]:
            ...
            sorting=[('Level',pymongo.ASCENDING),('Weight',pymongo.ASCENDING),('Character',pymongo.ASCENDING)]
...
#            s+="{0}\t{1}\t{2}\t{3}\t{4}\t{6}\t{7} \n".format(j,f['Level'],f['Weight'],f['Char'],f['R1'],f['R2'],f['date'],elapsed_t)
    ...
    def display_one_job_record(self,f):
        ...
        s="{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n".format(f.get('Level'),f.get('Weight',0),f.get('ch',0),f.get('R1',0),f.get('R2',0),date_beguns,date_stops,elapsed_t)
        return s
...
#            s+="{0}\t{1}\t{2}\t{3}\t{4}\t{6}\t{7} \n".format(j,f['Level'],f['Weight'],f['Char'],f['R1'],f['R2'],f['date'],elapsed_t)
    ...
    def display_one_progress_record(self,f):
        ...
        s="{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\n".format(f.get('Level'),f.get('Weight',0),f.get('Character',0),f.get('R1',0),f.get('R2',0),date_beguns,elapsed_t)
        return s
                        ...
                        for x in lx:
                            if also_empty<>1:
                                ...
                                if res[name][k][N][x][1]>0:
                                    s+= "\t "+str(x)+":"+str(res[name][k][N][x][0])+"("+str(res[name][k][N][x][1])+")"
                                else:
                                    s+= "\t "+str(x)+":"+str(res[name][k][N][x][0])
                        s+="\n"
                    s+=tstop
    ...
    def display_header(self,date=0):
        if date==1:
            ...
            s="{0:^7}{1:^7}{2:^7}{3:^20}{4:^10}{5:^7}{6:^7}{7:^15}{8:^20} \n".format('Level','Weight','Char','R','Even/Odd','Dim','Error','Num. coeff.','Cusp symmetries')
        return s
...
def setup_sqldb():
    ...
    DB.show('maass_forms')
    Q = SQLQuery(DB,{'table_name':'maass_forms','display_cols':['level'], 'expression':['level','=',1]})
...
def mongify_dict(data,lowercase=False):
    ...
    for k in data.keys():
        if isinstance(k,(int,float,str,unicode)):
            if isinstance(k,str) and lowercase==True:
                key=k.lower()
            else:
                key=k
