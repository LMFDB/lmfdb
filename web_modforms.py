...
class WebModFormSpace(Parent):
    ...
    def dimension_cuspforms(self):
        return self._fullspace.dimension()
    ...
    def sturm_bound(self):
        ...
        """
        return self._fullspace.sturm_bound()
    ...
    def oldspace_decomposition(self):
        r"""
        Get decomposition of the oldspace in self.fullspace into submodules.
...
class WebNewForm(SageObject):
    ...
    def __init__(self,k,N,chi=0,label='',fi=0,prec=10,bitprec=53,verbose=-1,data=None):
        ...
        try:
            if(len(label)>0): # the label takes priority
                ...
                for j in range(len(L)):
                    if(self._parent._galois_orbits_labels[j]==label):
                        self.f=self._parent._newforms[j]
            if(self.f == None):
                self.f=self._parent._newforms[fi]
        except IndexError:
            if(self._verbose>=0):
    ...
    def is_CM(self):
        ...
        if(len(self._is_CM)>0):
            return self._is_CM
        max_nump=self._number_of_hecke_eigenvalues_to_check()
        coeffs=self.f.coefficients(max_nump+1)
    ...
    def print_satake_parameters(self,type='alphas',prec=10,bits=53):
        satake=self.satake_parameters(prec,bits)
        tbl=dict()
        tbl['headersh']=satake[type][0].keys()
        #if(self.base_ring()==QQ):
        #    tbl['headersv']=[""]
        #    tbl['data']=[satake[type].values()]
        #    tbl['corner_label']="\(p\)"
        #else:
        tbl['atts']="border=\"1\""
        tbl['data']=list()
