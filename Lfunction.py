...
logger = utils.make_logger("LF", hl = True)


#
#   Possibly insecure code
#
#def old_get_attr_or_method_(thiswillbeexecuted, attr_or_method_name):
#    """
...
def get_attr_or_method_value(object, attr_or_method_name):
    """
        Given an object O and a string "text", this returns O.text() or O.text depending on
        whether text is an attribute or a method of O itself _or one of its superclasses_, which I will
        only know at running time. 
        
    """
    logging.critical(object, attr_or_method_name)
    if hasattr(object, attr_or_method_name):
        logging.critical("has "+attr_or_method_name)
        attr_or_method = getattr(object, attr_or_method_name)
        if callable(attr_or_method):
            logging.critical("identified as method")
            val = attr_or_method()
        else:
            logging.critical("identified as attr")
            val = attr_or_method
        return val
    else:
        return None
...
def constructor_logger(object, args):
    ...
    #    "lambda_fe", "mu_fe", "nu_fe", "sign", "weight"])
...
class Lfunction(object):
    ...
    def inject_database(self, relevant_info, time_limit = None):
        """
            ...
            The syntax requires relevant_info to be a list of methods or attributes _names_ of self
        ...
        update_dict = dict([(method_name,get_attr_or_method_value(self,method_name)) for method_name in relevant_info])
        
        logging.info("injecting " + str(update_dict))
        search_dict = {"original_mathematical_object": get_attr_or_method_value(self, "original_mathematical_object")}
...
class Lfunction_EC(Lfunction):
    ...
    def __init__(self, **args):
        ...
        self.sign = int(self.E.lseries().dokchitser().eps)
        self.kappa_fe = [1]
        self.lambda_fe = [0.5]
        self.mu_fe = []
        self.nu_fe = [0.5]
        ...
        self.integral_dirichlet_coefficients = self.E.anlist(self.numcoeff)[1:]  #remove a0

        self.dirichlet_coefficients = []
        
        # Renormalize the coefficients
        for n in range(0,len(self.integral_dirichlet_coefficients)-1):
           an = self.integral_dirichlet_coefficients[n]
           self.dirichlet_coefficients.append(float(an)/float(sqrt(n+1)))
