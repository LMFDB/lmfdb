...
class mf_hecke_cc(TableChecker):
    ...
    def check_lfunction_label(self):
        # check that lfunction_label is consistent with hecke_orbit_code, conrey_index, and embedding_index
        query = SQL("""SELECT t1.lfunction_label FROM mf_hecke_cc t1, mf_newforms t2 WHERE string_to_array(t1.lfunction_label,'.') != string_to_array(t2.label, '.') || ARRAY[t1.conrey_index::text, t1.embedding_index::text] AND t1.hecke_orbit_code = t2.hecke_orbit_code LIMIT 1""")
        cur = db._execute(query)
        if cur.rowcount > 0:
            return cur.fetchone()[0]
    ...
    def check_embedding_index(self):
        # check that embedding_index is consistent with conrey_index and embedding_m
        query = SQL("WITH foo AS ( SELECT lfunction_label, embedding_index, ROW_NUMBER() OVER ( PARTITION BY hecke_orbit_code, conrey_index  ORDER BY embedding_m) FROM mf_hecke_cc) SELECT lfunction_label FROM foo WHERE embedding_index != row_number LIMIT 1")
        cur = db._execute(query)
        if cur.rowcount > 0:
            return cur.fetchone()[0]
    ...
    def check_embedding_m(self):
        # check that embedding_m is consistent with conrey_index and embedding_index
        query = SQL("WITH foo AS ( SELECT lfunction_label, embedding_m, ROW_NUMBER() OVER ( PARTITION BY hecke_orbit_code ORDER BY conrey_index, embedding_index) FROM mf_hecke_cc) SELECT lfunction_label FROM foo WHERE embedding_m != row_number LIMIT 1")
        cur = db._execute(query)
        if cur.rowcount > 0:
            return cur.fetchone()[0]
