...
def HasFinitePointAt(F,p,c): # Tests whether y²=c*F(x) has a finite Qp-point with x and y both in Zp,
...
def IsSolubleAt(F,p): # Tests whether y²=F(x) has a Qp-point, assuming F integral of degree 6 and lc(F) squarefree
 ...
 if lc % p:
  ...
  # So A=1, and there must be cancellation mod p to prevent v_p(f(x))=5 --> x = -a5/a6 + O(p^0), and v_p(y) >= -2
  # Just renormalise
  a5 = F[F.degree()-1]
  if a5 % p == 0:
   return false
...
def InsolublePlaces(f,h=0): # List of primes at which the curve y²+h(x)*y=f(x) is not soluble
...
## The next functions are used to add the 'insoluble_places' entry to the curves in the database
...
def Process_One(c):
    ...
    P = [int(p) for p in P]
    c['insoluble_places'] = P
    return c

##########################

def Upload_One(g2,c,dry=true):
    Zx = PolynomialRing(ZZ,'x')
    label = str(c['label'])
    print "Updating",label
    [f,h] = eval(c['eqn'])
    P = InsolublePlaces(Zx(f),Zx(h))
    P = [int(p) for p in P]
    query = {'label':label}
    update = {'$set':{'insoluble_places':P}}
    if dry:
        print query,update
    else:
        g2.update_one(query,update)

def Upload_InsolublePlaces(dry=true):
    Zx = PolynomialRing(ZZ,'x')
    pw_dict = yaml.load(open("passwords.yaml"))
    username = pw_dict['data']['username'] 
    password = pw_dict['data']['password']
    load("data_mgt/utilities/browse.py")
    C['genus2_curves'].authenticate(username,password)
    g2 = genus2_curves.get_collection('curves')
    curves = g2.find()
    for c in curves:
        Upload_One(g2,c,dry)
